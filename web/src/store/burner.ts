...
import {
  worker_burner_dr_init_guest,
  worker_burner_dr_encrypt,
  worker_burner_dr_decrypt
...
export const useBurnerStore = createWithEqualityFn<BurnerState & BurnerActions>(
  (set, get) => ({
    ...initialState,
    ...
    initializeFromHash: async (hash: string) => {
      try {
        ...
        const [roomId, hostUserId, hostDeviceId, hostPqPk, hostClassicalPk] =
          parts
        ...
        const { state: stateFromWorker, guestClassicalPk: guestPk } =
        ...
        try {
          ...
          if (!state) {
            if (guestClassicalPk) {
              ...
              const guestPkBytes = sodium.from_base64(
                guestClassicalPk,
                sodium.base64_variants.URLSAFE_NO_PADDING
              )
              const savedCtBytes = sodium.from_base64(
                (header as { ct: string }).ct,
                sodium.base64_variants.URLSAFE_NO_PADDING
              )
          ...
          if (isHost) {
            ...
            useMessageStore
              ...
              .addOptimisticMessage(
                roomId,
                mainMsg as unknown as import('@nyx/shared').Message
              )
...
export async function generateBurnerLink(): Promise<string> {
  ...
  const myPqPkB64 = sodium.to_base64(
    myPqKeys.publicKey,
    sodium.base64_variants.URLSAFE_NO_PADDING
  )
  const b64Classical = sodium.to_base64(
    myDeviceKeys.publicKey,
    sodium.base64_variants.URLSAFE_NO_PADDING
  )
