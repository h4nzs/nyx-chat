...
export const useStoryStore = createWithEqualityFn<StoryState>((set, get) => ({
  ...
  fetchActiveStories: async (userId) => {
    ...
    try {
      ...
      const decryptedStories = await Promise.all(
        validStories.map(async (story) => {
          try {
            const base64Key = await getStoryKey(story.id)
            if (base64Key) {
              const decryptedData = await decryptStoryPayload(
                story.encryptedPayload,
                base64Key
              )
              return {
                ...story,
                decryptedData: decryptedData as unknown as Record<
                  string,
                  unknown
                >
              }
  ...
  postStory: async (file, text, privacy, selectedUserIds) => {
    const toastId = toast.loading(
      i18n.t('common:posting_story', 'Posting story...')
    )
    try {
      ...
      if (file) {
        ...
        if (file.type.startsWith('image/')) {
          try {
            fileToProcess = await compressImage(file, false)
          } catch (_e) {}
        }

        const { encryptedBlob, key: rawFileKey } =
          await encryptFile(fileToProcess)
        fileKey = rawFileKey
        mimeType = file.type
        ...
        if (!c.isGroup && c.participants) {
          const otherParticipant = c.participants.find(
            (p: Record<string, unknown>) => {
              const uId =
                p.userId || (p.user as Record<string, unknown>)?.id || p.id
              return uId !== me?.id
            }
          )
          ...
          if (otherParticipant) {
            const actualUserId =
              (otherParticipant as Record<string, unknown>).userId ||
              (
                (otherParticipant as Record<string, unknown>).user as Record<
                  string,
                  unknown
                >
              )?.id ||
              (otherParticipant as Record<string, unknown>).id
            if (actualUserId) {
      ...
      for (const targetId of targets) {
        ...
        if (convId) {
          try {
            ...
            console.error(
              `[Stories] Failed to send story key to user ${targetId}`,
              e
            )
          }
        }
      ...
      toast.success(i18n.t('common:posting_success', 'Story posted!'), {
        id: toastId
      })
    } catch (e: unknown) {
      console.error(e)
      toast.error(
        i18n.t(
          'common:posting_error',
          `Failed to post story: ${(e instanceof Error ? e.message : 'Unknown error') || 'Unknown error'}`
        ),
        { id: toastId }
      )
    }
