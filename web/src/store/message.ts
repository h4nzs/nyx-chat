...
function enrichMessagesWithSenderProfile(
  ...
  const participantsMap = new Map(
    conv.participants.map((p) => [
      ('userId' in p ? (p as unknown as { userId: string }).userId : p.id) ||
        p.id,
      p
    ])
  )
  ...
  return messages.map((m) => {
    ...
    const profileKey = Object.keys(cachedProfiles).find((k) =>
      k.startsWith(m.senderId)
    )
    ...
    const encryptedProfile =
      pInfo?.encryptedProfile || m.sender?.encryptedProfile

    // If we found any real name, apply it and preserve metadata
    if (
      resolvedName &&
      resolvedName !== 'Unknown' &&
      resolvedName !== 'Encrypted User'
    ) {
      return {
        ...m,
        sender: {
...
export async function decryptMessageObject(
  ...
  if (
    typeof rawMsg.tempId === 'number' &&
    Number.isSafeInteger(rawMsg.tempId)
  ) {
  ...
  let finalMessage: Message = {
    ...
    repliedToId: rawMsg.repliedToId
      ? asMessageId(rawMsg.repliedToId)
      : undefined,
  ...
  const conversation = useConversationStore
    .getState()
    .conversations.find((c) => c.id === rawMsg.conversationId)
  ...
  try {
    ...
    if (currentUser && rawMsg.senderId === currentUser.id) {
      ...
      if (mk) {
        const { worker_crypto_secretbox_xchacha20poly1305_open_easy } =
          await import('@lib/crypto-worker-proxy')
        const sodium = await getSodium()

        let cipherTextToUse: string | null | undefined = (
          'ciphertext' in rawMsg ? rawMsg.ciphertext : rawMsg.content
        ...
        if (cipherTextToUse) {
          try {
            ...
            const decryptedBytes =
              await worker_crypto_secretbox_xchacha20poly1305_open_easy(
                encrypted,
                nonce,
                mk
              )
            ...
            if (
              ...
              finalMessage.content.includes('"type":"file"')
            ) {
              try {
              ...
              finalMessage.content &&
              finalMessage.content.startsWith('{') &&
              finalMessage.content.includes('"type":"story_reply"')
            ) {
              try {
                const metadata = JSON.parse(finalMessage.content)
            ...
            if (rawMsg.repliedTo) {
              finalMessage.repliedTo = await decryptMessageObject(
                rawMsg.repliedTo as RawServerMessage,
                seenIds,
                depth + 1,
                options
              )
              ...
              try {
                ...
                const localRepliedMsg = await shadowVault.getMessage(
                  rawMsg.repliedToId
                )
                if (localRepliedMsg) finalMessage.repliedTo = localRepliedMsg
              } catch (e) {
                console.error(
                  '[Vault] Failed to fetch replied message locally',
                  e
                )
              }
            }
    ...
    let contentToDecrypt: string | undefined = (
      'ciphertext' in rawMsg ? rawMsg.ciphertext : undefined
    ) as string | undefined

    if (!contentToDecrypt) {
      contentToDecrypt = (('fileKey' in rawMsg ? rawMsg.fileKey : undefined) ||
        rawMsg.content) as string | undefined
    }

    if (
      !contentToDecrypt ||
      contentToDecrypt === 'waiting_for_key' ||
      contentToDecrypt === '[Requesting key to decrypt...]'
    ) {
      return finalMessage
    }
    ...
    if (
      !isGroup &&
      contentToDecrypt.startsWith('{') &&
      contentToDecrypt.includes('"x3dh":')
    ) {
      try {
        ...
        if (mk) {
          ...
          const { initiatorSigningKey, initiatorCiphertexts, otpkId } =
            payload.x3dh
          ...
          try {
            ...
            if (innerPayload.dr) {
              ...
              if (kemPk) {
                const sodium = await getSodium()
                theirRatchetPublicKey = sodium.from_base64(
                  kemPk,
                  sodium.base64_variants.URLSAFE_NO_PADDING
                )
              }
          ...
          if (!theirRatchetPublicKey) {
            throw new Error(
              "Cannot initialize Bob: Missing sender's ratchet key in first message."
            )
          }

          const { worker_dr_init_bob } =
            await import('@lib/crypto-worker-proxy')
        ...
        if (
          e instanceof Error &&
          (e.message.includes('Account upgrade required') ||
            e.message.includes('PQ keys missing'))
        ) {
          throw e
        }
      }
    ...
    const sessionOrSenderId = isGroup
      ? rawMsg.senderId
      : ('sessionId' in rawMsg ? rawMsg.sessionId : '') || ''
    ...
    if (result?.status === 'success') {
        ...
        try {
          ...
          if (parsed.profileKey) {
            ...
            useProfileStore
              ...
              .decryptAndCache(
                rawMsg.senderId,
                rawMsg.sender?.encryptedProfile || null
              )
      ...
      if (
        ...
        plainText.includes('"type":"story_reply"')
      ) {
        try {
          const metadata = JSON.parse(plainText)
          if (metadata.type === 'story_reply') {
      ...
      if (
        errMsg.includes('waiting for key') ||
        errMsg.includes('Missing sender')
      ) {
...
function parseReaction(
  ...
  try {
    ...
    if (!trimmed.startsWith('{') || !trimmed.includes('"type":"reaction"'))
      return null
...
function parseEdit(
  ...
  try {
    const trimmed = content.trim()
    if (!trimmed.startsWith('{') || !trimmed.includes('"type":"edit"'))
      return null
    const payload = JSON.parse(trimmed)
...
function parseSilent(
  ...
  try {
    ...
    if (
      [
        'PROTOCOL_UPGRADE_REQ',
        'PROTOCOL_UPGRADE_ACK',
        'PROTOCOL_DOWNGRADE'
...
// Helper to separate messages and reactions
function processMessagesAndReactions(
    ...
    if (reactionPayload) {
      ...
      if (silentPayload) {
        if (
          ...
          silentPayload.key &&
          silentPayload.storyId
        ) {
          // Save the story key
          saveStoryKey(silentPayload.storyId, silentPayload.key).catch((e) =>
            console.error('Failed to save story key from history', e)
          )
          continue
        }
        ...
        if (
          silentPayload.type === 'reaction_remove' &&
          silentPayload.targetMessageId &&
          silentPayload.emoji
        ) {
  ...
  const messageMap = new Map(
    [...existingMessages, ...chatMessages].map((m) => [m.id, m])
  )
  ...
  for (const msg of messageMap.values()) {
    for (const r of msg.reactions || []) {
      ...
      reactionActionMap.set(key, {
        action: 'add',
        ts: rObj.createdAt ? new Date(rObj.createdAt).getTime() : Date.now()
      })
    }
  }
  ...
  for (const reaction of reactions) {
    ...
    const eventTs = rObj.createdAt
      ? new Date(rObj.createdAt).getTime()
      : Date.now()
  ...
  for (const rr of reactionRemoves) {
    ...
    const eventTs = rrObj.createdAt
      ? new Date(rrObj.createdAt).getTime()
      : Date.now()
    const existing = reactionActionMap.get(key)
    if (!existing || eventTs >= existing.ts) {
  ...
  for (const reaction of reactions) {
    ...
    if (target) {
      const key = `${reaction.messageId}|${reaction.userId}|${reaction.emoji}`
      if (
        reactionActionMap.get(key)?.action === 'add' &&
        !target.reactions?.some((r) => r.id === reaction.id)
      ) {
        target.reactions = [
          ...(target.reactions || []),
          { ...reaction, userId: asUserId(reaction.userId) }
        ]
      }
    }
  }
  ...
  for (const un of unsends) {
    ...
    if (target && target.senderId === un.senderId) {
      ...
      import('@lib/shadowVaultDb').then((m) =>
        m.shadowVault.upsertMessages([
          {
            id: targetId,
...
type Actions = {
  ...
  addLocalReaction: (
    conversationId: string,
    messageId: string,
    reaction: { id: string; emoji: string; userId: string; isMessage?: boolean }
  ) => void
  removeLocalReaction: (
    conversationId: string,
    messageId: string,
...
const pendingStatuses: Record<
  string,
  { conversationId: string; userId: string; status: string }
> = {}

// Helper for evaluating control messages
const evaluateControlMessage = async (
  decrypted: Message,
  conversationId: string
): Promise<boolean> => {
  if (
    (decrypted as Record<string, unknown>).type === 'STORY_KEY' ||
    (decrypted.content && decrypted.content.startsWith('STORY_KEY:'))
  ) {
    try {
      const payloadStr = decrypted.content
        ? decrypted.content.replace('STORY_KEY:', '')
        : ''
      ...
      if (payload.storyId && payload.key) {
        ...
        console.log(
          `[Stories] Received and securely stored key for story ${payload.storyId}`
        )
      }
    } catch (e) {
  ...
  if (decrypted.content && decrypted.content.startsWith('{')) {
    try {
      ...
      if (
        data.type === 'PROTOCOL_UPGRADE_REQ' ||
        data.type === 'PROTOCOL_UPGRADE_ACK' ||
        data.type === 'PROTOCOL_DOWNGRADE'
      ) {
        const { shadowVault } = await import('@lib/shadowVaultDb')
        ...
        if (currentSession && messageVersion <= (currentSession.version || 0)) {
          console.log(
            `[Shield] Ignoring replay attack or old control message. Msg version: ${messageVersion}, Current: ${currentSession.version}`
          )
          return true
        }
      }
      ...
      if (
        ...
        type WindowWithPending = Window &
          typeof globalThis &
          Record<string, unknown>
        const isPending = (window as WindowWithPending)[
          `pendingPqUpgrade_${conversationId}`
        ]
        ...
        if (isPending) {
          delete (window as WindowWithPending)[
            `pendingPqUpgrade_${conversationId}`
          ]
        }

        const { worker_burner_dr_init_guest } =
          await import('@lib/crypto-worker-proxy')
        const { getSodiumLib } = await import('@utils/crypto')
        const sodium = await getSodiumLib()

        const hostClassicalPkBytes = sodium.from_base64(
          data.hostClassicalPk,
          sodium.base64_variants.URLSAFE_NO_PADDING
        )
        const hostPqPkBytes = sodium.from_base64(
          data.hostPqPk,
          sodium.base64_variants.URLSAFE_NO_PADDING
        )

        const { state: newState, guestClassicalPk: guestPk } =
          await worker_burner_dr_init_guest({
        ...
        useConversationStore
          ...
          .updateConversation(conversationId, {
            encryptionMode: 'PQ_DR',
            activePqDeviceId: peerDeviceId
          })
      ...
      if (
        data.type === 'PROTOCOL_UPGRADE_ACK' &&
        data.savedCt &&
        data.guestClassicalPk
      ) {
        const { worker_burner_dr_init_host } =
          await import('@lib/crypto-worker-proxy')
        const { getSodiumLib, getMyEncryptionKeyPair } =
        ...
        const guestClassicalPkBytes = sodium.from_base64(
          data.guestClassicalPk,
          sodium.base64_variants.URLSAFE_NO_PADDING
        )
        const savedCtBytes = sodium.from_base64(
          data.savedCt,
          sodium.base64_variants.URLSAFE_NO_PADDING
        )
        ...
        useConversationStore
          .getState()
          .updateConversation(conversationId, { encryptionMode: 'PQ_DR' })
        return true
      }
      ...
      if (data.type === 'PROTOCOL_DOWNGRADE') {
        ...
        useConversationStore
          ...
          .updateConversation(conversationId, {
            ...
            activePqDeviceId: null
          })
        return true
      }
    } catch (e) {
      console.error(
        `[Shield] Error processing protocol message for conversation ${conversationId}`,
        { error: e, content: decrypted.content }
      )
    }
  }
  return false
...
export const useMessageStore = createWithEqualityFn<State & Actions>(
  (set, get) => ({
    ...initialState,
    ...
    repairSecureSession: async (conversationId, isGroup, isAuto = false) => {
      try {
        ...
        const { forceRotateGroupSenderKey, rotateGroupKey } =
          await import('@utils/crypto')
        ...
        if (!isAuto) {
          toast.success(
            i18n.t(
              'common:secure_session_state_reset_next_message_',
              'Secure session state reset. Next message will negotiate new keys.'
            )
          )
        }
      } catch (error) {
        ...
        if (!isAuto)
          toast.error(
            i18n.t(
              'errors:failed_to_repair_session',
    ...
    broadcastHistorySync: async (selfConversationId) => {
      ...
      const toastId = toast.loading(
        i18n.t(
          'common:preparing_history_sync_for_your_linked_d',
          'Preparing history sync for your linked devices...'
        )
      )
      try {
        ...
        toast.success(
          i18n.t(
            'common:history_sync_success',
            'History securely synced to your linked devices!'
          ),
          { id: toastId }
        )
      } catch (e) {
        console.error(e)
        toast.error(
          i18n.t('common:history_sync_failure', 'Failed to sync history.'),
          { id: toastId }
        )
      }
    },
    ...
    removeMessages: async (conversationId, messageIds) => {
      ...
      const selectedMessages = currentMessages.filter((m) =>
        messageIds.includes(m.id)
      )

      // Check if all selected messages are mine
      const allMine =
        user && selectedMessages.every((m) => m.senderId === user.id)
      ...
      if (allMine) {
        selectedMessages.forEach((message) => {
          ...
          api(`/api/messages/${message.id}${query}`, {
            ...
            console.error(
              `Failed to delete message ${message.id} from server:`,
              error
            )
          })
        })
      ...
      set((state) => {
        ...
        return {
          messages: {
            ...state.messages,
            [conversationId]: current.filter((m) => !messageIds.includes(m.id))
          },
    ...
    fetchTypingLinkPreview: async (text: string) => {
      try {
        const res = await api('/api/previews/link', {
          method: 'POST',
    ...
    sendMessage: async (
      ...
      if (conversationId.startsWith('burner_')) {
        ...
        try {
          if (
            lastMsgPreview?.startsWith('{') &&
            lastMsgPreview.includes('"type":"file"')
          ) {
        ...
        try {
          const { worker_burner_dr_encrypt } =
            await import('../lib/crypto-worker-proxy')
          const {
            state: newState,
            header,
            ciphertext
          } = await worker_burner_dr_encrypt({
            state,
          ...
          get().updateMessage(conversationId, `temp_${actualTempId}`, {
            status: 'SENT'
          })
        } catch (e) {
      ...
      if (!hasRestoredKeys) {
        toast.error(
          i18n.t(
            'errors:you_must_restore_your_keys_from_your_rec',
            'You must restore your keys from your recovery phrase before you can send messages.'
          )
        )
        return
      }
      ...
      if (!conversation) {
        toast.error(
          i18n.t('errors:conversation_not_found', 'Conversation not found.')
        )
        return
      }
      ...
      try {
        ...
        if (distributionKeys && distributionKeys.length > 0) {
          ...
          if (forceRotate) {
            useConversationStore
              .getState()
              .markKeyRotationNeeded(conversationId, false)
          }
        ...
        toast.error(
          i18n.t(
            'errors:failed_to_establish_secure_session',
      ...
      const isProtocolUpgradeReq =
        silentPayload?.type === 'PROTOCOL_UPGRADE_REQ'
      const isProtocolUpgradeAck =
        silentPayload?.type === 'PROTOCOL_UPGRADE_ACK'
      ...
      const shouldBeSilent =
        ...
        isEditPayload ||
        isReactionPayload ||
        isProtocolUpgradeReq ||
        isProtocolUpgradeAck ||
        isProtocolDowngrade
      ...
      if (!shouldBeSilent) {
          ...
          statuses: [
            {
              ...
              status: 'READ',
              messageId: asMessageId(`temp_${actualTempId}`),
              id: `temp_status_${actualTempId}`,
        ...
        if (
          ...
          optimisticContent.startsWith('{') &&
          optimisticContent.includes('"type":"story_reply"')
        ) {
          try {
        ...
        try {
          if (
            ...
            lastMsgPreview?.startsWith('{') &&
            lastMsgPreview.includes('"type":"story_reply"')
          ) {
      ...
      try {
        ...
        if (contentToEncrypt) {
          try {
            const profileKey = await import('@lib/keychainDb').then((m) =>
              m.getProfileKey(user.id)
            )
            if (profileKey) {
              let parsedObj: Record<string, unknown> | null = null
              if (contentToEncrypt.trim().startsWith('{')) {
                try {
                  parsedObj = JSON.parse(contentToEncrypt)
                } catch (e) {}
              }
        ...
        try {
          ...
          if (myAuthUser?.encryptedProfile) {
            try {
              ...
              const myDecrypted = await profileStore.decryptAndCache(
                myAuthUser.id,
                myAuthUser.encryptedProfile
              )
          ...
          if (!isSilentMessage) {
            ...
            if (
              ...
              data.content.includes('"type":"story_reply"')
            ) {
              try {
                ...
                for (const device of targetDevices) {
                  ...
                  if (targetPublicKey) {
                    try {
                      ...
                      try {
                        recipientPubBytes = sodium.from_base64(
                          targetPublicKey,
                          sodium.base64_variants.URLSAFE_NO_PADDING
                        )
                      } catch (e1) {
                      ...
                      const sealed = await worker_crypto_box_seal(
                        pushDataBytes,
                        recipientPubBytes
                      )
                      pushPayloads[device.id] = sodium.to_base64(
                        sealed,
                        sodium.base64_variants.URLSAFE_NO_PADDING
                      )
                    } catch (e) {
                      console.error(
                        `Failed to seal push for device ${device.id}`,
                        e
                      )
                    }
                  }
        ...
        const payload = {
          ...
          fileSize: undefined,
          pushPayloads:
            Object.keys(pushPayloads).length > 0 ? pushPayloads : undefined
        }
        ...
        socket
          ...
          .emit(
            ...
            async (
              err: Error | null,
              res: { ok: boolean; msg?: RawServerMessage; error?: string }
            ) => {
              if (err) {
                ...
                if (!isReactionPayload && !shouldBeSilent) {
                  ...
                  toast.error(
                    i18n.t(
                      'errors:failed_to_send_message_timeout',
                  ...
                  if (reactionData) {
                    get().removeLocalReaction(
                      conversationId,
                      reactionData.targetMessageId,
                      `temp_react_${actualTempId}`
                    )
                  }
                }
                return
              ...
              if (res.ok && res.msg) {
                ...
                import('@utils/crypto')
                  .then(
                    async ({
                      ...
                      const mk = await retrieveMessageKeySecurely(
                        `temp_${actualTempId}`
                      )
                      if (mk) {
                ...
                const existingMsg = get().messages[conversationId]?.find(
                  (m) => m.id === tempIdStr || m.tempId === actualTempId
                )
                ...
                if (
                  ...
                  finalContent.includes('"ciphertext"')
                ) {
                  finalContent = '🔒 You sent this message (Encrypted)'
                  import('@utils/crypto')
                    .then(async ({ retrieveMessageKeySecurely }) => {
                      try {
                        ...
                        if (mk) {
                          ...
                          const sodium =
                            await import('@lib/sodiumInitializer').then((m) =>
                              m.getSodium()
                            )
                          ...
                          if (ciphertext) {
                            ...
                            get().updateMessage(conversationId, res.msg!.id, {
                              content: plainText
                            })
                          }
                        }
                        ...
                        console.error(
                          'Async self-decrypt failed in callback:',
                          e
                        )
                      }
                    })
                ...
                if (!isReactionPayload && !shouldBeSilent) {
                  ...
                  if (res.error?.includes('SANDBOX_LIMIT_REACHED')) {
                    toast.error(
                      i18n.t(
                        'errors:sandbox_limit_reached_verify_your_accoun',
                        'Sandbox limit reached! Verify your account to unlock unlimited messaging.'
                      )
                    )
                  } else if (res.error) {
                  ...
                  toast.error(
                    i18n.t(
                      'errors:failed_to_send_reaction',
    ...
    processOfflineQueue: async () => {
      ...
      for (const item of queue) {
        ...
        if (attempt > 5) {
          console.warn(
            `[Queue] Dropping message ${tempId} after too many retries.`
          )
          await removeFromQueue(tempId)
          get().updateMessage(conversationId, `temp_${tempId}`, {
            error: true,
            status: 'FAILED'
          })
          continue
        ...
        await new Promise<void>((resolve) => {
          const timeoutId = setTimeout(() => {
            console.error(
              `[Queue] Timeout waiting for ACK for message ${tempId}`
            )
            ...
            async (res: {
              ...
              if (res.ok && res.msg) {
                ...
                const isProtocolDowngrade =
                  silentPayload?.type === 'PROTOCOL_DOWNGRADE'

                const shouldBeSilent =
                  payloadData.isSilent ||
                  isCallInit ||
                  isGhostSync ||
                  isUnsend ||
                  isReactionRemove ||
                ...
                try {
                  if (
                    ...
                    payloadData.content.startsWith('{')
                  ) {
                  ...
                  import('@utils/crypto')
                    .then(async ({ retrieveMessageKeySecurely }) => {
                      try {
                        ...
                        console.error(
                          'Async self-decrypt failed in offline queue callback:',
                          e
                        )
                      }
                    })
                ...
                console.error(
                  `[Queue] Failed to send queued message ${tempId}:`,
                  res.error
                )
    ...
    uploadFile: async (conversationId, file) => {
      ...
      if (!hasRestoredKeys) {
        toast.error(
          i18n.t(
            'errors:you_must_restore_your_keys_from_your_rec',
            'You must restore your keys from your recovery phrase before you can send files.'
          )
        )
        return
      }
        ...
        statuses: [
          {
            userId: asUserId(user.id),
            status: 'READ',
            messageId: asMessageId(`temp_${tempId}`),
            id: `temp_status_${tempId}`,
            updatedAt: new Date().toISOString()
          }
        ],
        fileName: file.name,
    ...
    loadMessagesForConversation: async (id) => {
        ...
        set((state) => ({
          messages: {
            ...
            [id]: [
              {
                id: 'msg-1',
                content: 'Welcome to NYX. No active chats found.',
                senderId: 'bot-1',
                createdAt: new Date().toISOString(),
                conversationId: id,
                type: 'SYSTEM'
      ...
      try {
        const localMessagesRaw = await shadowVault.getMessagesByConversation(
          id,
          50
        )
        if (localMessagesRaw.length > 0) {
          localWasEmpty = false
          const localProcessed = processMessagesAndReactions(
            localMessagesRaw,
            []
          )
          const localEnriched = enrichMessagesWithSenderProfile(
            id,
            localProcessed
          )
          set((state) => ({
        ...
        console.error(
          '[Local Vault] Failed to load messages from IndexedDB:',
          e
        )
      }
      ...
      try {
        ...
        const res = await api<{ items: Message[] }>(
          `/api/messages/${id}?limit=50`
        )
        ...
        if (fetchedMessages.length > 0) {
          ...
          for (const message of fetchedMessages) {
            if (
              message.type === 'SYSTEM' &&
              message.content &&
              message.content.includes('GROUP_KEY_DISTRIBUTION')
            ) {
          ...
          for (const msg of controlMessages) {
            try {
              ...
              if (payload.type === 'GROUP_KEY_DISTRIBUTION') {
                type AuthUserWithDevices = {
                  devices?: { id: string; isCurrent?: boolean }[]
                }
                const devices = (
                  useAuthStore.getState().user as AuthUserWithDevices | null
                )?.devices
                const myDevice = Array.isArray(devices)
                  ? devices.find((d) => d.isCurrent)?.id
                  : undefined

                // Fallback for broadcast or specific device targeting
                if (
                  !payload.targetDeviceId ||
                  payload.targetDeviceId === myDevice
                ) {
                  const { storeReceivedSessionKey } =
                    await import('@utils/crypto')
          ...
          for (const message of normalMessages) {
            try {
              ...
              const isLocalValid =
                ...
                ![
                  'waiting_for_key',
                  '[Decryption Failed: Key out of sync]',
                  '🔒 Decryption Error',
                  '<Decryption Failed>'
                ].includes(localMessage.content || '')
              ...
              const decrypted = await decryptMessageObject(
                message,
                undefined,
                0,
                { skipRetries: true }
              ...
              console.error(
                `[Decrypt] Error decrypting message ${message.id}:`,
                err
              )
          ...
          for (const m of combined) {
            ...
            if (existing) {
              const existingIsValid =
                (existing.content || existing.fileMeta || existing.fileUrl) &&
                ![
                  'waiting_for_key',
                  '[Decryption Failed: Key out of sync]',
                  '🔒 Decryption Error'
                ].includes(existing.content || '')
              const mIsFailure =
                (!m.content && !m.fileMeta && !m.fileUrl) ||
                [
                  'waiting_for_key',
                  '[Decryption Failed: Key out of sync]',
                  '🔒 Decryption Error'
              ...
              if (existingIsValid && mIsFailure) {
                uniqueMessagesMap.set(m.id, {
                  ...existing,
                  repliedTo: existing.repliedTo || m.repliedTo
                })
              } else if (!existingIsValid && !mIsFailure) {
                uniqueMessagesMap.set(m.id, {
                  ...m,
                  repliedTo: m.repliedTo || existing.repliedTo
                })
              } else {
                uniqueMessagesMap.set(m.id, {
                  ...existing,
          ...
          const enrichedMessages = enrichMessagesWithSenderProfile(
            id,
            allMessages
          )
    ...
    loadPreviousMessages: async (conversationId) => {
      ...
      try {
        ...
        const localMessages = await shadowVault.getMessagesByConversation(
          conversationId,
          50,
        ...
        set((state) => {
          ...
          if (enrichedMessages.length > MAX_MESSAGES_IN_RAM) {
            prunedMessages = enrichedMessages.slice(
              enrichedMessages.length - MAX_MESSAGES_IN_RAM
            )
          }
          ...
          return {
            ...
            hasMore: {
              ...state.hasMore,
              [conversationId]: localMessages.length === 50
            }
          }
        })
      } catch (error) {
        console.error(
          'Failed to load previous messages from Local Vault',
          error
        )
      } finally {
        set((state) => ({
    ...
    loadMessageContext: async (messageId: string) => {
      try {
        ...
        if (!targetMessage || !targetMessage.conversationId) {
          console.warn(
            `[Local Vault] Pesan ${messageId} tidak ditemukan di database lokal.`
          )
          return
        }
        ...
        console.error(
          `Failed to load context for message ${messageId} from Local Vault`,
          error
        )
      }
    },
    ...
    addIncomingMessage: async (conversationId, message) => {
      const previousLock =
        incomingMessageLocks.get(conversationId) || Promise.resolve()
      let release: () => void
    ...
    doAddIncomingMessage: async (conversationId, message) => {
      ...
      if (decrypted.content === 'waiting_for_key' || decrypted.error) {
        console.log(
          `[Ratchet] Decryption failed for ${message.id}. Retrying once in 500ms...`
        )
      ...
      if (decrypted.repliedToId && !decrypted.repliedTo) {
        try {
          const repliedMessage = await shadowVault.getMessage(
            decrypted.repliedToId
          )
          if (repliedMessage) {
      ...
      if (
        decrypted.error ||
        decrypted.content === 'waiting_for_key' ||
        decrypted.content?.startsWith('[')
      ) {
        const existing = await shadowVault.getMessage(decrypted.id)
        if (
          existing &&
          !existing.isDeletedLocal &&
          existing.content &&
          !existing.content.startsWith('[')
        ) {
          console.warn(
            `[Shield] Prevented overwriting valid local message ${decrypted.id} with failure.`
          )
          return existing
        }
      ...
      if (silentPayload) {
        ...
        if (silentPayload.type === 'GHOST_SYNC') {
          cleanUpOptimisticBubble() // ✅ Bersihkan
          console.log(
            `[Ghost Sync] Received sync from ${decrypted.senderId}. Settle ratchet state silently.`
          )
          return decrypted
        }

        if (
          silentPayload.type === 'HISTORY_SYNC' &&
          silentPayload.url &&
          silentPayload.key
        ) {
          const currentUser = useAuthStore.getState().user
          ...
          if ('syncId' in silentPayload && silentPayload.syncId === mySyncId) {
            console.log(
              '[History Sync] Ignored own sync payload on current device'
            )
            return decrypted
          }
          ...
          console.log(
            `[History Sync] Received payload from ${decrypted.senderId}. Downloading...`
          )
          ...
          setTimeout(async () => {
            ...
            try {
              ...
              toast.success('History synchronized successfully! Reloading...', {
                id: toastId
              })
              ...
              console.error(
                '[History Sync] Failed to process history payload:',
                e
              )
            ...
            const updatedMessages = currentMessages.map((m) => {
              if (m.id === silentPayload.targetMessageId) {
                const newReactions =
                  m.reactions?.filter(
                    (r) =>
                      r.userId !== decrypted.senderId ||
                      r.emoji !== silentPayload.emoji
                  ) || []
      ...
      if (reactionPayload) {
          ...
          shadowVault
            ...
            .then((targetMsg) => {
              if (targetMsg) {
                ...
                shadowVault.upsertMessages([
                  { ...targetMsg, reactions: updatedReactions }
                ])
              }
            })
            .catch(console.error)
        ...
        set((state) => {
          const currentMessages = state.messages[conversationId] || []
          const updatedMessages = currentMessages.map((m) =>
            m.id === editPayload.targetMessageId
              ? { ...m, content: editPayload.text, isEdited: true }
              : m
          )
          const editedMsg = updatedMessages.find(
            (m) => m.id === editPayload.targetMessageId
          )
          if (editedMsg) {
            ...
            import('@store/conversation')
              .then((m) => {
                const conv = m.useConversationStore
                  .getState()
                  .conversations.find((c) => c.id === conversationId)
                if (conv?.lastMessage?.id === editPayload.targetMessageId) {
                  m.useConversationStore
                    .getState()
                    .updateConversationLastMessage(conversationId, editedMsg)
                }
              })
              .catch(console.error)
          }
        ...
        // ==========================================
        const [enriched] = enrichMessagesWithSenderProfile(conversationId, [
          decrypted
        ])
        ...
        if (
          ...
          set((state) => {
            ...
            return {
              messages: {
                ...state.messages,
                [conversationId]: [...currentMessages, finalDecrypted]
              }
            }
          })

          // --- Logika Notifikasi Dynamic Island ---
          const isViewingChat = window.location.pathname.includes(
            `/chat/${finalDecrypted.conversationId}`
          )
          if (
            !isViewingChat &&
            !finalDecrypted.isSilent &&
            ...
            import('@store/dynamicIsland')
              .then(({ default: useDynamicIslandStore }) => {
                const sender = finalDecrypted.sender as unknown as {
                ...
                if (finalDecrypted.fileUrl || finalDecrypted.isBlindAttachment)
                  snippet = 'Sent an attachment 📎'
                if (
                  finalDecrypted.content &&
                  finalDecrypted.content.startsWith('🔒')
                )
                ...
                useDynamicIslandStore.getState().addActivity(
                  {
                    ...
                    ReturnType<
                      typeof useDynamicIslandStore.getState
                    >['addActivity']
                  >[0],
                  4000
                )
          ...
          if (!finalDecrypted.isSilent) {
            import('@store/conversation')
              .then((m) => {
                m.useConversationStore
                  .getState()
                  .updateConversationLastMessage(conversationId, finalDecrypted)
              })
              .catch(console.error)
          }
        }
    ...
    replaceOptimisticMessage: async (conversationId, tempId, newMessage) => {
      ...
      const existingTombstone =
        (await shadowVault.getMessage(tempIdStr)) ||
        (await shadowVault.getMessage(tempIdDashStr))
      ...
      if (existingTombstone && existingTombstone.isDeletedLocal) {
        ...
        await shadowVault.upsertMessages([
          {
            ...newMessage,
            id: newMessage.id!,
            conversationId,
      ...
      set((state) => {
        ...
        const pendingKey = newMsgIdStr
          ? `${newMessage.conversationId}:${newMsgIdStr}`
          : ''
        ...
        const finalMessage: Message = {
          ...
          content:
            newMessage.content !== undefined
              ...
              : oldMsg?.content,
          fileUrl:
            newMessage.fileUrl !== undefined
              ? newMessage.fileUrl
              : oldMsg?.fileUrl,
          fileKey:
            newMessage.fileKey !== undefined
              ? newMessage.fileKey
              : oldMsg?.fileKey,
          fileName:
            newMessage.fileName !== undefined
              ...
              : oldMsg?.fileName,
          fileType:
            newMessage.fileType !== undefined
              ? newMessage.fileType
              : oldMsg?.fileType,
          fileSize:
            newMessage.fileSize !== undefined
              ? newMessage.fileSize
              : oldMsg?.fileSize,
          duration:
            newMessage.duration !== undefined
              ? newMessage.duration
              : oldMsg?.duration,
          isBlindAttachment:
            newMessage.isBlindAttachment !== undefined
              ? newMessage.isBlindAttachment
              : oldMsg?.isBlindAttachment,
          repliedTo:
            newMessage.repliedTo !== undefined
        ...
        return {
          messages: {
            ...state.messages,
            [conversationId]: [...filteredMessages, finalMessage].sort(
              (a, b) =>
          ...
          [conversationId]: (state.messages[conversationId] || []).map((m) => {
            if (m.id === messageId) {
              ...
              if (!newReactions.some((r) => r.id === reaction.id)) {
                newReactions.push({
                  ...reaction,
                  userId: asUserId(reaction.userId)
                })
              }
              ...
              return {
                ...
                reactions: (m.reactions || []).map((r) =>
                  r.id === tempId
                    ? {
                        ...finalReaction,
                        userId: asUserId(finalReaction.userId)
                      }
                    : r
                )
              }
      ...
      set((state) => {
        ...
        for (const convoId in newMessages) {
          newMessages[convoId] = newMessages[convoId].map((m) =>
            m.sender?.id === user.id
              ? { ...m, sender: { ...(m.sender || { id: user.id }), ...user } }
              : m
          ) as Message[]
        }
    ...
    updateMessageStatus: (conversationId, messageId, userId, status) => {
      set((state) => {
        ...
        if (msgToSave) {
          ...
          import('@lib/shadowVaultDb').then((m) =>
            m.shadowVault.upsertMessages([savedObj])
          )
        }
