...
export const usePresenceStore = create<State>((set) => ({
  ...
  userLeft: (userId) =>
    set((state) => {
      ...
      const newTypingIndicators = state.typingIndicators.filter(
        (i) => i.id !== userId
      )
      return {
  ...
  addOrUpdate: (indicator) =>
    set((state) => {
      const existing = state.typingIndicators.find(
        (i) =>
          i.id === indicator.id && i.conversationId === indicator.conversationId
      )
      if (existing) {
        if (!indicator.isTyping) {
          return {
            typingIndicators: state.typingIndicators.filter(
              (i) =>
                i.id !== indicator.id ||
                i.conversationId !== indicator.conversationId
            )
          }
        }
        return state
