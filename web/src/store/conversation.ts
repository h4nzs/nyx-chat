...
import type {
  ...
  Message,
  Participant,
  ConversationUi as Conversation
} from '@nyx/shared'
...
const sortConversations = (
  ...
  [...list].sort((a, b) => {
    ...
    const aIsPinned = a.participants.some(
      (p) => p.id === currentUserId && p.isPinned
    )
    const bIsPinned = b.participants.some(
      (p) => p.id === currentUserId && p.isPinned
    )
...
const withPreview = (msg: Message): Message => {
  ...
  if (msg.fileUrl) {
    if (msg.fileType?.startsWith('image/'))
      return { ...msg, preview: '📷 Image' }
    if (msg.fileType?.startsWith('video/'))
      return { ...msg, preview: '🎥 Video' }
...
export const useConversationStore = createWithEqualityFn<State & Actions>(
  (set, get) => ({
    ...initialState,
    ...
    markKeyRotationNeeded: (id, needed) =>
      set((s) => ({
        conversations: s.conversations.map((c) =>
          c.id === id ? { ...c, requiresKeyRotation: needed } : c
        )
      })),
    ...
    searchUsers: async (query) => {
      try {
        ...
        const searchQuery = isAlreadyHash
          ? trimmedQuery
          : await import('@lib/crypto-worker-proxy').then((m) =>
              m.hashUsername(trimmedQuery)
            )
    ...
    upgradeToPqDr: async (conversationId: string) => {
      ...
      ;(window as WindowWithPending)[`pendingPqUpgrade_${conversationId}`] =
        true
      ...
      try {
        ...
        const hostClassicalPk = sodium.to_base64(
          myClassicalKeys.publicKey,
          sodium.base64_variants.URLSAFE_NO_PADDING
        )
        const hostPqPk = sodium.to_base64(
          myPqKeys.publicKey,
          sodium.base64_variants.URLSAFE_NO_PADDING
        )
    ...
    loadConversations: async () => {
      if (sessionStorage.getItem('nyx_decoy_mode') === 'true') {
        const dummyConvo = {
          ...
          unreadCount: 0,
          participants: [
            { id: 'bot-1', username: 'system_bot', name: 'NYX Service' }
          ],
          ...
          lastMessage: {
            ...
            senderId: 'bot-1',
            createdAt: new Date().toISOString(),
            conversationId: 'decoy-1',
            type: 'SYSTEM'
          }
        }
        set({
          conversations: [dummyConvo as unknown as Conversation],
          loading: false,
      ...
      try {
        ...
        const conversations = await Promise.all(
          rawConversations.map(async (c) => {
            ...
            try {
              const localMsgs = await shadowVault.getMessagesByConversation(
                c.id,
                1
              )
            ...
            const serverMsgTime = lastMessage
              ? new Date(lastMessage.createdAt).getTime()
              : 0
            const localMsgTime = localLastMessage
              ? new Date(localLastMessage.createdAt).getTime()
              : 0

            let finalLastMessage =
              localMsgTime > serverMsgTime ? localLastMessage : lastMessage

            if (finalLastMessage) {
              const pInfo = participants.find(
                (p) => p.id === finalLastMessage!.senderId
              )
              if (pInfo) {
                finalLastMessage.sender = {
                  ...(finalLastMessage.sender || {
                    id: finalLastMessage.senderId
                  }),
                  ...pInfo
                }
              }
            ...
            if (c.isGroup && c.encryptedMetadata) {
              try {
                const decrypted = await decryptGroupMetadata(
                  c.encryptedMetadata,
                  c.id
                )
        ...
        const reconciledConversations = await Promise.all(
          conversations.map(async (fetched) => {
            if (!fetched.isGroup) {
            ...
            if (fetched.isGroup) {
              ...
              if (existing) {
                const existingIds = existing.participants
                  ...
                  .sort()
                  .join(',')
                const fetchedIds = fetched.participants
                  .map((p) => p.id)
                  .sort()
                  .join(',')
    ...
    deleteConversation: async (id) => {
      ...
      try {
        ...
        const errorMessage =
          ...
          i18n.t(
            'errors:failed_to_delete_conversation',
    ...
    deleteGroup: async (id) => {
      try {
        ...
        if (
          ...
          toast.error(
            i18n.t(
              'errors:only_the_group_creator_can_delete_the_gr',
    ...
    startConversation: async (
      ...
      if (!user) {
        throw new Error(
          'Cannot start a conversation: user is not authenticated.'
        )
      }
      ...
      try {
        ...
        throw new Error(
          `Failed to establish conversation. ${(error instanceof Error ? error.message : 'Unknown error') || ''}`
        )
      }
    },
    ...
    createGroup: async (
      ...
      try {
        ...
        const distributionKeys = await ensureGroupSession(
          conv.id,
          conv.participants as unknown as Participant[],
          true
        )
        if (distributionKeys) {
    ...
    addOrUpdateConversation: async (conversation) => {
      ...
      if (
        ...
        try {
          const dec = await decryptGroupMetadata(
            conversation.encryptedMetadata as string,
            conversation.id
          )
      ...
      set((state) => {
        ...
        if (existing) {
          ...
          return {
            conversations: sortConversations(
              [{ ...conversation, decryptedMetadata }, ...state.conversations],
              useAuthStore.getState().user?.id
            )
          }
        }
    ...
    updateConversation: async (id, data) => {
      ...
      set((state) => {
        ...
        if (oldConv && oldConv.isGroup && data.participants) {
          const oldIds = oldConv.participants
            .map((p) => p.id)
            .sort()
            .join(',')
          const newIds = data.participants
            .map((p) => p.id)
            .sort()
            .join(',')
          if (oldIds !== newIds) {
            forceRotateGroupSenderKey(id).catch(() => {
              console.warn('Key rotation deferred')
            })
          }
        }
      ...
      set((state) => {
        ...
        state.conversations.forEach((c) => {
          ...
          const hasCryptoChanged =
            (userDetails.publicKey !== undefined &&
              userDetails.publicKey !== existingParticipant.publicKey) ||
            (userDetails.pqPublicKey !== undefined &&
              userDetails.pqPublicKey !== existingParticipant.pqPublicKey) ||
            (userDetails.signingKey !== undefined &&
              userDetails.signingKey !== existingParticipant.signingKey) ||
            (userDetails.devices !== undefined &&
              JSON.stringify(userDetails.devices) !==
                JSON.stringify(existingParticipant.devices)) ||
            (role !== undefined && role !== existingParticipant.role)

          if (hasCryptoChanged) {
            affectedConvoIds.push(c.id)
            import('@utils/crypto').then((m) =>
              m.forceRotateGroupSenderKey(c.id).catch(console.error)
            )
          }
        })
        ...
        return {
          conversations: state.conversations.map((c) => {
            if (
              !affectedConvoIds.includes(c.id) &&
            ...
            return {
              ...c,
              requiresKeyRotation: affectedConvoIds.includes(c.id)
                ? true
                : c.requiresKeyRotation,
    ...
    addParticipants: (conversationId, newParticipants) => {
      import('@utils/crypto').then((m) =>
        m.forceRotateGroupSenderKey(conversationId).catch(console.error)
      )
      set((state) => ({
        conversations: state.conversations.map((c) => {
    ...
    updateConversationLastMessage: (conversationId, message) => {
      set((state) => {
        ...
        const currentLastMsgTime = conversation.lastMessage
    ...
    togglePinConversation: async (conversationId) => {
      ...
      try {
        set((state) => {
          ...
          return {
            conversations: sortConversations(updatedConversations, meId)
          }
        })
        ...
        const errorMessage =
          (error instanceof Error ? error.message : undefined) ||
          i18n.t(
            'errors:failed_to_toggle_pinned_conversation',
            'Failed to toggle pinned conversation.'
          )
        toast.error(errorMessage)
