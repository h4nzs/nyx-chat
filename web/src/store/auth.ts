...
export async function setupAndUploadPreKeyBundle() {
  try {
    ...
    const identityKeyB64 = sodium.to_base64(
      identityKey,
      sodium.base64_variants.URLSAFE_NO_PADDING
    )
    const pqIdentityKeyB64 = sodium.to_base64(
      pqIdentityKey,
      sodium.base64_variants.URLSAFE_NO_PADDING
    )
    ...
    const pqSignature = sodium.crypto_sign_detached(
      pqSignedPreKey,
      signingPrivateKey
    )
...
type Actions = {
  ...
  getPqEncryptionKeyPair: () => Promise<{
    publicKey: Uint8Array
    privateKey: Uint8Array
  }>
  getSigningPrivateKey: () => Promise<Uint8Array>
  getSignedPreKeyPair: () => Promise<{
    publicKey: Uint8Array
    privateKey: Uint8Array
  }>
  getPqSignedPreKeyPair: () => Promise<{
    publicKey: Uint8Array
    privateKey: Uint8Array
  }>
  ...
  (set, get) => {
    ...
    if (savedUser) {
      try {
        ...
        if (validated.success) {
          ...
          console.warn(
            '[Zustand Persist] Corrupted user data in localStorage, dropping...'
          )
          localStorage.removeItem('user')
        }
      } catch {
        console.warn(
          '[Zustand Persist] Invalid JSON in localStorage, dropping...'
        )
        localStorage.removeItem('user')
      }
    }
    ...
    const retrieveAndCacheKeys = async (): Promise<RetrievedKeys> => {
      ...
      const promptForPassword = async (
        retrieveFn: typeof retrievePrivateKeys
      ): Promise<RetrievedKeys> => {
        return new Promise((resolve, reject) => {
          ...
          useModalStore.getState().showPasswordPrompt(async (password) => {
            ...
            try {
              ...
              if (!result.success) {
                const reason =
                  result.reason === 'incorrect_password'
                    ? 'Incorrect password.'
                    : `Failed to retrieve keys: ${result.reason}`
    ...
    return {
      ...
      sendReadReceipts: savedReadReceipts
        ? JSON.parse(savedReadReceipts)
        : true,
      hasRestoredKeys: false,
      blockedUserIds: [],

      setHasRestoredKeys: async (_hasKeys) =>
        set({ hasRestoredKeys: await hasStoredKeys() }),
          ...
          try {
            ...
            if (result.success) {
              ...
              import('./auth').then(({ setupAndUploadPreKeyBundle }) => {
                setupAndUploadPreKeyBundle().catch((e) =>
                  console.warn('[Auto-Heal] Failed to upload keys:', e)
                )
              })
      ...
      bootstrap: async (force = false) => {
        ...
        try {
          const refreshRes = await api<{ ok: boolean; accessToken?: string }>(
            '/api/auth/refresh',
            { method: 'POST' }
          )
      ...
      login: async (usernameHash, password, restoredNotSynced = false) => {
        ...
        try {
          ...
          if (alreadyHasKeys && !restoredNotSynced) {
            ...
            if (!localEncryptedKeys)
              throw new Error('Local keys missing unexpectedly.')
            ...
            if (result.success && result.keys) {
              ...
              const encryptionKeyPair = sodium.crypto_box_seed_keypair(
                result.keys.encryption
              )
              const pqEncryptionKeyPair = sodium.crypto_kem_xwing_seed_keypair(
                result.keys.pqEncryption
              )
              const signingPublicKeyBytes = result.keys.signing.slice(32)

              newPublicKey = sodium.to_base64(
                encryptionKeyPair.publicKey,
                sodium.base64_variants.URLSAFE_NO_PADDING
              )
              newPqPublicKey = sodium.to_base64(
                pqEncryptionKeyPair.publicKey,
                sodium.base64_variants.URLSAFE_NO_PADDING
              )
              newSigningKey = sodium.to_base64(
              ...
              throw new Error(
                'Invalid password for local keys. Please recover your account.'
              )
            }
            ...
            const localEncryptedKeys = await getEncryptedKeys()
            if (!localEncryptedKeys)
              throw new Error('Local keys missing unexpectedly after restore.')
            ...
            if (result.success && result.keys) {
              ...
              throw new Error(
                'Failed to read restored keys. Please restore your account again.'
              )
            }
          }
            ...
            try {
              ...
              if (storedEncryptedKeys) {
                ...
                if (result.success) {
                  ...
                  throw new Error(
                    `Login successful, but failed to decrypt keys: ${result.reason}`
                  )
                }
              }
            } catch (e) {
              ...
              toast.error(
                i18n.t(
                  'errors:could_not_decrypt_your_stored_keys_pleas',
                  'Could not decrypt your stored keys. Please restore your account if the password has changed.'
                )
              )
            }
          }
          ...
          if (restoredNotSynced) {
            try {
              ...
              console.error('Failed to sync restored keys:', e)
            }
          } else if (get().hasRestoredKeys) {
            setupAndUploadPreKeyBundle().catch((e) =>
              console.error('Failed to upload pre-key bundle on login:', e)
            )
          } else {
            toast(
              'To enable secure messaging, restore your account from your recovery phrase in Settings.',
              { duration: 7000 }
            )
          }
          ...
          try {
            ...
            console.error('Reset OTPK failed:', e)
          }
          connectSocket()
        } catch (error: unknown) {
      ...
      registerAndGeneratePhrase: async ({
        encryptedProfile,
        usernameHash,
        password,
        turnstileToken
        ...
        set({ isInitializingCrypto: true })
        try {
          const { registerAndGenerateKeys, retrievePrivateKeys } =
            await import('@lib/crypto-worker-proxy')
          const {
      ...
      emergencyLogout: async () => {
        try {
          await api('/api/auth/logout-all', { method: 'POST' }).catch((e) =>
            console.error('Server kill failed:', e)
          )
      ...
      updateAvatar: async (avatar: File) => {
        const toastId = toast.loading(
          i18n.t('common:processing_avatar', 'Processing avatar...')
        )
        ...
        try {
          ...
          toast.success('Avatar uploaded! (Profile update required)', {
            id: toastId
          })
          return fileUrl
        } catch (e: unknown) {
      ...
      blockUser: async (userId) => {
        const toastId = toast.loading(
          i18n.t('common:blocking_user', 'Blocking user...')
        )
        try {
          await authFetch(`/api/users/${userId}/block`, { method: 'POST' })
          toast.success(i18n.t('common:user_blocked', 'User blocked'), {
            id: toastId
          })
          set((state) => ({
          ...
          const errorMsg = errorDetails
            ? JSON.parse(String(errorDetails)).error
      ...
      unblockUser: async (userId) => {
        const toastId = toast.loading(
          i18n.t('common:unblocking_user', 'Unblocking user...')
        )
        try {
      ...
      loadBlockedUsers: async () => {
        try {
          const blockedUsers = await authFetch<{ id: string }[]>(
