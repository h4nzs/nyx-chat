...
export async function getGroupSenderState(
  conversationId: string
): Promise<GroupSenderState | null> {
  ...
  if (record) {
    if (typeof record.state.CK === 'string') {
      ...
      ckString = sodium.to_base64(
        record.state.CK as unknown as Uint8Array,
...
export async function saveGroupSenderState(
  state: GroupSenderState
): Promise<void> {
...
export async function getGroupReceiverState(
  conversationId: string,
  senderId: string,
  senderDeviceKey?: string
): Promise<GroupReceiverState | null> {
  const id = senderDeviceKey
...
export async function saveGroupReceiverState(
  state: GroupReceiverState
): Promise<void> {
  return enqueueWrite(async () => {
 ...
 */
export async function storeGroupSkippedKey(
  conversationId: string,
  senderId: string,
  senderDeviceKey: string,
  n: number,
 ...
 */
export async function takeGroupSkippedKey(
  conversationId: string,
  senderId: string,
  senderDeviceKey: string,
  n: number
...
export async function deleteGroupSenderState(
  conversationId: string
): Promise<void> {
  return enqueueWrite(async () => {
    await db.groupSenderStates.delete(conversationId)
...
export async function storePendingHeader(
  conversationId: string,
  header: Record<string, unknown>
): Promise<void> {
  return enqueueWrite(async () => {
    await db.pendingHeaders.put({
...
export async function getPendingHeader(
  conversationId: string
...
export async function deletePendingHeader(
  conversationId: string
): Promise<void> {
  return enqueueWrite(async () => {
...
export async function storeOneTimePreKey(
  keyId: number,
...
export async function storeRatchetSession(
  conversationId: string,
...
export async function storeSkippedKey(
  headerKey: string,
...
export async function getSkippedKey(
  headerKey: string
): Promise<Uint8Array | null> {
...
export async function deleteGroupReceiverStates(
  conversationId: string
): Promise<void> {
  return enqueueWrite(async () => {
    await db.groupReceiverStates
...
export async function storeMessageKey(
  messageId: string,
  encryptedMk: Uint8Array
): Promise<void> {
  return enqueueWrite(async () => {
    await db.messageKeys.put({
...
export async function deleteConversationKeychain(
  conversationId: string
): Promise<void> {
  await Promise.all([
...
export async function exportDatabaseToJson(): Promise<string> {
  ...
  return JSON.stringify(exportData, (key, value) => {
    if (value instanceof Uint8Array) {
      return {
        __type: 'Uint8Array',
        data: sodium.to_base64(value, sodium.base64_variants.URLSAFE_NO_PADDING)
      }
    }
...
export async function importDatabaseFromJson(
  ...
  return enqueueWrite(async () => {
    ...
    try {
      ...
      const isVaultEnvelope = (
        obj: unknown
      ): obj is { encrypted: boolean; salt: string; data: string } => {
        return (
          typeof obj === 'object' &&
          obj !== null &&
          'encrypted' in obj &&
      ...
      if (isVaultEnvelope(parsedInit)) {
        ...
        const { deriveKeyFromPassword, decryptWithKey } =
