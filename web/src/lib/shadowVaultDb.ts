...
export const decryptVaultText = async (
...
class NyxShadowVaultProxy {
  ...
  async savePqDrSession(record: PqDrSessionRecord): Promise<void> {
    try {
      const encryptedState = await encryptVaultText(
  ...
  async getPqDrSession(
    conversationId: string
  ): Promise<PqDrSessionRecord | undefined> {
    try {
  ...
  async hasPqDrSession(conversationId: string): Promise<boolean> {
    try {
      const count = await db.pqDrSessions
  ...
  async importDatabase(jsonString: string): Promise<void> {
    try {
      ...
      if (data.messages && Array.isArray(data.messages)) {
        ...
        console.log(
          `[ShadowVault] Successfully imported ${data.messages.length} messages.`
        )
      }
    } catch (e) {
  ...
  async upsertMessages(messages: Message[]) {
    ...
    try {
      ...
      for (const m of validMessages) {
        ...
        const mSender = m.sender as
          | { name?: string; username?: string; avatarUrl?: string }
          | undefined
        const hasValidName =
          mSender?.name &&
          mSender.name !== 'Unknown' &&
          mSender.name !== 'Encrypted User'

        if (hasValidName) {
          encryptedSenderName =
            (await encryptVaultText(mSender.name as string)) || undefined
          if (mSender.username) {
        ...
        if (m.fileUrl || m.isBlindAttachment) {
          encryptedFileMeta = await encryptVaultText(
            JSON.stringify({
              fileUrl: m.fileUrl,
              fileKey: m.fileKey,
              fileName: m.fileName,
  ...
  private async parseRecordsToMessages(
    records: DecryptedMessageRecord[]
  ): Promise<Message[]> {
    ...
    for (const r of records) {
      ...
      if (r.senderName)
        decryptedSenderName =
          (await decryptVaultText(r.senderName)) || undefined
      if (r.senderUsername)
        decryptedSenderUsername =
          (await decryptVaultText(r.senderUsername)) || undefined
      if (r.senderAvatarUrl)
        decryptedSenderAvatarUrl =
          (await decryptVaultText(r.senderAvatarUrl)) || undefined

      if (r.fileMeta) {
        const rawMeta = await decryptVaultText(r.fileMeta)
        if (rawMeta) {
          try {
            decryptedFileMeta = JSON.parse(rawMeta)
          } catch {}
        }
      }
...
export async function saveStoryKey(
  storyId: string,
  base64Key: string
): Promise<void> {
