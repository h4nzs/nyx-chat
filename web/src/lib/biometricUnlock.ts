...
async function encryptData(
  ...
  const finalKey = sodium.crypto_generichash(
...
export async function setupBiometricUnlock(
  ...
  const attResp = await startRegistration(
    authOptions as unknown as Parameters<typeof startRegistration>[0]
  )
  const extensionResults =
    attResp.clientExtensionResults as PRFClientExtensionResults
  ...
  if (prfSupported) {
    ...
    if (!keyBuffer) {
      throw new Error(
        'Biometric registration requires a separate authentication step to complete setup. Please re-authenticate to enable vault encryption.'
      )
    }
...
export async function unlockWithBiometric(
  ...
  const asseResp = await startAuthentication(
    authOptions as unknown as Parameters<typeof startAuthentication>[0]
  )
  ...
  if (vaultStr) {
    try {
      ...
      const extensionResults =
        asseResp.clientExtensionResults as PRFClientExtensionResults
      const keyBuffer = extensionResults.prf?.results?.first

      if (keyBuffer) {
        recoveryPhrase = await decryptData(
          vault.ciphertext,
          vault.iv,
          keyBuffer
        )
