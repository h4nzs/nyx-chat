  ...
  for (const safeMessage of messagesToProcess) {
    try {
      ...
      if (meId && safeMessage.senderId === meId) {
        ...
        const isOptimisticEcho = useMessageStore
          .getState()
          .messages[
            safeMessage.conversationId
          ]?.some((m) => m.id === safeMessage.id || (m.tempId && String(m.tempId) === String(safeMessage.tempId)))
        if (isOptimisticEcho) {
          continue
        }
      ...
      const decryptedMessage = await addIncomingMessage(
        safeMessage.conversationId,
        safeMessage
      )
  ...
  while (attempt < MAX_RETRIES) {
    try {
      ...
      useConversationStore
        .getState()
        .updateConversation(conversationId, { keyRotationPending: false })
      return
    } catch (err: unknown) {
      attempt++
      console.error(
        `[socket] Key rotation attempt ${attempt} failed for ${conversationId}:`,
        err
      )
      if (attempt >= MAX_RETRIES) {
        console.error(
          `[socket] All key rotation retries failed for ${conversationId}. Marking as pending.`
        )
        useConversationStore
          .getState()
          .updateConversation(conversationId, { keyRotationPending: true })
        toast.error(
          i18n.t(
            'errors:group_key_rotation_critical',
            'CRITICAL: Failed to rotate keys for group. The chat is insecure.'
          ),
          { duration: 10000 }
        )
      } else {
...
export function getSocket() {
  if (!socket) {
    ...
    const { addOrUpdate, setOnlineUsers, userJoined, userLeft } =
      usePresenceStore.getState()
    const { updateMessage, removeLocalReaction, updateMessageStatus } =
      useMessageStore.getState()
    ...
    socket.on('disconnect', (reason) => {
      ...
      if (reason !== 'io client disconnect')
        toast.error(
          i18n.t(
            'errors:disconnected_reconnecting',
      ...
      if (!parsed.success) {
        const payloadSummary =
          typeof rawPayload === 'object' && rawPayload !== null
            ...
            : typeof rawPayload
        console.error(
          '[Zod Shield] Dropping invalid incoming message:',
    ...
    socket.on(
      'burner:receive',
      async (payload: { roomId?: string; ciphertext: string }) => {
        const { useBurnerStore } = await import('@store/burner')
        const roomId =
          payload.roomId ||
          Object.keys(useBurnerStore.getState().activeSessions)[0]
        if (roomId) {
          await useBurnerStore
            .getState()
            .receiveMessage(roomId, payload.ciphertext)
        }
      }
    )
    ...
    socket.on('burner:terminated', async (payload: { roomId: string }) => {
      ...
      if (payload?.roomId) {
        useBurnerStore
          .getState()
          .terminateSession(
            'This secure session has been terminated by the host.'
          )
    ...
    socket.on('message:viewed', ({ messageId, conversationId }) => {
      useMessageStore
        .getState()
        .updateMessage(conversationId, messageId, { isViewed: true })
    })
    ...
    socket.on('typing:update', ({ userId, conversationId, isTyping }) =>
      addOrUpdate({ id: userId, conversationId, isTyping })
    )
    socket.on('reaction:new', ({ conversationId, messageId, reaction }) => {
      const { user: me } = useAuthStore.getState()
      const { replaceOptimisticReaction, addLocalReaction } =
        useMessageStore.getState()
    ...
    socket.on('reaction:deleted', ({ conversationId, messageId, reactionId }) =>
    ...
    socket.on('conversation:new', (newConversation) => {
      ...
      if (newConversation.isGroup) {
        useConversationStore
          .getState()
          .markKeyRotationNeeded(newConversation.id, true)
      ...
      toast.success(
        i18n.t(
          'common:added_to_group',
          `You've been added to "${newConversation.title || 'a new chat'}"`,
          { groupName: newConversation.title || 'a new chat' }
        )
      )
    })

    socket.on('conversation:updated', (updates) =>
      conversationStore.updateConversation(updates.id, updates)
    )
    ...
    socket.on(
      ...
      (data: { conversationId: string }) => {
        useConversationStore
          .getState()
          .markKeyRotationNeeded(data.conversationId, true)
        import('@utils/crypto').then((m) =>
          m.forceRotateGroupSenderKey(data.conversationId).catch(console.error)
        )
    ...
    socket.on(
      'session:request_key',
      async (data: { conversationId: string; requesterId: string }) => {
        useConversationStore
          .getState()
          .markKeyRotationNeeded(data.conversationId, true)
      }
    ...
    socket.on(
      ...
      ({ conversationId, newParticipants }) => {
        useConversationStore.getState().addParticipants(
          conversationId,
          newParticipants.map((p) => ({ ...p, id: asUserId(p.id) }))
        )
        useConversationStore
          .getState()
          .markKeyRotationNeeded(conversationId, true)
    ...
    socket.on('session:fulfill_request', (data) =>
      fulfillKeyRequest(data).catch(console.error)
    )
    socket.on('group:fulfill_key_request', (data) =>
      fulfillGroupKeyRequest(data).catch(console.error)
    )
    socket.on('session:new_key', (data) => {
      storeReceivedSessionKey(data)
        .then(() => {
          ...
          useMessageStore
            .getState()
            .reDecryptPendingMessages(data.conversationId)
        })
        .catch(console.error)
    })
    socket.on('force_logout', (payload) => {
      ...
      toast.error(
        i18n.t(
          'errors:this_session_has_been_logged_out_remotel',
          'This session has been logged out remotely.'
        )
      )
    ...
    socket.on(
      ...
      async ({ conversationId, messageId }) => {
        ...
        if (isViewingChat) {
          toast.success(
            i18n.t(
              'chat:messages.message_retracted',
              'A message has been retracted by the sender.'
            ),
            {
              icon: '🗑️',
              duration: 3000,
...
export function emitSessionKeyRequest(
  conversationId: string,
  sessionId: string,
  targetId?: string
) {
...
export function emitGroupKeyDistribution(
  conversationId: string,
  keys: {
    userId: string
    key: string
  ...
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      reject(
        new Error(
          'Request timeout: Key distribution server acknowledgment took too long'
        )
      )
    }, 10000)

    ;(socket as unknown as import('socket.io-client').Socket).emit(
...
export function emitGroupKeyRequest(
  conversationId: string,
...
export function emitGroupKeyFulfillment(payload: {
  requesterId: string
  conversationId: string
  encryptedKey: string
  targetDeviceId?: string
