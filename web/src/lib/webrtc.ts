...
export const getNetworkQuality = async (): Promise<
  'Good' | 'Fair' | 'Poor'
> => {
...
const sendSecureSignal = async (
  ...
  try {
    ...
    getSocket()?.emit('webrtc:secure_signal', {
      to,
      type,
...
const createPeerConnection = (
  ...
  pc.onicecandidate = (event) => {
    if (event.candidate) {
      sendSecureSignal(targetUserId, 'ice-candidate', {
  ...
  pc.ontrack = (event) => {
    if (event.streams && event.streams[0]) {
      useCallStore
        .getState()
        .addRemoteStream(asUserId(targetUserId), event.streams[0])
    }
  }
...
const getMediaStream = async (video: boolean) => {
  try {
    ...
    import('react-hot-toast').then((m) =>
      m.default.error(
        `Media Error: ${(error instanceof Error ? error.message : 'Unknown error') || 'Permission denied'}`
      )
    )
    throw error
  }
...
export const startCall = async (
  to: string,
  isVideo: boolean,
  callerProfile: MinimalProfile
) => {
  try {
    const { generateCallKey, encryptCallSignal } =
      await import('../utils/crypto')
    const callKey = await generateCallKey()

    useCallStore
      .getState()
      .setOutgoingCall(asUserId(to), isVideo, callerProfile, callKey)
    ...
    import('react-hot-toast').then((m) =>
      m.default.error(
        i18n.t('errors:call_failed', 'Failed to start E2EE call.')
      )
    )
    cleanupCall()
  }
...
export const rejectCall = () => {
  ...
  state.remoteUsers.forEach((u) =>
    sendSecureSignal(u.id, 'reject', { reason: 'declined' })
  )
  cleanupCall()
}
...
export const initWebRTCListeners = (socket: Socket | null) => {
  ...
  socket.on('webrtc:secure_signal', async (rawPayload: unknown) => {
    ...
    if (!parsed.success) {
      console.error(
        '[WebRTC Zod Shield] Dropping invalid signaling payload:',
        parsed.error.format()
      )
      return
    }
    ...
    if (!callKey) {
      ...
      if (!callKey) {
        console.warn(
          `[WebRTC] SECURITY BLOCK: Dropping signal ${data.type} because callKey is missing after timeout.`
        )
        return
      }
    }
    ...
    try {
      ...
      if (typeof data.payload === 'string') {
        decryptedPayload = (await decryptCallSignal(
          data.payload,
          callKey
        )) as SignalingPayload
      } else if (
        ...
        console.warn(
          `[WebRTC] DEV MODE: Menerima payload tidak terenkripsi untuk tipe ${data.type}`
        )
        decryptedPayload = data.payload as SignalingPayload
      } else {
        // 🚨 PRODUCTION SHIELD: Tolak mentah-mentah jika bukan string terenkripsi!
        console.warn(
          `[WebRTC] SECURITY BLOCK: Dropping signal ${data.type} with unencrypted payload.`
        )
      ...
      if (data.type === 'request') {
        if (
          typeof (decryptedPayload as { isVideo?: unknown }).isVideo !==
          'boolean'
        ) {
          console.warn(
            '[WebRTC] Invalid request payload: isVideo missing or not boolean'
          )
          return
        }
      } else if (data.type === 'offer') {
        const payloadWithOffer = decryptedPayload as {
        ...
        const payloadWithAnswer = decryptedPayload as {
        ...
        const candidate = (decryptedPayload as { candidate?: unknown })
          .candidate
      ...
      switch (data.type) {
        case 'request':
          if (state.callState === 'idle') {
            useCallStore
              ...
              .setIncomingCall(
                data.from,
                (decryptedPayload as { isVideo: boolean }).isVideo,
                (decryptedPayload as { callerProfile: MinimalProfile })
                  .callerProfile,
                callKey
              )
          } else {
            if (state.ephemeralCallKey === callKey) {
              useCallStore
                ...
                .addRemoteUser(
                  (decryptedPayload as { callerProfile?: MinimalProfile })
                    .callerProfile || { id: data.from }
                )
            } else {
              sendSecureSignal(data.from as string, 'reject', {
                reason: 'busy'
              })
            }
          }
          ...
          try {
            await pc.setRemoteDescription(
              new RTCSessionDescription(
                (decryptedPayload as { offer: RTCSessionDescriptionInit }).offer
              )
            )
        ...
        case 'answer':
          ...
          try {
            await pc.setRemoteDescription(
              new RTCSessionDescription(
                (decryptedPayload as { answer: RTCSessionDescriptionInit })
                  .answer
              )
            )
          } catch (e) {
        ...
        case 'ice-candidate':
          ...
          try {
            await pc.addIceCandidate(
              new RTCIceCandidate(
                (decryptedPayload as { candidate: RTCIceCandidateInit })
                  .candidate
              )
            )
          } catch (e) {
      ...
      console.error(
        `Failed to decrypt and process secure signal ${data.type}`,
        e
      )
      if (e instanceof DOMException || (e as Error).name === 'OperationError') {
        console.warn(
          `[WebRTC] Invalid call key detected for peer ${data.from}. Removing peer from call.`
        )
        ...
        if (useCallStore.getState().remoteUsers.length === 0) {
          console.warn(
            `[WebRTC] No peers left. Purging current call state to allow recovery.`
          )
          cleanupCall()
        }
      }
