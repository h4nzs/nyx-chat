...
export const deriveKeyFromPassword = async (
  ...
  const result = await sendToWorker<Uint8Array>('DERIVE_KEY', {
    password,
    salt: toArray(salt)
  })
...
export const encryptWithKey = async (
  ...
  return sendToWorker<string>('ENCRYPT_DATA', {
...
export const decryptWithKey = async (
  ...
  return sendToWorker<unknown>('DECRYPT_DATA', {
    keyBytes: toArray(keyBytes),
    encryptedString
  })
}
...
export async function recoverAccountWithSignature(
  ...
  return sendToWorker('recoverAccountWithSignature', {
    phrase,
    newPassword,
    identifier,
    timestamp,
...
export async function reEncryptBundleFromMasterKey(
  ...
  return sendToWorker('reEncryptBundleFromMasterKey', {
    masterKey: toArray(masterKey),
    newPassword
  })
}

export async function retrievePrivateKeys(
  encryptedDataStr: string,
  password: string
): Promise<RetrieveKeysResult> {
  const result = await sendToWorker<RetrieveKeysResult>('retrievePrivateKeys', {
    encryptedDataStr,
    password
  })
  if (result.success) {
    return {
      ...
      keys: {
        ...
        pqEncryption: result.keys.pqEncryption
          ? new Uint8Array(result.keys.pqEncryption)
          : undefined,
        signing: new Uint8Array(result.keys.signing),
        signedPreKey: new Uint8Array(result.keys.signedPreKey),
        pqSignedPreKey: result.keys.pqSignedPreKey
          ? new Uint8Array(result.keys.pqSignedPreKey)
          : undefined,
        masterSeed: result.keys.masterSeed
          ? new Uint8Array(result.keys.masterSeed)
          : undefined
      }
    }
  }
...
export async function generateSafetyNumber(
  ...
  return sendToWorker('generateSafetyNumber', {
    myPublicKey: toArray(myPublicKey),
    theirPublicKey: toArray(theirPublicKey)
  })
}
...
export function worker_crypto_secretbox_xchacha20poly1305_easy(
  message: string | CryptoBuffer,
  nonce: CryptoBuffer,
  key: CryptoBuffer
): Promise<Uint8Array> {
  return sendToWorker('crypto_secretbox_xchacha20poly1305_easy', {
...
export function worker_crypto_secretbox_xchacha20poly1305_open_easy(
  ciphertext: CryptoBuffer,
  nonce: CryptoBuffer,
  key: CryptoBuffer
): Promise<Uint8Array> {
  return sendToWorker('crypto_secretbox_xchacha20poly1305_open_easy', {
...
export function worker_crypto_box_seal(
  message: CryptoBuffer,
  publicKey: CryptoBuffer
): Promise<Uint8Array> {
  return sendToWorker('crypto_box_seal', {
    message: toArray(message),
    publicKey: toArray(publicKey)
  })
}

export function worker_crypto_box_seal_open(
  ciphertext: CryptoBuffer,
  publicKey: CryptoBuffer,
  privateKey: CryptoBuffer
): Promise<Uint8Array> {
  return sendToWorker('crypto_box_seal_open', {
    ciphertext: toArray(ciphertext),
    publicKey: toArray(publicKey),
...
export function worker_pq_box_seal(
...
export function worker_pq_box_seal_open(
...
export function worker_x3dh_initiator(payload: {
  ...
  return sendToWorker('x3dh_initiator', {
    ...
    theirPqIdentityKey: payload.theirPqIdentityKey
      ? toArray(payload.theirPqIdentityKey)
      : undefined,
    theirSignedPreKey: toArray(payload.theirSignedPreKey),
    theirPqSignedPreKey: payload.theirPqSignedPreKey
      ? toArray(payload.theirPqSignedPreKey)
      : undefined,
    ...
    theirOneTimePreKey: payload.theirOneTimePreKey
      ? toArray(payload.theirOneTimePreKey)
      : undefined,
    theirPqOneTimePreKey: payload.theirPqOneTimePreKey
      ? toArray(payload.theirPqOneTimePreKey)
      : undefined
  })
}
...
export function worker_x3dh_recipient(payload: {
  ...
  return sendToWorker('x3dh_recipient', {
    ...
    myPqIdentityKey: {
      privateKey: toArray(payload.myPqIdentityKey.privateKey)
    },
    myPqSignedPreKey: {
    ...
    myOneTimePreKey: payload.myOneTimePreKey
      ? { privateKey: toArray(payload.myOneTimePreKey.privateKey) }
      : undefined
  })
}
...
export function worker_file_encrypt(
  fileBuffer: ArrayBuffer
): Promise<{ combinedData: ArrayBuffer; key: Uint8Array }> {
  return sendToWorker('file_encrypt', { fileBuffer })
}

export function worker_file_decrypt(
  combinedData: ArrayBuffer,
  keyBytes: Uint8Array
): Promise<ArrayBuffer> {
  return sendToWorker('file_decrypt', {
    combinedData,
    keyBytes: toArray(keyBytes)
  })
}

export function worker_encrypt_session_key(
...
export function worker_decrypt_session_key(
...
export function worker_generate_otpk_batch(
  count: number,
  startId: number,
  masterSeed: CryptoBuffer
): Promise<
...
> {
  return sendToWorker('generate_otpk_batch', {
    count,
    startId,
    masterSeed: toArray(masterSeed)
  })
...
export function worker_dr_ratchet_decrypt(payload: {
  ...
  skippedKeys: { kemPk: string; n: number; mk: string }[]
  mk: Uint8Array
}> {
  return sendToWorker<{
    state: DoubleRatchetState
...
import type {
  BurnerDoubleRatchetState,
  BurnerDoubleRatchetHeader
...
export function worker_burner_dr_init_host(payload: {
  ...
  return sendToWorker<{ state: BurnerDoubleRatchetState }>(
    'burner_dr_init_host',
    {
