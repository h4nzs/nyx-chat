    ...
    (async () => {
      try {
        ...
        if (
          ...
          data.data?.encryptedPushPayload
        ) {
          try {
            ...
            const encryptedKeys = (await getKvValue('nyx_encrypted_keys')) as
              | string
              | undefined
            const autoUnlockKey = (await getKvValue(
              'nyx_device_auto_unlock_key'
            )) as string | undefined
            ...
            if (encryptedKeys && autoUnlockKey) {
              ...
              if (parts.length === 2) {
                ...
                const sealedPayloadBytes = sodium.from_base64(
                  data.data.encryptedPushPayload,
                  sodium.base64_variants.URLSAFE_NO_PADDING
                )
                const decryptedPushBytes = sodium.crypto_box_seal_open(
                  sealedPayloadBytes,
                  publicKey,
                  privateKey
                )
                ...
                const decryptedPayload = JSON.parse(
                  sodium.to_string(decryptedPushBytes)
                )

                title = decryptedPayload.title || title
                body = decryptedPayload.body || body
                conversationId =
                  decryptedPayload.conversationId || conversationId
            ...
            console.error(
              '[SW] Push decryption failed, falling back to generic payload:',
              cryptoError
            )
          }
        }
        ...
        for (const client of clientList) {
          ...
          if (client.visibilityState === 'visible') {
            ...
            if (
              conversationId &&
              clientUrl.pathname.includes(`/chat/${conversationId}`)
            ) {
        ...
        if (isFocusedOnChat) {
          console.log(
            '[SW] User is active in this chat. Suppressing OS notification.'
          )
