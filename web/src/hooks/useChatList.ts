...
export function useChatList() {
  ...
  const { blockUser, unblockUser, blockedUserIds } = useAuthStore(
    useShallow((state) => ({
  ...
  const handleSearch = useCallback(
    debounce(async (query: string) => {
      ...
      try {
        ...
        const users = await authFetch<User[]>(
          `/api/users/search?q=${encodeURIComponent(hashedQuery)}`
        )
  ...
  const handleSelectUser = async (user: User) => {
    const conversationId = await startConversation(user.id, {
      name: user.name || t('common:defaults.unknown', 'Unknown'),
  ...
  const filteredConversations = conversations.filter((c) => {
    const title = c.isGroup
      ? c.decryptedMetadata?.title ||
        i18n.t('common:defaults.unknown_group', 'Unknown Group')
      : c.participants.find((p) => p.id !== meId)?.name ||
        i18n.t('common:defaults.unknown_user', 'Unknown User')
