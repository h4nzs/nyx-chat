...
export function usePushNotifications() {
  ...
  const sendSubscriptionToSocket = useCallback(
  ...
  const subscribeToPush = useCallback(async () => {
    if (!('serviceWorker' in navigator) || !('PushManager' in window)) {
      toast.error(
        i18n.t(
          'errors:push_notifications_not_supported',
    ...
    try {
      ...
      if (perm !== 'granted') {
        toast.error(
          i18n.t(
            'errors:notification_permission_denied',
      ...
      if (mountedRef.current) {
        ...
        localStorage.setItem('nyx_push_enabled', 'true')
        toast.success(
          i18n.t('common:notifications_enabled', 'Notifications enabled!')
        )
      }
    } catch (error: unknown) {
      ...
      toast.error(
        i18n.t(
          'errors:failed_to_enable_notifications',
  ...
  const unsubscribeFromPush = useCallback(async () => {
    ...
    try {
      ...
      if (mountedRef.current) {
        ...
        toast.success(
          i18n.t('common:notifications_disabled', 'Notifications disabled')
        )
      }
    } catch (error) {
      ...
      toast.error(
        i18n.t(
          'errors:failed_to_disable_notifications',
