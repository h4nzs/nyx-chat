...
function _sanitizeError(error: unknown): string {
  ...
  return msg
    .replace(/\{.*?\}/g, '[REDACTED_OBJ]')
    .replace(/\b[a-zA-Z0-9+/=_-]{32,}\b/g, '[REDACTED_STR]')
    .substring(0, 150)
}
...
async function _decryptData(
  ...
  try {
    const parsed = JSON.parse(encryptedString) as {
      iv: number[]
      data: number[]
    }
...
async function storePrivateKeys(
  keys: {
  ...
  const privateKeysObj = {
    encryption: sodium.to_base64(
      keys.encryption,
      sodium.base64_variants[B64_VARIANT]
    ),
    pqEncryption: keys.pqEncryption
      ? sodium.to_base64(keys.pqEncryption, sodium.base64_variants[B64_VARIANT])
      : undefined,
    signing: sodium.to_base64(
      keys.signing,
      sodium.base64_variants[B64_VARIANT]
    ),
    ...
    pqSignedPreKey: keys.pqSignedPreKey
      ? sodium.to_base64(
          keys.pqSignedPreKey,
          sodium.base64_variants[B64_VARIANT]
        )
    ...
    masterSeed: keys.masterSeed
      ? sodium.to_base64(keys.masterSeed, sodium.base64_variants[B64_VARIANT])
      : undefined
  }
  ...
  try {
    ...
    return (
      sodium.to_base64(salt, sodium.base64_variants[B64_VARIANT]) +
      '.' +
      encryptedData
    )
  } finally {
...
async function retrievePrivateKeys(
  encryptedDataWithSaltStr: string,
  password: string
): Promise<RetrieveKeysResult> {
  let kek: Uint8Array | null = null
  try {
    if (!encryptedDataWithSaltStr)
    ...
    const privateKeysObj =
      typeof privateKeysRaw === 'string'
        ? JSON.parse(privateKeysRaw)
        : privateKeysRaw

    const keys = privateKeysObj as {
      encryption: string
      pqEncryption?: string
      signing: string
      signedPreKey: string
    ...
    if (!keys.signedPreKey || !keys.pqEncryption || !keys.pqSignedPreKey)
      return { success: false, reason: 'legacy_bundle' }

    return {
      success: true,
      keys: {
        encryption: sodium.from_base64(
          keys.encryption,
          sodium.base64_variants[B64_VARIANT]
        ),
        pqEncryption: sodium.from_base64(
          keys.pqEncryption,
          sodium.base64_variants[B64_VARIANT]
        ),
        signing: sodium.from_base64(
          keys.signing,
          sodium.base64_variants[B64_VARIANT]
        ),
        signedPreKey: sodium.from_base64(
          keys.signedPreKey,
          sodium.base64_variants[B64_VARIANT]
        ),
        pqSignedPreKey: sodium.from_base64(
          keys.pqSignedPreKey,
          sodium.base64_variants[B64_VARIANT]
        ),
        masterSeed: keys.masterSeed
...
function generateSafetyNumber(
  ...
  const digitGroups = chunks.map((chunk: string) =>
    parseInt(chunk, 16).toString().padStart(5, '0').slice(-5)
  )
...
function deserializeState(
  serialized: DoubleRatchetState
): RuntimeDoubleRatchetState {
  return {
    KEMs: serialized.KEMs
      ? {
          publicKey: sodium.from_base64(
            serialized.KEMs.publicKey,
            sodium.base64_variants.URLSAFE_NO_PADDING
          ),
          privateKey: sodium.from_base64(
            serialized.KEMs.privateKey,
            sodium.base64_variants.URLSAFE_NO_PADDING
          )
        }
      : null,
...
function serializeState(
  runtime: RuntimeDoubleRatchetState
): DoubleRatchetState {
  return {
    KEMs: runtime.KEMs
      ? {
          publicKey: sodium.to_base64(
            runtime.KEMs.publicKey,
            sodium.base64_variants.URLSAFE_NO_PADDING
          ),
          privateKey: sodium.to_base64(
            runtime.KEMs.privateKey,
            sodium.base64_variants.URLSAFE_NO_PADDING
          )
        }
      : null,
...
export type WorkerMessage =
  | {
      type: 'DERIVE_KEY'
      payload: { password: string; salt: CryptoBuffer }
      id: string
    }
  | {
      type: 'ENCRYPT_DATA'
      payload: { keyBytes: CryptoBuffer; data: unknown }
      id: string
    }
  | {
      type: 'DECRYPT_DATA'
      payload: { keyBytes: CryptoBuffer; encryptedString: string }
      id: string
    }
  | {
      type: 'registerAndGenerateKeys'
      payload: { password: string }
      id: string
    }
  | {
      type: 'retrievePrivateKeys'
      payload: { encryptedDataStr: string; password: string }
      id: string
    }
  | {
      type: 'generateSafetyNumber'
      payload: { myPublicKey: CryptoBuffer; theirPublicKey: CryptoBuffer }
      id: string
    }
  | {
      type: 'crypto_secretbox_xchacha20poly1305_easy'
      payload: {
        message: string | CryptoBuffer
        nonce: CryptoBuffer
        key: CryptoBuffer
      }
      id: string
  ...
  | {
      type: 'crypto_secretbox_xchacha20poly1305_open_easy'
      payload: {
  ...
  | {
      ...
      payload: {
        ...
        pqPublicKey: CryptoBuffer
        classicalPublicKey: CryptoBuffer
      }
      id: string
    }
  | {
      type: 'pq_box_seal_open'
      payload: {
        combinedPayload: CryptoBuffer
        pqPrivateKey: CryptoBuffer
        classicalPrivateKey: CryptoBuffer
      }
      id: string
    }
  | {
  ...
  | {
      type: 'crypto_box_seal'
      payload: { message: CryptoBuffer; publicKey: CryptoBuffer }
      id: string
    }
  | {
      type: 'file_encrypt'
      payload: { fileBuffer: ArrayBuffer | Uint8Array }
      id: string
    }
  | {
      type: 'file_decrypt'
      payload: {
        combinedData: ArrayBuffer | Uint8Array
        keyBytes: CryptoBuffer
      }
      id: string
    }
  | {
      type: 'getRecoveryPhrase'
      payload: { encryptedDataStr: string; password: string }
      id: string
    }
  | {
      type: 'restoreFromPhrase'
      payload: { phrase: string; password: string }
      id: string
    }
  | {
      type: 'recoverAccountWithSignature'
      payload: {
        phrase: string
        newPassword: string
        identifier: string
  ...
  | {
      type: 'encryptProfile'
      payload: { profileJsonString: string; profileKeyB64: string }
      id: string
    }
  | {
      type: 'decryptProfile'
      payload: { encryptedProfileB64: string; profileKeyB64: string }
      id: string
    }
  | { type: 'generateProfileKey'; payload: void; id: string }
  | {
      type: 'minePoW'
      payload: { salt: string; difficulty: number }
      id: string
    }
  | { type: 'generate_random_key'; payload: void; id: string }
  | {
      type: 'reEncryptBundleFromMasterKey'
      payload: { masterKey: CryptoBuffer; newPassword: string }
      id: string
    }
  | {
      type: 'encrypt_session_key'
      payload: { sessionKey: CryptoBuffer; masterSeed: CryptoBuffer }
      id: string
    }
  | {
      type: 'decrypt_session_key'
      payload: { encryptedKey: CryptoBuffer; masterSeed: CryptoBuffer }
      id: string
    }
  | {
      type: 'generate_otpk_batch'
      payload: { count: number; startId: number; masterSeed: CryptoBuffer }
      id: string
    }
  | {
      type: 'x3dh_recipient_regenerate'
      payload: {
        keyId: number
  ...
  | {
      type: 'dr_init_alice'
      payload: { sk: CryptoBuffer; theirSignedPreKeyPublic: CryptoBuffer }
      id: string
    }
  | {
      type: 'dr_init_bob'
      payload: { sk: CryptoBuffer; mySignedPreKey: SodiumKeyPair }
      id: string
    }
  | {
      type: 'dr_ratchet_encrypt'
      payload: {
  ...
  | {
      type: 'burner_dr_init_guest'
      payload: { hostClassicalPk: CryptoBuffer; hostPqPk: CryptoBuffer }
      id: string
    }
  | {
      type: 'burner_dr_init_host'
      payload: {
    ...
    switch (type) {
      ...
      case 'registerAndGenerateKeys': {
        ...
        const encryptionSeed = sodium.crypto_generichash(
          32,
          stretchedSeed,
          new Uint8Array(new TextEncoder().encode('encryption'))
        )
        const pqEncryptionSeed = sodium.crypto_generichash(
          32,
          stretchedSeed,
          new Uint8Array(new TextEncoder().encode('pq-encryption'))
        )
        const signingSeed = sodium.crypto_generichash(
          32,
          stretchedSeed,
          new Uint8Array(new TextEncoder().encode('signing'))
        )
        const signedPreKeySeed = sodium.crypto_generichash(
          32,
          stretchedSeed,
          new Uint8Array(new TextEncoder().encode('signed-pre-key'))
        )
        const pqSignedPreKeySeed = sodium.crypto_generichash(
          32,
          stretchedSeed,
          new Uint8Array(new TextEncoder().encode('pq-signed-pre-key'))
        )

        const encryptionKeyPair = sodium.crypto_box_seed_keypair(encryptionSeed)
        const pqEncryptionKeyPair =
          sodium.crypto_kem_xwing_seed_keypair(pqEncryptionSeed)
        const signingKeyPair = sodium.crypto_sign_seed_keypair(signingSeed)
        const signedPreKeyPair =
          sodium.crypto_box_seed_keypair(signedPreKeySeed)
        const pqSignedPreKeyPair =
          sodium.crypto_kem_xwing_seed_keypair(pqSignedPreKeySeed)
        ...
        try {
          ...
          result = {
            encryptionPublicKeyB64: exportPublicKey(
              encryptionKeyPair.publicKey
            ),
            pqEncryptionPublicKeyB64: exportPublicKey(
              pqEncryptionKeyPair.publicKey
            ),
      ...
      case 'crypto_secretbox_xchacha20poly1305_easy': {
        ...
        try {
          ...
          if (messageBytes && typeof message !== 'string')
      ...
      case 'pq_box_seal': {
        ...
        if (classicalPublicKeyBytes.length !== 32) {
          throw new Error(
            `invalid publicKey length: expected 32 bytes for classical key, got ${classicalPublicKeyBytes.length}`
          )
        }
        if (
          pqPublicKeyBytes.length !== sodium.crypto_kem_xwing_PUBLICKEYBYTES
        ) {
          throw new Error(
            `invalid PQ publicKey length: expected ${sodium.crypto_kem_xwing_PUBLICKEYBYTES} bytes, got ${pqPublicKeyBytes.length}`
          )
        }
        ...
        try {
          ...
          sharedSecret = new Uint8Array(
            classicalSharedSecret.length + pqSharedSecret.length
          )
          ...
          encryptedMessage = sodium.crypto_aead_xchacha20poly1305_ietf_encrypt(
            messageBytes,
            null,
            null,
            nonce,
      ...
      case 'pq_box_seal_open': {
        ...
        const payloadBytes =
          typeof combinedPayload === 'string'
            ? sodium.from_base64(
                combinedPayload,
                sodium.base64_variants.URLSAFE_NO_PADDING
              )
            : new Uint8Array(combinedPayload)
        const pqPrivateKeyBytes =
          typeof pqPrivateKey === 'string'
            ? sodium.from_base64(
                pqPrivateKey,
                sodium.base64_variants.URLSAFE_NO_PADDING
              )
            : new Uint8Array(pqPrivateKey)
        const classicalPrivateKeyBytes =
          typeof classicalPrivateKey === 'string'
            ? sodium.from_base64(
                classicalPrivateKey,
                sodium.base64_variants.URLSAFE_NO_PADDING
              )
            : new Uint8Array(classicalPrivateKey)
        ...
        if (
          payloadBytes.length <
          ephemeralPubKeyLength + pqCiphertextLength + nonceLength
        ) {
          throw new Error('Payload too short for pq_box_seal_open')
        }

        let offset = 0
        const ephemeralPubKey = payloadBytes.slice(
          offset,
          offset + ephemeralPubKeyLength
        )
        offset += ephemeralPubKeyLength

        const pqCiphertext = payloadBytes.slice(
          offset,
        ...
        try {
          ...
          const pqSharedSecret = sodium.crypto_kem_xwing_dec(
            pqCiphertext,
            pqPrivateKeyBytes
          )
      ...
      case 'x3dh_initiator': {
        ...
        if (
          !sodium.crypto_sign_verify_detached(
            pqSignatureBytes,
            theirPqSignedPreKeyBytes,
            theirSigningKeyBytes
          )
        ) {
          throw new Error(
            'Invalid Post-Quantum signature on PQ signed pre-key.'
          )
        }

        const mySigningKeyPrivateBytes = new Uint8Array(mySigningKey.privateKey)
        const myIdentityKeyPrivateBytes =
          sodium.crypto_sign_ed25519_sk_to_curve25519(mySigningKeyPrivateBytes)
        ...
        try {
          ...
          const pqIdResult = sodium.crypto_kem_xwing_enc(
            new Uint8Array(theirPqIdentityKey)
          )
          secrets.push(pqIdResult.sharedSecret)
          ct_id = pqIdResult.ciphertext

          const pqSpkResult = sodium.crypto_kem_xwing_enc(
            theirPqSignedPreKeyBytes
          )
          ...
          if (theirOneTimePreKey || theirPqOneTimePreKey) {
            ...
            const dh4 = sodium.crypto_scalarmult(
              ephemeralKeyPair.privateKey,
              theirOneTimePreKeyBytes
            )
            secrets.push(dh4)

            const pqOtpkResult = sodium.crypto_kem_xwing_enc(
              new Uint8Array(theirPqOneTimePreKey)
            )
          ...
          if (ct_id)
            ciphertextsObj.ct_id = sodium.to_base64(
              ct_id,
              sodium.base64_variants.URLSAFE_NO_PADDING
            )
          if (ct_spk)
            ciphertextsObj.ct_spk = sodium.to_base64(
              ct_spk,
              sodium.base64_variants.URLSAFE_NO_PADDING
            )
          if (ct_otpk)
            ciphertextsObj.ct_otpk = sodium.to_base64(
              ct_otpk,
              sodium.base64_variants.URLSAFE_NO_PADDING
            )
      ...
      case 'x3dh_recipient': {
        ...
        const myIdentityKeyPrivateBytes = new Uint8Array(
          myIdentityKey.privateKey
        )
        const mySignedPreKeyPrivateBytes = new Uint8Array(
          mySignedPreKey.privateKey
        )
        const myPqIdentityKeyPrivateBytes = new Uint8Array(
          myPqIdentityKey.privateKey
        )
        const myPqSignedPreKeyPrivateBytes = new Uint8Array(
          myPqSignedPreKey.privateKey
        )

        const theirSigningKeyBytes = new Uint8Array(theirSigningKey)
        const theirIdentityKeyBytes =
          sodium.crypto_sign_ed25519_pk_to_curve25519(theirSigningKeyBytes)

        const ciphertextsStr = new TextDecoder().decode(
          sodium.from_base64(
            initiatorCiphertexts as string,
            sodium.base64_variants.URLSAFE_NO_PADDING
          )
        )
        const ciphertexts = JSON.parse(ciphertextsStr)

        const theirEphemeralKeyBytes = sodium.from_base64(
          ciphertexts.ek,
          sodium.base64_variants.URLSAFE_NO_PADDING
        )
        ...
        const ct_id = sodium.from_base64(
          ciphertexts.ct_id,
          sodium.base64_variants.URLSAFE_NO_PADDING
        )
        const ct_spk = sodium.from_base64(
          ciphertexts.ct_spk,
          sodium.base64_variants.URLSAFE_NO_PADDING
        )
        ...
        try {
          ...
          const ss_id = sodium.crypto_kem_xwing_dec(
            ct_id,
            myPqIdentityKeyPrivateBytes
          )
          const ss_spk = sodium.crypto_kem_xwing_dec(
            ct_spk,
            myPqSignedPreKeyPrivateBytes
          )
          ...
          if (ciphertexts.ct_otpk) {
            if (!myOneTimePreKey) {
              throw new Error(
                'Sender used One-Time Pre-Key but local device is missing the key.'
              )
            }
            const privateKeysJson = new TextDecoder().decode(
              new Uint8Array(myOneTimePreKey.privateKey)
            )
            ...
            const ct_otpk = sodium.from_base64(
              ciphertexts.ct_otpk,
              sodium.base64_variants.URLSAFE_NO_PADDING
            )
      ...
      case 'file_decrypt': {
        ...
        if (dataBytes.length <= headerSize) {
          throw new Error(
            'File decryption failed: Invalid or missing ciphertext'
          )
        }
        ...
        const chunkSize =
          1024 * 1024 * 2 + sodium.crypto_secretstream_xchacha20poly1305_ABYTES
        ...
        while (offset < dataBytes.length) {
          const chunk = dataBytes.slice(
            offset,
            Math.min(offset + chunkSize, dataBytes.length)
          )
          const pullRes = sodium.crypto_secretstream_xchacha20poly1305_pull(
            state,
            chunk
          )
          if (!pullRes) {
        ...
        if (
          lastTag !== sodium.crypto_secretstream_xchacha20poly1305_TAG_FINAL
        ) {
      ...
      case 'getRecoveryPhrase': {
        ...
        if (resultData.success && resultData.keys.masterSeed) {
          const mnemonic = entropyToMnemonic(
            resultData.keys.masterSeed,
            wordlist
          )
          if (!mnemonic || mnemonic.trim().length === 0) {
            throw new Error(
              `Mnemonic generation failed. Output length: ${mnemonic?.length}`
            )
          }
          result = mnemonic
        } else {
          throw new Error(
            'Failed to retrieve master seed. Incorrect password or invalid bundle.'
          )
        }
        break
      }
      ...
      case 'reEncryptBundleFromMasterKey': {
        ...
        const stretchedSeed = await _stretchSeed(
      ...
      case 'encrypt_session_key': {
        ...
        const storageKey = sodium.crypto_generichash(
          32,
          masterSeedBytes,
          new Uint8Array(new TextEncoder().encode('session-storage'))
        )
      ...
      case 'generate_otpk_batch': {
        ...
        const storageKey = sodium.crypto_generichash(
          32,
          masterSeedBytes,
          new Uint8Array(new TextEncoder().encode('otpk-storage'))
        )
        ...
        for (let i = 0; i < count; i++) {
          ...
          seedInput.set(
            new TextEncoder().encode('OTPK'),
            masterSeedBytes.length
          )
          ...
          pqSeedInput.set(
            new TextEncoder().encode('pq-otpk'),
            masterSeedBytes.length
          )
      ...
      case 'x3dh_recipient_regenerate': {
        ...
        try {
          ...
          if (ciphertexts.ct_otpk) {
            ...
            const ss_otpk = sodium.crypto_kem_xwing_dec(
              ct_otpk,
              pqOtpkKeyPair.privateKey
            )
            secrets.push(ss_otpk)
          }
      ...
      case 'dr_init_alice': {
        const { sk, theirSignedPreKeyPublic } = payload as {
          sk: CryptoBuffer
          theirSignedPreKeyPublic: CryptoBuffer
        }
        ...
        try {
          ...
          sharedSecret = new Uint8Array(
            skBytes.length + pqResult.sharedSecret.length
          )
      ...
      case 'dr_init_bob': {
        const { sk, mySignedPreKey } = payload as {
          sk: CryptoBuffer
          mySignedPreKey: SodiumKeyPair
        }
      ...
      case 'dr_ratchet_encrypt': {
        const { serializedState, plaintext } = payload as {
          serializedState: DoubleRatchetState
      ...
      case 'dr_ratchet_decrypt': {
        const { serializedState, header, ciphertext } = payload as {
          serializedState: DoubleRatchetState
          header: DoubleRatchetHeader
          ciphertext: CryptoBuffer
        }
        ...
        let newKEMs: { publicKey: Uint8Array; privateKey: Uint8Array } | null =
          null
        ...
        try {
          if (!state.KEMr || sodium.compare(headerKemPk, state.KEMr) !== 0) {
            ...
            if (header.pn - state.Nr > MAX_SKIP) {
              throw new Error(
                `Too many skipped messages: ${header.pn - state.Nr}`
              )
            }
            if (state.CKr && state.KEMr) {
              while (state.Nr < header.pn) {
                ...
                skippedKeys.push({
                  kemPk: bytesToB64(state.KEMr) || '',
                  n: state.Nr,
                  mk: bytesToB64(skippedMK) || ''
                })
          ...
          if (
            newKEMs &&
            (!state.KEMs || state.KEMs.privateKey !== newKEMs.privateKey)
          ) {
      ...
      case 'group_ratchet_decrypt': {
        ...
        if (!isValid)
          throw new Error(
            'Invalid group message signature. Potential spoofing detected!'
          )
        ...
        if (header.n - currentN > MAX_SKIP) {
          ...
          throw new Error(
            `Too many skipped messages (${header.n - currentN}). Potential DoS attack.`
          )
        }
        ...
        if (currentN === header.n) {
          ...
          throw new Error(
            'Message N is older than current state. Possibly replayed or already decrypted.'
          )
        }
      ...
      case 'group_decrypt_skipped': {
        const { mk, headerN, ciphertext, signature, senderSigningPublicKey } =
          payload
        ...
        try {
          ...
          if (!isValid)
            throw new Error(
              'Invalid group message signature (skipped key). Potential spoofing detected!'
            )
      ...
      case 'burner_dr_init_guest': {
        const { hostClassicalPk, hostPqPk } = payload as {
        ...
        try {
          ...
          const classicalShared = sodium.crypto_scalarmult(
            guestClassicalKeypair.privateKey,
            hCPK
          )

          // Gabungkan (Hybrid PQ-DR Root Key)
          const combinedSecret = new Uint8Array(
            classicalShared.length + pqResult.sharedSecret.length
          )
          ...
          const state: BurnerDoubleRatchetState = {
            ...
            CKr: null,
            KEMs_pub: pqKeypair
              ? bytesToB64(pqKeypair.publicKey) || null
              : null,
            KEMs_priv: pqKeypair
              ? bytesToB64(pqKeypair.privateKey) || null
              : null,
          ...
          result = {
            state,
            guestClassicalPk:
              guestClassicalKeypair && guestClassicalKeypair.publicKey
                ? bytesToB64(
                    new Uint8Array(
                      guestClassicalKeypair.publicKey as Iterable<number>
                    )
                  ) || ''
                : ''
          }
          ...
          if (pqKeypair) sodium.memzero(pqKeypair.privateKey)
          if (guestClassicalKeypair)
            sodium.memzero(guestClassicalKeypair.privateKey)
          sodium.memzero(hCPK)
      ...
      case 'burner_dr_init_host': {
        const { guestClassicalPk, hostClassicalSk, savedCt, hostPqSk } =
          payload as {
            guestClassicalPk: CryptoBuffer
            hostClassicalSk: CryptoBuffer
            savedCt: CryptoBuffer
        ...
        try {
          ...
          // Gabungkan
          const combinedSecret = new Uint8Array(
            classicalShared.length + pqSharedSecret.length
          )
          combinedSecret.set(classicalShared, 0)
      ...
      case 'burner_dr_encrypt': {
        const { state: serializedState, plaintext } = payload as {
          state: BurnerDoubleRatchetState
          plaintext: CryptoBuffer | string
        }
