...
export default function Register() {
  ...
  const { registerAndGeneratePhrase, user } = useAuthStore(
    useShallow((s) => ({
  ...
  useEffect(() => {
    if (
      user &&
      sessionStorage.getItem('nyx_registration_in_progress') !== 'true'
    ) {
  ...
  async function handleRegister(data: {
    ...
    if (name.length > 80) {
      throw new Error(t('auth:validation.name_length'))
    }
    if (!username || username.length < 3) {
      throw new Error(t('auth:validation.username_required'))
    }
    const usernameRegex = /^[a-zA-Z0-9_]+$/
    if (!usernameRegex.test(username)) {
      throw new Error(t('auth:validation.username_format'))
    }
    if (!password) {
      throw new Error(t('auth:validation.password_required'))
    }
    if (password.length < 8) {
      throw new Error(t('auth:validation.password_length'))
    }
    // --- End Validation ---
    ...
    try {
      ...
      const { hashUsername, generateProfileKey, encryptProfile } =
      ...
      setError(
        (err instanceof Error
          ? err.message
          : t('common:errors.unknown', 'Unknown error')) ||
          t('auth:errors.registration_failed', 'Registration failed')
      )
    }
  }
  ...
  const handleBiometricRegister = async () => {
    ...
    try {
      ...
      if (verificationResp.verified) {
        ...
        throw new Error(
          t('auth:errors.verification_failed', 'Verification failed')
        )
      }
    } catch (error: unknown) {
      if ((error as Error).name === 'NotAllowedError') {
        ...
        toast.error(
          `${t('common:errors.error_prefix', 'Error:')} ${error instanceof Error ? error.message : t('common:errors.unknown', 'Unknown error')}`
        )
      }
    } finally {
      ...
      <div className="min-h-screen flex items-center justify-center bg-bg-main p-4 relative">
        ...
        <div className="max-w-md w-full bg-bg-surface rounded-2xl p-8 shadow-neu-flat dark:shadow-neu-flat-dark text-center">
          ...
          <h2 className="text-2xl font-black text-text-primary mb-2 tracking-tight">
            {t('auth:titles.trust_verification')}
          </h2>
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-bg-main relative">
      ...
      <SEO
        title="Register"
        description="Create a new anonymous, end-to-end encrypted account on NYX. No tracking, no ads."
        ...
        <div className="max-w-md w-full mx-auto">
          ...
          <div className="mb-8">
            <h2 className="text-2xl font-bold text-accent mb-2">
              {t('auth:titles.anonymous_reg')}
            </h2>
            <p className="text-text-secondary">
              {t('auth:subtitles.register_desc')}
            </p>
          </div>
          ...
          <AuthForm
            ...
            button={
              !import.meta.env.VITE_TURNSTILE_SITE_KEY
                ? 'Configuration Error'
                : !turnstileToken
                  ? t('auth:status.verifying_security', 'Checking Security...')
            ...
            {import.meta.env.VITE_TURNSTILE_SITE_KEY ? (
              <Turnstile
                ...
                onError={() =>
                  toast.error(
                    t(
                      'auth:errors.security_check_failed',
      ...
      <div className="w-full md:w-3/5 bg-gradient-to-br from-stone-900 to-black relative overflow-hidden flex items-center justify-center p-8">
        ...
        <div className="relative z-10 text-center max-w-lg">
          <h2 className="text-3xl font-black text-white mb-4 tracking-tighter">
            {t('auth:marketing.pure_anonymity')}
          </h2>
          <p className="text-stone-400 mb-6">
            {t('auth:marketing.anonymity_desc')}
          </p>
        </div>
      </div>
    </div>
