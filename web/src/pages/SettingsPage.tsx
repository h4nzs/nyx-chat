...
const ControlModule = ({
  ...
  >
    ...
    <div className="flex items-center gap-4 mb-6 pl-2">
      ...
      <h3 className="text-xs font-black tracking-[0.2em] uppercase text-text-secondary">
        {title}
      </h3>
      <div className="h-[2px] flex-1 bg-bg-main shadow-neu-pressed dark:shadow-neu-pressed-dark rounded-full"></div>
    </div>

    <div className="relative z-10 pl-2 pr-2">{children}</div>
  </div>
)
...
export default function SettingsPage() {
  ...
  const {
    ...
    useShallow((s) => ({
      ...
      sendReadReceipts: s.sendReadReceipts,
      setReadReceipts: s.setReadReceipts,
      logout: s.logout,
      emergencyLogout: s.emergencyLogout,
      setUser: s.setUser
    }))
  )
  ...
  const { showConfirm } = useModalStore(
    useShallow((s) => ({ showConfirm: s.showConfirm }))
  )
  ...
  useEffect(() => {
    if (
      ...
      setPreviewUrl(
        profile.avatarUrl ? toAbsoluteUrl(profile.avatarUrl) || null : null
      )
    }
  }, [profile])
  ...
  const handleProfileSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const profileKeyB64 = await import('@lib/keychainDb').then((m) =>
  ...
  const handleRegisterPasskey = async () => {
    try {
      ...
      if (autoUnlockKey && encryptedKeysStr) {
        try {
          phraseToLock = await getRecoveryPhrase(
            encryptedKeysStr,
            autoUnlockKey
          )
        } catch (e) {
          console.error(
            'getRecoveryPhrase failed',
            e instanceof Error
      ...
      toast.loading(t('settings:messages.biometric_initializing'), {
        id: 'passkey'
      })
      const options = await api<unknown>(
        '/api/auth/webauthn/register/options?force=true'
      )

      toast.loading(t('settings:messages.biometric_scan_now'), {
        id: 'passkey'
      })
      ...
      const verificationResp = await api<{ verified: boolean }>(
      ...
      if (verificationResp.verified) {
        toast.success(t('settings:messages.biometric_success'), {
          id: 'passkey'
        })
  ...
  const handleProofOfWork = async () => {
    ...
    try {
      const { salt, difficulty } = await api<{
        salt: string
        difficulty: number
      ...
      toast.error(t('settings:messages.mining_failed', { error: errorMsg }), {
        id: toastId
      })
    } finally {
  ...
  const handleLogout = async () => {
    ...
    showConfirm(
      ...
      t(
        'settings:emergency.logout_desc',
        'This will log you out and clear your device data.'
      ),
      async () => {
        ...
        try {
          ...
          try {
            ...
            console.warn(
              'Failed to clear secondary sessions, proceeding to current session logout.'
            )
          }
          await api('/api/auth/logout', { method: 'POST' })

          toast.success(t('settings:messages.emergency_success'), {
            id: toastId
          })
          await executeLocalWipe()
        } catch (error: unknown) {
  ...
  return (
    <div className="w-full bg-bg-main text-text-primary p-4 md:p-8 font-sans selection:bg-accent selection:text-white pb-32">
      ...
      <header className="max-w-7xl mx-auto mb-10 flex items-center gap-6">
        ...
        <div>
          <h1 className="text-4xl font-black uppercase tracking-tighter opacity-90">
            {t('settings:header.control_deck')}
          </h1>
          <p className="text-sm font-mono text-text-secondary tracking-widest uppercase">
            {t('settings:header.system_version', {
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-12 gap-4">
        ...
        <div className="col-span-1 md:col-span-12 lg:col-span-8">
          <form onSubmit={handleProfileSubmit} className="h-full">
            <ControlModule
              title={t('settings:modules.identity')}
            ...
            >
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between items-center">
                      <label className="text-[10px] font-bold uppercase tracking-widest text-text-secondary pl-2">
                        {t('settings:identity.anonymous_id')}
                      </label>
                      {user.isVerified ? (
                    ...
                    <div className="w-full bg-black/5 dark:bg-white/5 text-sm font-mono text-text-primary p-4 rounded-xl flex items-center border border-transparent">
                      ...
                      {user.id}
                      <FiLock
                        className="ml-auto text-text-secondary opacity-50"
                        size={12}
                      />
                    </div>
                    {!user.isVerified && (
              ...
              <div className="mt-8 flex justify-end">
                ...
                >
                  {isLoading
                    ? t('settings:identity.processing')
                    : t('settings:identity.save_btn')}
                </button>
              </div>
            </ControlModule>
          </form>
          ...
          >
            ...
            <div className="relative z-10 p-8 h-full flex flex-col justify-between">
              <div>
                <div className="flex items-center gap-2 mb-2 text-accent">
                  ...
                  <span className="font-black tracking-widest uppercase text-xs">
                    {t('settings:modules.power')}
                  </span>
                </div>
                <h3 className="text-2xl font-bold leading-tight mb-2 whitespace-pre-line">
                  {t('settings:power.refuel_title')}
                </h3>
              ...
              <div className="mt-6 flex items-center justify-between">
                <span className="text-[10px] uppercase font-bold tracking-widest text-text-secondary/50">
                  {t('settings:power.status')}
                </span>
        ...
        <div className="col-span-1 md:col-span-6 lg:col-span-4">
          <ControlModule
            title={t('settings:modules.visual')}
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3 pt-2">
                <span className="text-[10px] font-bold uppercase tracking-widest text-text-secondary pl-1">
                  {t('settings:visual.accent_emitter')}
                </span>
            ...
            <div className="space-y-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-left">
                    <div className="font-bold text-sm">
                      {hasBioVault
                        ? t('settings:privacy.biometric_active')
                        : user.isVerified
                          ? t('settings:privacy.biometric_enable')
                          : t('settings:privacy.biometric_enable')}
                    </div>
                    <div className="text-[10px] text-text-secondary">
                      {t('auth:buttons.verify_identity')}
                    </div>
                  </div>
                </div>
                <div
                  className={`w-2 h-2 rounded-full shadow-[0_0_5px] ${hasBioVault ? 'bg-green-500 shadow-green-500' : 'bg-gray-500 shadow-transparent'}`}
                ></div>
              </button>
              ...
              <div className="pt-4 border-t border-white/5 space-y-3 mt-4">
                <div>
                  <h4 className="text-sm font-bold text-text-primary flex items-center gap-2">
                    ...
                    {t('settings:privacy.panic_password')}
                  </h4>
              ...
              <div className="pt-4 border-t border-white/5 space-y-3 mt-4">
                ...
                <div className="flex gap-2 items-center">
                  ...
                  >
                    <option value="">
                      {t('settings:privacy.auto_destruct_options.disabled')}
                    </option>
                    <option value="7">
                      {t('settings:privacy.auto_destruct_options.7_days')}
                    </option>
                    <option value="14">
                      {t('settings:privacy.auto_destruct_options.14_days')}
                    </option>
                    <option value="30">
                      {t('settings:privacy.auto_destruct_options.30_days')}
                    </option>
                  </select>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-sm font-bold text-text-primary">
                    {t('settings:smart.ai_reply')}
                  </h3>
                  <p className="text-[10px] text-text-secondary mt-0.5">
                    {t('settings:smart.ai_desc')}
                  </p>
                </div>
                <RockerSwitch
                ...
                <div className="p-3 bg-accent/5 border border-accent/10 rounded-lg">
                  <p className="text-[10px] text-text-secondary leading-relaxed">
                    <strong className="text-accent">
                      {t('settings:smart.privacy_note')}
                    </strong>{' '}
                    <Trans i18nKey="settings:smart.privacy_desc">
                      Incoming messages are decrypted on-device and sent
                      securely to Google Gemini for analysis. Messages are{' '}
                      <strong className="text-text-primary">not stored</strong>{' '}
                      by our servers.
                    </Trans>
                  </p>
                </div>
        ...
        <div className="col-span-1 md:col-span-12 lg:col-span-12">
          <ControlModule
            title={t('settings:modules.support')}
          ...
          >
                ...
                <div className="mt-3 p-4 bg-accent/10 border border-accent/20 rounded-2xl flex items-start gap-3 transition-all animate-in fade-in slide-in-from-top-2">
                  ...
                  <div className="text-sm text-text-secondary leading-relaxed">
                    <p className="text-accent font-bold mb-1">
                      {t('settings:support.background_guide')}
                    </p>
                    <p className="mb-2">
                    ...
                    <div className="bg-black/20 p-3 rounded-xl border border-white/5 space-y-2 text-xs">
                      <p>
                        <Trans i18nKey="settings:support.android_guide_steps">
                          ...
                          Settings {'>'} Apps {'>'} NYX (Chrome/any Browser you
                          use if you are not installing nyx into home screen){' '}
                          {'>'} Battery {'>'}{' '}
                          <span className="text-emerald-400">Unrestricted</span>
                        </Trans>
                      </p>
                      <p>
                        <Trans i18nKey="settings:support.ios_guide_steps">
                          <strong className="text-text-primary">🍎 iOS:</strong>{' '}
                          Settings {'>'} NYX (or Safari/Chrome) {'>'}{' '}
            ...
            <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4">
              <ActionButton
                label={t('settings:support.help_center')}
                icon={FiHelpCircle}
                onClick={() =>
                  window.open('https://nyx-app.my.id/help', '_blank')
                }
              />
              <ActionButton
                label={t('settings:support.report_bug')}
                icon={FiFlag}
                onClick={() => setShowReportModal(true)}
              />
              <ActionButton
                label={t('settings:support.legal')}
                icon={FiShield}
                onClick={() =>
                  window.open('https://nyx-app.my.id/privacy', '_blank')
                }
              />
            </div>
          </ControlModule>
        ...
        <div className="col-span-1 md:col-span-12 mt-8 mb-10 space-y-4">
          ...
          >
            ...
            <div className="relative z-10 flex flex-col items-center justify-center gap-2 text-orange-500 group-hover:text-orange-600">
              ...
              <span className="text-xs font-mono opacity-70">
                {t('settings:emergency.terminate_sessions')}
              </span>
            </div>
          </button>
          ...
          >
            <div className="relative z-10 flex flex-col items-center justify-center gap-2 text-red-600">
              ...
              <span className="text-xl font-black uppercase tracking-[0.2em]">
                {t('settings:emergency.delete_account')}
              </span>
      ...
      <ModalBase
        ...
        title={t('settings:modals.upgrade_title')}
      >
        <div className="space-y-6">
          <p className="text-sm text-text-secondary text-center">
            <Trans i18nKey="settings:modals.sandbox_mode">
              You are currently in{' '}
              <span className="text-yellow-500 font-bold">Sandbox Mode</span>.
          ...
          <div className="grid grid-cols-1 gap-4">
            ...
            >
              ...
              <div>
                <h3 className="font-bold text-text-primary">
                  {t('settings:modals.instant_biometric')}
                </h3>
                <p className="text-xs text-text-secondary mt-1">
                  {t('settings:modals.biometric_desc')}
                </p>
              </div>
            </button>
            ...
            >
              ...
              <div>
                <h3 className="font-bold text-text-primary">
                  {t('settings:modals.pow_mining')}
                </h3>
                <p className="text-xs text-text-secondary mt-1">
                  {miningStatus === 'idle'
                    ? t('settings:modals.pow_idle')
                    : miningStatus === 'mining'
                      ? t('settings:modals.pow_mining_status')
                      : t('settings:modals.pow_verifying')}
                </p>
              </div>
            </button>
          </div>
      ...
      <ModalBase
        ...
        title={t('settings:modals.delete_confirm_title')}
      >
        <form onSubmit={handleDeleteAccount} className="space-y-6">
          <div className="bg-red-500/10 border border-red-500/20 p-4 rounded-xl flex items-start gap-3">
            ...
            <div className="space-y-2">
              <h4 className="text-red-500 font-bold text-sm">
                {t('settings:modals.final_warning')}
              </h4>
          ...
          <div className="flex gap-3 pt-2">
            ...
            >
              {isDeleting
                ? t('settings:identity.processing')
                : t('settings:modals.execute_delete')}
            </button>
          </div>
        </form>
      </ModalBase>
