  ...
  useEffect(() => {
    ...
    const processConnection = async () => {
      ...
      try {
        ...
        const decryptedProfile = await useProfileStore
          .getState()
          .decryptAndCache(targetUser.id, targetUser.encryptedProfile || null)

        const targetName =
          decryptedProfile?.name &&
          decryptedProfile.name !== 'Unknown' &&
          decryptedProfile.name !== 'Encrypted User'
            ? decryptedProfile.name
        ...
        showConfirm(
          ...
          async () => {
            try {
              toast.loading(t('connect.connecting_to', { name: targetName }), {
                id: 'connect'
              })
