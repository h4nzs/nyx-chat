      ...
      <div className="w-full max-w-2xl mb-8">
        ...
        >
          ...
          <span className="font-bold text-sm uppercase tracking-wide">
            {t('common:actions.return', 'Return')}
          </span>
        </Link>
      </div>
      ...
      >
        ...
        <div className="bg-bg-surface p-8 border-b border-black/5 dark:border-white/5 relative">
          <div className="flex items-center gap-6">
            ...
            <div>
              <h1 className="text-2xl font-black uppercase tracking-tighter text-text-primary">
                {t('settings:keys_page.title')}
              </h1>
              <p className="text-xs font-mono text-text-secondary mt-1 uppercase tracking-widest">
                {t('settings:keys_page.subtitle')}
              </p>
            </div>
          </div>
        ...
        <div className="p-8 space-y-8">
          <p className="text-sm text-text-secondary leading-relaxed font-medium">
            <Trans i18nKey="settings:keys_page.description">
              Your private keys are the only way to decrypt your messages. They
              are stored locally on this device.
              <strong className="text-text-primary">
                {' '}
                Losing these keys means losing your message history forever.
              </strong>
            </Trans>
          </p>
            ...
            <div className="p-6 rounded-2xl bg-bg-main shadow-neu-pressed-light dark:shadow-neu-pressed-dark border border-white/10">
              <div className="flex items-start gap-4 mb-4">
                ...
                <div>
                  <h3 className="font-bold text-text-primary">
                    {t('settings:keys_page.recovery_title')}
                  </h3>
                  <p className="text-xs text-text-secondary mt-1">
                    {t('settings:keys_page.recovery_desc')}
                  </p>
                </div>
              </div>
              <button
              ...
              >
                {isProcessing ? (
                  ...
                  t('settings:keys_page.reveal_btn')
                )}
              </button>
            </div>
            ...
            <div className="relative p-6 rounded-2xl bg-red-500/5 border border-red-500/20 overflow-hidden">
              ...
              <div className="flex items-start gap-4 mb-4">
                ...
                <div>
                  <h3 className="font-bold text-red-600 dark:text-red-400">
                    {t('settings:keys_page.rotation_title')}
                  </h3>
                  <p className="text-xs text-red-600/70 dark:text-red-400/70 mt-1">
                    {t('settings:keys_page.rotation_desc')}
                  </p>
                </div>
              </div>
              ...
              >
                {isProcessing ? (
                  ...
                  t('settings:keys_page.rotate_btn')
                )}
              </button>
            </div>
          </div>
