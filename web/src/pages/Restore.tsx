...
export default function RestorePage() {
  ...
  const handleRestore = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const challengeRes = await api<{ nonce: string }>(
        `/api/auth/recover/challenge?identifier=${encodeURIComponent(identifier)}`,
        { method: 'GET' }
      )
      ...
      const {
        ...
        signatureB64
      } = await recoverAccountWithSignature(
        trimmedPhrase,
        password,
        identifier,
        timestamp,
      ...
      if (
        ...
        toast.error(
          (error instanceof Error
            ? error.message
            : t('common:errors.unknown')) || t('auth:restore.error_generic')
        )
      }
    } finally {
    ...
    <div className="min-h-screen w-full flex flex-col items-center justify-center bg-bg-main text-text-primary p-4 font-mono">
      <div className="w-full max-w-md">
        ...
        <form
          ...
          className="bg-bg-surface p-8 rounded-2xl shadow-neumorphic-convex border border-white/5 relative overflow-hidden"
        >
          <div className="space-y-6">
            <div className="form-control">
              <label className="label mb-2 block">
                <span className="text-[10px] font-bold uppercase tracking-widest text-text-secondary">
                  {t('auth:restore.labels.identifier')}
                </span>
              </label>
              <input
                type="text"
            ...
            <div className="form-control">
              <label className="label mb-2 block">
                <span className="text-[10px] font-bold uppercase tracking-widest text-text-secondary">
                  {t('auth:restore.labels.new_password')}
                </span>
              </label>
              <input
                type="password"
          ...
          <div className="mt-8">
            ...
            >
              ...
              {isRestoring
                ? t('auth:restore.buttons.verifying')
                : t('auth:restore.buttons.recover')}
            </button>
          </div>
        </form>
