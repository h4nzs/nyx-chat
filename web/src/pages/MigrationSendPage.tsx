...
export default function MigrationSendPage() {
  ...
  const [status, setStatus] = useState<
    'prefetching' | 'scanning' | 'encrypting' | 'sending' | 'success'
  >('prefetching')
    ...
    const prefetch = async () => {
      try {
        ...
        if (socket?.connected) {
          socket.disconnect()
          console.log(
            'System Frozen: Socket disconnected to prevent Ratchet Race Condition.'
          )
        }
        ...
        toast.error(
          tRef.current(
            'common:migration.read_vault_failed',
            'Gagal membaca brankas data.'
          )
        )
      }
    }
    ...
    const timer = setTimeout(() => {
      ...
      scannerRef.current
        ...
        .catch((err) => {
          ...
          toast.error(
            tRef.current(
              'common:migration.camera_denied',
  ...
  const processMigration = async (qrText: string) => {
    ...
    toast.loading(
      t('common:migration.encrypting_vault', 'Mengenknripsi brankas...'),
      { id: 'send' }
    )
    ...
    try {
      ...
      const receiverPubKeyBytes = sodium.from_base64(
        pubKey,
        sodium.base64_variants.URLSAFE_NO_PADDING
      )

      // 1. Encrypt Huge Vault using AES-GCM (Worker)
      const { combinedData, key: aesKey } = await worker_file_encrypt(
      ...
      const sealedKey = sodium.to_base64(
        sealedKeyBytes,
        sodium.base64_variants.URLSAFE_NO_PADDING
      )
      ...
      setStatus('sending')
      toast.loading(
        t('common:migration.tunneling', 'Membangun terowongan aman...'),
        { id: 'send' }
      )
      ...
      toast.loading(
        t('common:migration.waiting_receiver', 'Menunggu penerima...'),
        { id: 'send' }
      )
      ...
      if (ackResult) {
        setStatus('success')
        toast.success(
          t('common:migration.transfer_complete', 'Transfer selesai!'),
          { id: 'send' }
        )
        ...
        throw new Error(
          t(
            'common:migration.receiver_failed',
            'Penerima gagal mendekripsi atau kehabisan waktu'
          )
        )
      }
    } catch (e) {
      console.error(e)
      toast.error(
        t('common:migration.failed_generic', 'Proses migrasi gagal.'),
        { id: 'send' }
      )
      setStatus('scanning')
    }
    ...
    <div className="min-h-screen bg-bg-main text-text-primary flex flex-col items-center justify-center p-4">
      <div className="text-center mb-8">
        <h1 className="text-2xl font-black uppercase tracking-widest text-text-primary">
          {t('common:migration.send_title', 'Kirim Data')}
        </h1>
        <p className="text-xs text-text-secondary mt-2">
          {t('common:migration.send_desc', 'Pindai QR code di perangkat baru')}
        </p>
      </div>
      ...
      <div className="w-full max-w-sm aspect-square bg-black/90 rounded-3xl overflow-hidden shadow-neumorphic-pressed relative mb-8">
        {status === 'prefetching' && (
          <div className="absolute inset-0 flex flex-col items-center justify-center text-accent">
            ...
            <span className="font-mono text-xs uppercase">
              {t('common:migration.preparing', 'Menyiapkan Data...')}
            </span>
          </div>
        )}
        ...
        {(status === 'encrypting' || status === 'sending') && (
          <div className="absolute inset-0 bg-black/90 flex flex-col items-center justify-center text-accent z-10">
            ...
            <span className="font-mono text-[10px] uppercase mt-2">
              {t('common:migration.status_tunneling', 'Mengirim Data...')}
            </span>
          </div>
        )}
      </div>
