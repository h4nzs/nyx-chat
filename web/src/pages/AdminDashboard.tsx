...
export default function AdminDashboard() {
  ...
  const loadMetrics = () => {
    authFetch('/api/admin/system-status')
      ...
      .catch((err: unknown) =>
        toast.error(
          i18n.t('errors:failed_to_load_metrics', 'Failed to load metrics')
        )
      )
  }
    ...
    showConfirm(
      ...
      async () => {
        try {
          ...
          toast.success(
            i18n.t('common:user_unbanned', `User @${user.username} unbanned!`, {
    ...
    <div className="min-h-screen bg-bg-main text-text-primary p-8 font-mono overflow-y-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-bg-surface p-6 rounded-xl border border-white/10 shadow-lg">
          <h3 className="text-xs font-bold opacity-50 mb-2 tracking-wider">
            VPS RAM USAGE
          </h3>
          <p className="text-xl font-bold">{metrics.vps.ramUsage}</p>
          <p className="text-xs text-text-secondary mt-2">
            Uptime: {metrics.vps.uptime}
          </p>
        </div>
        <div className="bg-bg-surface p-6 rounded-xl border border-white/10 shadow-lg">
          <h3 className="text-xs font-bold opacity-50 mb-2 tracking-wider">
        ...
        <div className="bg-bg-surface p-6 rounded-xl border border-white/10 shadow-lg">
          ...
          <p className="text-2xl font-bold text-blue-400">
            {metrics.storage.totalSizeMB}
          </p>
          <p className="text-xs text-text-secondary mt-2">
            {metrics.storage.totalFiles} Files
          </p>
        </div>
      </div>
        ...
        <div className="lg:col-span-1">
          <div className="bg-bg-surface p-8 rounded-xl border border-red-500/20 shadow-lg">
            ...
            <p className="text-sm text-text-secondary mb-6 leading-relaxed">
              Suspend user access immediately. This action will forcibly
              disconnect the user and prevent future logins.
            </p>
            <button
          ...
          <div className="bg-bg-surface rounded-xl border border-white/10 shadow-lg overflow-hidden flex flex-col h-[500px]">
            <div className="p-4 border-b border-white/10 bg-white/5 flex justify-between items-center">
              <h3 className="font-bold text-sm tracking-wider">
                {t('suspended_accounts', 'SUSPENDED ACCOUNTS')} (
                {bannedUsers.length})
              </h3>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              {bannedUsers.length === 0 ? (
                ...
                bannedUsers.map((user) => (
                  <div
                    key={user.id}
                    className="bg-bg-main p-4 rounded-lg border border-white/5 flex flex-col md:flex-row md:items-center justify-between gap-4"
                  >
                    <div>
