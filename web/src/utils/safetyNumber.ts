...
export async function computeSafetyNumberParts(
  ...
  if (mySigningKey.length === 64) {
    ...
    throw new Error(
      `Invalid mySigningKey length: expected 32 or 64, got ${mySigningKey.length}`
    )
  }
  myParts.push(normalizedMySigningKey)

  const myTotalLen = myParts.reduce(
    (acc: number, p: Uint8Array) => acc + p.length,
    0
  )
  ...
  // Their Parts
  const theirX25519PubKey = sodium.from_base64(
    peer.publicKey,
  ...
  if (theirSigningPubKeyRaw.length === 64) {
    ...
    throw new Error(
      `Invalid peer signingKey length: expected 32 or 64, got ${theirSigningPubKeyRaw.length}`
    )
  }
  ...
  const theirTotalLen = theirParts.reduce(
    (acc: number, p: Uint8Array) => acc + p.length,
    0
  )
