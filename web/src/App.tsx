...
const AppContent = () => {
  const { theme, accent } = useThemeStore(
    useShallow((s) => ({ theme: s.theme, accent: s.accent }))
  )
  const { bootstrap, logout, user, isBootstrapping } = useAuthStore(
    useShallow((s) => ({
  ...
  useEffect(() => {
    const handleSwMessage = (event: MessageEvent) => {
      if (event.data && event.data.type === 'PWA_ROUTER_NAVIGATE') {
        console.log(
          '[App] Received navigation command from SW:',
          event.data.url
        )
  ...
  const isDeviceFlow = useCallback((pathname: string) => {
    return (
      ...
      pathname.startsWith('/migrate-receive')
    )
  }, [])
  ...
  useEffect(() => {
    const initAuth = async () => {
      try {
        ...
        const { user, accessToken, silentRefresh, logout } =
  ...
  useEffect(() => {
    const handleVisibilityChange = async () => {
      ...
      if (document.visibilityState === 'visible') {
        ...
        const { privacyCloak } = await import('./store/settings').then((m) =>
