...
export default function LazyImage({
  ...
  const [decryptionStatus, setDecryptionStatus] =
    useState<DecryptionStatus>('pending')
  ...
  useEffect(() => {
    ...
    const handleImageLoad = async () => {
      ...
      // 2. Cek Enkripsi
      const isEncrypted =
        message.fileType?.includes('encrypted') ||
  ...
  const renderOverlay = () => {
    ...
    const baseClasses =
      'absolute inset-0 flex flex-col items-center justify-center rounded-lg backdrop-blur-sm transition-all duration-300 z-10 p-2 text-center'
    ...
    if (decryptionStatus === 'waiting_for_key') {
      return (
        <div
          className={`${baseClasses} bg-yellow-500/20 text-yellow-600 dark:text-yellow-400`}
        >
      ...
      return (
        ...
        >
          ...
          <span className="text-[10px] font-medium">
            {t('messages.failed_retry', 'Failed. Click to retry.')}
          </span>
        </div>
      )
    }
  ...
  return (
    <div
      className={`relative overflow-hidden bg-gray-100 dark:bg-gray-800 rounded-lg flex items-center justify-center min-w-[200px] min-h-[150px] ${className || ''}`}
    >
      {renderOverlay()}
      {imageUrl ? (
        <img
