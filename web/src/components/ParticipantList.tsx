...
const ParticipantActions = ({
  ...
  const handleRoleChange = async (newRole: 'ADMIN' | 'MEMBER') => {
    ...
    try {
      await api(
        `/api/conversations/${conversationId}/participants/${participant.id}/role`,
        {
          method: 'PUT',
      ...
      toast.success(
        t('modals:participants.toasts.role_changed', {
          name: profile.name,
  ...
  const handleRemove = () => {
    ...
    showConfirm(
      ...
      async () => {
        try {
          await api(
            `/api/conversations/${conversationId}/participants/${participant.id}`,
            {
              method: 'DELETE'
            }
          )
          toast.success(
            t('modals:participants.toasts.removed', { name: profile.name })
          )
        } catch (error: unknown) {
          const msg =
            error instanceof Error ? error.message : t('common:errors.unknown')
          toast.error(
            t('modals:participants.toasts.remove_failed', { error: msg })
          )
        }
      }
    )
  ...
  const handleBlockToggle = async () => {
    ...
    try {
      if (isBlocked) {
        ...
        toast.success(
          t('modals:participants.toasts.unblocked', { name: profile.name })
        )
      } else {
        await blockUser(participant.id)
        toast.success(
          t('modals:participants.toasts.blocked', { name: profile.name })
        )
      }
    } catch (error: unknown) {
      const msg =
            ...
            {amIAdmin && participant.role === 'MEMBER' && (
              <li>
                <button
                  ...
                  className="w-full text-left px-4 py-2 text-sm text-text-primary hover:bg-bg-surface"
                >
                  {t('modals:participants.make_admin')}
                </button>
              </li>
            )}
            {amIAdmin &&
              ...
              user?.id !== participant.id && (
                <li>
                  <button
                    ...
                    className="w-full text-left px-4 py-2 text-sm text-text-primary hover:bg-bg-surface"
                  >
                    {t('modals:participants.dismiss_admin')}
                  </button>
                </li>
              )}
            {amIAdmin && user?.id !== participant.id && (
              <li>
                <button
                  ...
                  className="w-full text-left px-4 py-2 text-sm text-destructive hover:bg-destructive hover:text-destructive-foreground"
                >
                  {t('modals:participants.remove')}
                </button>
              </li>
            )}
            <li>
              <button
                ...
                className={`w-full text-left px-4 py-2 text-sm ${isBlocked ? 'text-green-500 hover:bg-green-500/10' : 'text-destructive hover:bg-destructive/10'}`}
              >
                {isBlocked
                  ? t('modals:participants.unblock')
                  : t('modals:participants.block')}
              </button>
            </li>
          </ul>
        </div>
...
const ParticipantItem = ({
  ...
  conversationId: ConversationId
  amIAdmin: boolean
  handleProfileClick: (p: Participant) => void
}) => {
      ...
      >
        {profile.avatarUrl ? (
          ...
          <DefaultAvatar
            ...
            id={p.id}
            className="w-10 h-10 bg-bg-primary flex-shrink-0"
          />
        )}
        <div className="min-w-0 flex-1">
          ...
          <p className="text-xs text-text-secondary truncate">
            {profile.description || t('modals:group_info.no_desc')}
          </p>
          {p.role === 'ADMIN' && (
            <p className="text-xs text-accent-color">
              {t('modals:participants.admin_role', 'Admin')}
            </p>
          )}
        </div>
      </button>
