...
export default function EditGroupInfoModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!trimmedTitle) {
      setError(
        t('modals:edit_group.error_empty_title', 'Title cannot be empty')
      )
      return
    }
    setIsLoading(true)
      ...
      footer={
        <>
          <button
            type="button"
            onClick={onClose}
            className="px-4 py-2 rounded-md text-text-primary bg-secondary hover:bg-secondary/80"
          >
    ...
    >
      <form id="edit-group-form" onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          >
            {t('modals:edit_group.group_name')}
          </label>
          <input
            id="group-title"
            type="text"
        ...
        <div>
          <label
            ...
            className="block text-sm font-medium text-text-secondary mb-1"
          >
            {t('modals:edit_group.description')}
          </label>
          <textarea
