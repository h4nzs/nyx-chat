...
const ContactItem = ({
  ...
  const profile = useProfileStore((state) => {
    const cacheKey = contact.encryptedProfile
      ? `${contact.id}_${contact.encryptedProfile.substring(0, 32)}`
      : contact.id
  ...
  useEffect(() => {
    if (!profile && contact.encryptedProfile) {
      useProfileStore
        .getState()
        .decryptAndCache(contact.id, contact.encryptedProfile)
    }
      ...
      <div className="flex items-center gap-3">
        {avatarUrl ? (
          <img
            src={avatarUrl}
            alt=""
            className="w-9 h-9 rounded-full border border-white/10 group-hover:border-accent/50 transition-colors object-cover"
          />
        ) : (
          <DefaultAvatar
            name={name}
            id={contact.id}
            className="w-9 h-9 border border-white/10 group-hover:border-accent/50 transition-colors"
          />
        )}
...
export default function CreateStoryModal({ onClose }: { onClose: () => void }) {
  ...
  const [privacyMode, setPrivacyMode] = useState<'ALL' | 'EXCLUDE' | 'ONLY'>(
    'ALL'
  )
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await useStoryStore.getState().postStory(
      file,
      text,
      privacyMode,
      selectedUsers.map((id) => asUserId(id))
    )
    onClose()
  }
  ...
  return (
    <>
      <ModalBase
        isOpen={true}
        onClose={onClose}
        title={t('modals:create_story.title')}
      >
          ...
          {safePreviewUrl ? (
            <div className="relative w-full h-48 rounded-xl overflow-hidden bg-black/20 group">
              {file?.type.startsWith('video/') ? (
                ...
                <img
                  src={safePreviewUrl}
                  alt={t('common:defaults.preview', 'Preview')}
                  className="w-full h-full object-contain"
                />
              )}
            ...
            <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-white/10 rounded-xl hover:border-accent/50 hover:bg-accent/5 transition-all cursor-pointer">
              ...
              <span className="text-sm text-text-secondary">
                {t('modals:create_story.add_media')}
              </span>
              <input
                type="file"
                className="hidden"
          ...
          <div className="flex flex-col pt-2">
            ...
            >
              ...
              <span className="font-medium">
                {privacyMode === 'ALL'
                  ? t('modals:create_story.privacy.all')
                  : privacyMode === 'EXCLUDE'
                    ? t('modals:create_story.privacy.exclude', {
                        count: selectedUsers.length
                      })
                    : t('modals:create_story.privacy.only', {
                        count: selectedUsers.length
                      })}
              </span>
            </button>
        ...
        <div className="fixed inset-0 z-[150] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in zoom-in-95 duration-200">
          <div className="bg-bg-main w-full max-w-sm rounded-3xl border border-white/10 flex flex-col max-h-[85vh] shadow-2xl overflow-hidden">
            <div className="p-5 border-b border-white/5 flex justify-between items-center bg-black/20">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-text-primary text-lg">
                  {t('modals:create_story.privacy.label')}
                </h3>
              </div>
              <button
                onClick={() => setShowPrivacySettings(false)}
                className="text-text-secondary hover:text-white p-1 bg-white/5 rounded-full transition-colors"
              >
                <FiX size={18} />
              </button>
            </div>
            ...
            <div className="p-2 flex flex-col overflow-y-auto custom-scrollbar">
              <div className="p-3 space-y-4">
                <label className="flex items-center justify-between cursor-pointer p-3 rounded-xl hover:bg-white/5 transition-colors border border-transparent hover:border-white/5">
                  <span className="text-text-primary font-medium">
                    {t('modals:create_story.privacy.option_all')}
                  </span>
                  <input
                    type="radio"
                    name="privacy"
                ...
                <label className="flex items-center justify-between cursor-pointer p-3 rounded-xl hover:bg-white/5 transition-colors border border-transparent hover:border-white/5">
                  <span className="text-text-primary font-medium">
                    {t('modals:create_story.privacy.option_exclude')}
                  </span>
                  <input
                    type="radio"
                    name="privacy"
                    checked={privacyMode === 'EXCLUDE'}
                    onChange={() => {
                ...
                <label className="flex items-center justify-between cursor-pointer p-3 rounded-xl hover:bg-white/5 transition-colors border border-transparent hover:border-white/5">
                  <span className="text-text-primary font-medium">
                    {t('modals:create_story.privacy.option_only')}
                  </span>
                  <input
                    type="radio"
                    name="privacy"
              ...
              {privacyMode !== 'ALL' && (
                <div className="mt-2 border-t border-white/5 pt-4 px-3">
                  <p className="text-[11px] font-bold text-text-secondary mb-3 uppercase tracking-wider px-2">
                    {t('modals:create_story.privacy.select_contacts')}
                  </p>
                  <div className="space-y-1">
                    ...
                    {contacts.length === 0 ? (
                      <p className="text-xs text-text-secondary text-center py-6">
                        {t('modals:create_story.privacy.no_contacts')}
                      </p>
                    ) : null}
                  </div>
                </div>
            ...
            <div className="p-4 border-t border-white/5 bg-black/40">
              <button
                onClick={() => setShowPrivacySettings(false)}
                className="w-full bg-accent text-white py-3.5 rounded-2xl font-bold shadow-neu-pressed hover:scale-[1.02] active:scale-95 transition-all"
              >
