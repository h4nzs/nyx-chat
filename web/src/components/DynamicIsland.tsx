...
import useDynamicIslandStore, {
  Activity,
  NotificationActivity,
  UploadActivity
} from '@store/dynamicIsland'
...
const NotificationView = ({ activity }: { activity: NotificationActivity }) => {
  ...
  const profile = useUserProfile(
    activity.sender as { id: string; encryptedProfile?: string | null }
  )
  ...
  return (
    <div
      onClick={handleClick}
      className="w-full h-full flex items-center gap-3 px-1 cursor-pointer group"
    >
    ...
    <div className="w-full h-full flex items-center gap-3 px-1">
      ...
      <div className="flex-1 min-w-0 flex flex-col justify-center gap-1">
        <div className="flex justify-between items-center">
          <p className="text-[10px] font-bold text-white/90 uppercase tracking-wider truncate max-w-[120px]">
            {activity.fileName}
          </p>
