    ...
    const generate = async () => {
      try {
        ...
        if (mounted) {
          setError(
            t(
              'burner.failed_generate',
              'Failed to generate secure burner link.'
            )
          )
          setIsGenerating(false)
        }
      ...
      >
        ...
        >
            ...
            {isGenerating ? (
              <div className="flex items-center gap-3 text-text-secondary">
                ...
                <span className="text-sm font-mono">
                  {t('burner.provisioning', 'Provisioning keys...')}
                </span>
              </div>
            ) : error ? (
          ...
          >
            ...
            {copied
              ? t('burner.copied', 'Copied to Clipboard')
              : t('burner.copy_link', 'Copy Secure Link')}
          </button>
        </motion.div>
      </motion.div>
    </AnimatePresence>
