...
export const LinkedDevicesPanel: React.FC = () => {
  ...
  const broadcastHistorySync = useMessageStore(
    (state) => state.broadcastHistorySync
  )
  ...
  const loadDevices = async () => {
    try {
      ...
      toast.error(
        i18n.t(
          'errors:failed_to_load_linked_devices',
      ...
      .showConfirm(
        ...
        async () => {
          try {
            ...
            toast.success(
              i18n.t(
                'common:device_revoked_successfully',
            ...
            toast.error(
              i18n.t(
                'errors:failed_to_revoke_device',
  ...
  const handleSyncHistory = async () => {
    ...
    const selfChat = conversations.find(
      (c) =>
        !c.isGroup &&
    ...
    if (!selfChat) {
      toast.error(
        i18n.t(
          'errors:please_start_a_chat_with_yourself_saved_',
          "Please start a chat with yourself ('Saved Messages') first to use as a sync channel."
        )
      )
      return
    }
    ...
    try {
      ...
      toast.success(
        i18n.t(
          'common:history_sync_payload_broadcasted_success',
          'History sync payload broadcasted successfully.'
        )
      )
    } catch (e) {
      ...
      toast.error(
        i18n.t(
          'errors:failed_to_broadcast_history_sync',
          'Failed to broadcast history sync.'
        )
      )
    }
  }
  ...
  if (isLoading)
    return (
      <div className="p-4 text-center text-text-secondary">
        {t('linked_devices.loading', 'Loading devices...')}
      </div>
    )
  ...
  return (
    <div className="relative bg-bg-main rounded-xl p-6 overflow-hidden shadow-neu-flat dark:shadow-neu-flat-dark border-t border-white/40 dark:border-white/5">
      ...
      <div className="flex items-center gap-4 mb-6 pl-2">
        ...
        <h3 className="text-xs font-black tracking-[0.2em] uppercase text-text-secondary hidden sm:block">
          {t('linked_devices.title', 'Linked Devices')}
        </h3>
      ...
      <div className="relative z-10 pl-2 pr-2 space-y-4">
        {devices.map((device) => {
          ...
          return (
            <div
              key={device.id}
              className="p-4 flex items-center justify-between rounded-xl bg-bg-main shadow-neu-pressed dark:shadow-neu-pressed-dark"
            >
              <div className="flex items-center gap-4">
                <div
                  className={`p-3 rounded-full shadow-neu-flat dark:shadow-neu-flat-dark ${device.isCurrent ? 'text-emerald-500' : 'text-text-secondary'}`}
                >
                  {isMobile ? (
                ...
                <div className="min-w-0 pr-2">
                  <div className="flex items-center gap-2 flex-wrap">
                    <p className="font-bold text-sm text-text-primary truncate max-w-[150px] sm:max-w-xs">
                      {device.name ||
                        t('linked_devices.unknown_device', 'Unknown Device')}
                    </p>
                    {device.isCurrent && (
                  ...
                  <p className="text-[10px] text-text-secondary mt-1 uppercase tracking-wider font-mono truncate">
                    {t('linked_devices.active', 'Active: ')}{' '}
