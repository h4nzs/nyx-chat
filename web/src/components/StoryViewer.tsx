    ...
    const loadMedia = async () => {
      ...
      if (currentStory.decryptedData.fileKey) {
        try {
          ...
          let originalType =
            currentStory.decryptedData.mimeType?.split(';')[0] ||
            'application/octet-stream'
          if (
            currentStory.decryptedData.mediaUrl
              ?.toLowerCase()
              .endsWith('.svg') ||
            currentStory.decryptedData.mimeType?.includes('svg')
          ) {
            originalType = 'image/svg+xml'
          }
          let decryptedBlob = await decryptFile(
            encryptedBlob,
            currentStory.decryptedData.fileKey,
            originalType
          )
  ...
  const handleDelete = async () => {
    try {
      ...
      useStoryStore.setState((state) => ({
        stories: {
          ...state.stories,
          [userId]: state.stories[userId].filter(
    ...
    >
      ...
      <div className="absolute top-6 left-0 right-0 px-4 flex justify-between items-center z-20 bg-gradient-to-b from-black/60 to-transparent pb-4 pt-2 pointer-events-auto">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-white text-sm font-bold shadow-sm drop-shadow-md">
              {profile.name}
            </p>
        ...
        <div className="flex items-center gap-4">
          {isMe && (
            <button
              ...
              className="p-2 text-white/80 hover:text-red-500 transition-colors drop-shadow-md"
            >
              <FiTrash2 size={20} />
            </button>
          )}
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black/80 to-transparent z-20 pointer-events-auto">
          ...
          >
            ...
            <button
              ...
              className="text-white p-1.5 hover:text-accent hover:scale-110 active:scale-95 transition-all disabled:opacity-30 disabled:hover:scale-100"
            >
              <FiSend size={18} />
            </button>
          </form>
