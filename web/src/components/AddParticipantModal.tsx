    ...
    const delayDebounceFn = setTimeout(async () => {
      ...
      if (rawQuery.length > 2) {
        ...
        try {
          ...
          const users = await api<MinimalProfile[]>(
          ...
          setSearchResults(
            optimisticUsers.filter(
              (u) => !existingParticipantIds.includes(asUserId(u.id))
            )
          )
        } catch (error) {
    ...
    <ModalBase
      ...
      footer={
        <>
          ...
          >
            {isLoading
              ? t('modals:add_participant.adding')
              : t('modals:add_participant.add_selected')}
          </button>
        </>
      }
    >
        ...
        <div className="mb-4">
          ...
          {isSearching && (
            <p className="text-sm text-text-secondary mt-2">
              {t('modals:add_participant.searching')}
            </p>
          )}
        </div>
            ...
            : searchTerm.trim().length > 2 &&
              !isSearching && (
                <p className="p-2 text-text-secondary">
                  {t('modals:add_participant.no_users')}
                </p>
              )}
        </div>
      </form>
