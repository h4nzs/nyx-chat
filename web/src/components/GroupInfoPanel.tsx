...
const GroupInfoPanel = ({
  ...
  const amIAdmin =
    conversation.participants.find((p) => p.id === user?.id)?.role === 'ADMIN'
  ...
  const handleUploadCroppedAvatar = async (croppedFile: File) => {
    const toastId = toast.loading(
      t('modals:group_info.toasts.processing_avatar')
    )
    ...
    try {
      ...
      const { encryptGroupMetadata, ensureGroupSession } =
      ...
      toast.success(t('modals:group_info.toasts.avatar_updated'), {
        id: toastId
      })
      ...
      toast.error(t('modals:group_info.toasts.upload_failed', { error: msg }), {
        id: toastId
      })
      setAvatarCropTarget(null)
    }
  ...
  const handleForceRotateKeys = async () => {
    ...
    try {
      const { forceRotateGroupSenderKey, ensureGroupSession } =
        await import('@utils/crypto')
      const { emitGroupKeyDistribution } = await import('@lib/socket')
      ...
      if (distributionKeys) {
        ...
        toast.success('Encryption keys rotated successfully via ML-KEM', {
          id: toastId
        })
      } else {
  ...
  const handleLeaveGroup = async () => {
    ...
    try {
      ...
      const msg =
        error instanceof Error ? error.message : t('common:errors.unknown')
      toast.error(t('modals:group_info.toasts.leave_failed', { error: msg }), {
        id: toastId
      })
    }
  }
  ...
  return (
    <div className="fixed inset-0 z-40">
      ...
      <div
        className={`absolute top-0 right-0 h-full w-full max-w-md bg-bg-surface shadow-neumorphic-convex z-50 flex flex-col transition-transform duration-300 ease-in-out ${isPanelOpen ? 'translate-x-0' : 'translate-x-full'}`}
      >
        <header className="p-4 flex items-center flex-shrink-0">
          <button
            onClick={handleClose}
            className="btn-flat p-2 rounded-full text-text-secondary mr-2"
          >
            <FiX size={24} />
          </button>
          <h2 className="text-xl font-bold text-text-primary">
            {t('modals:group_info.title')}
          </h2>
        </header>
                ...
                >
                  ...
                  <div className="bg-bg-surface rounded-xl shadow-neumorphic-convex p-6 text-center relative">
                      ...
                      {amIAdmin && (
                        <>
                          <button
                            ...
                            className="absolute bottom-0 right-0 bg-accent-gradient rounded-full p-2 text-white hover:opacity-90"
                            aria-label={t('modals:group_info.change_avatar')}
                          >
                            <FiEdit2 size={16} />
                          </button>
                          <input
                    ...
                    <p className="text-text-secondary mt-1">
                      {conversation.decryptedMetadata?.description ||
                        t('modals:group_info.no_desc')}
                    </p>
                    {amIAdmin && (
                      <button
                        onClick={() => setIsEditing(true)}
                        className="absolute top-4 right-4 text-text-secondary hover:text-accent-color"
                      >
                        <FiEdit2 size={20} />
                      </button>
                    )}
                  ...
                  <div className="bg-bg-surface rounded-xl shadow-neumorphic-convex">
                    <div className="p-6 border-b border-border">
                      <h4 className="text-lg font-semibold text-text-primary">
                        {t('modals:group_info.member_count', {
        ...
        {isEditing && (
          <EditGroupInfoModal
            ...
            currentDescription={
              conversation.decryptedMetadata?.description || null
            }
