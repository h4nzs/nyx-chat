...
const UserProfile = memo(function UserProfile() {
  ...
  const { showConfirm: confirmLogout } = useModalStore(
    useShallow((state) => ({ showConfirm: state.showConfirm }))
  )
  const { privacyCloak, setPrivacyCloak } = useSettingsStore(
    useShallow((s) => ({
      privacyCloak: s.privacyCloak,
      setPrivacyCloak: s.setPrivacyCloak
    }))
  )
      ...
      <div className="flex items-center justify-between px-6 py-6 bg-bg-main z-10">
          ...
          <div className="relative flex-shrink-0">
            {profile.avatarUrl ? (
              ...
              <DefaultAvatar
                ...
                id={user?.id}
                className="w-10 h-10 border-2 border-bg-main group-hover:border-accent transition-colors"
              />
            )}
          ...
          <div className="min-w-0">
            ...
            {user.isVerified && (
              <span className="text-[10px] text-accent font-bold tracking-wider">
                {t('modals:user_info.verified', 'VERIFIED')}
              </span>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          ...
          <button
            ...
            aria-label={t(
              'common:actions.toggle_cloak',
              'Toggle Privacy Cloak'
            )}
          >
        ...
        <div className="flex items-center gap-2">
          ...
          {user.isVerified && (
            <div
              ...
              title={t('modals:user_info.verified', 'Verified')}
            ></div>
          )}
        </div>
...
const SearchResults = memo(function SearchResults({
  ...
  return (
    <Virtuoso
      ...
      components={{
        Header: () => (
          <p className="text-xs font-bold text-text-secondary px-6 mb-4 mt-2">
            {t('sidebar.global_search')}
          </p>
        ),
        EmptyPlaceholder: () => (
      ...
      itemContent={(index, user) => (
        <SearchResultItem key={user.id} user={user} onSelect={onSelect} />
      )}
    />
  )
})
...
const ConversationItem = memo(
  function ConversationItem({
    ...
    const peerUser = !conversation.isGroup
      ? conversation.participants?.find((p) => p.id !== meId)
      : null
    const peerProfile = useUserProfile(
      peerUser as { id: string; encryptedProfile?: string | null }
    )
    const title = conversation.isGroup
      ? conversation.decryptedMetadata?.title ||
        t('common:defaults.group_unknown', 'Unknown Group')
      : peerProfile.name || t('common:defaults.conversation', 'Conversation')
    const isUnread = conversation.unreadCount > 0
    const isPinnedByMe = Boolean(
      conversation.participants?.some((p) => p.id === meId && p.isPinned)
    )
    const openMenu = useContextMenuStore((s) => s.openMenu)

    const cloakClass = privacyCloak
      ? 'blur-[6px] opacity-70 group-hover:blur-none group-hover:opacity-100 group-active:blur-none group-active:opacity-100 transition-all duration-300 select-none'
      : ''
    ...
    const avatarSrc = conversation.isGroup
      ...
      : peerProfile.avatarUrl
        ? toAbsoluteUrl(peerProfile.avatarUrl)
        : undefined
    ...
    const renderPreviewText = () => {
      if (!conversation.lastMessage)
        return t('chat:messages.no_messages_yet', 'No messages yet')
          ...
          <span className="flex items-center gap-1 text-accent text-sm font-medium">
            {conversation.lastMessage.isViewed ? (
              <span className="flex items-center gap-1">
                <FiLock size={12} /> {t('chat:messages.opened', 'Opened')}
              </span>
            ) : (
              <span className="flex items-center gap-1">
                <FiEyeOff size={12} />{' '}
                {t('chat:messages.view_once_message', 'View once message')}
              </span>
            )}
          </span>
        )
      ...
      if (conversation.lastMessage.preview !== undefined) {
        return (
          ...
          t('chat:messages.no_messages_yet', 'No messages yet')
        )
      }
      return (
    ...
    const handleContextMenu = (e: React.MouseEvent | React.TouchEvent) => {
      ...
      openMenu(e, [
        ...(peerUser
          ? [
              {
                label: t('chat:actions.view_profile', 'View Profile'),
                icon: <FiUser />,
                onClick: () => onUserClick(peerUser.id)
              }
            ]
          : []),
        {
          label: isPinnedByMe
            ? t('chat:actions.unpin_chat', 'Unpin Chat')
            : t('chat:actions.pin_chat', 'Pin Chat'),
          icon: <FiMaximize2 />,
        ...
        ...(!conversation.isGroup
          ? [
              {
                label: isBlocked
                  ? t('chat:actions.unblock_user', 'Unblock User')
                  : t('chat:actions.block_user', 'Block User'),
                icon: <FiSlash />,
                onClick: () => {
          ...
          : []),
        {
          label: conversation.isGroup
            ? t('chat:actions.delete_group', 'Delete Group')
            : t('chat:actions.delete_chat', 'Delete Chat'),
          icon: <FiTrash2 />,
          destructive: true,
          onClick: () =>
            onMenuSelect(conversation.isGroup ? 'deleteGroup' : 'deleteChat')
        }
      ])
    }
      ...
      >
        <SwipeableItem
          leftAction={{
            ...
            color: isPinnedByMe ? 'bg-blue-500' : 'bg-green-500',
            onAction: () => onTogglePin(conversation.id)
          }}
          rightAction={{
        ...
        >
          ...
          >
            ...
            <div className="relative flex-shrink-0">
                ...
                {avatarSrc ? (
                  <img
                    ...
                    className={clsx(
                      ...
                      isActive
                        ? 'border-bg-surface shadow-inner'
                        : 'border-bg-main shadow-sm',
                      cloakClass
                    )}
                  />
                ) : (
                  <DefaultAvatar
                    name={
                      conversation.isGroup
                        ? conversation.decryptedMetadata?.title || 'group'
                        : title
                    }
              ...
              {peerUser && (
                <span
                  className={`absolute bottom-0 right-0 w-3 h-3 rounded-full border-2 border-bg-surface ${isOnline ? 'bg-green-500' : 'bg-gray-400'}`}
                />
              )}
            </div>
                  ...
                  {isPinnedByMe && (
                    <span className="text-accent flex-shrink-0">
                      ...
                      >
                        ...
                        <path d="M22 21v-2a4 8 0 0 0-3-3.87" />
...
export default function ChatList() {
  ...
  const { blockedUserIds, blockUser, unblockUser } = useAuthStore(
    useShallow((state) => ({
  ...
  const { showConfirm, openProfileModal } = useModalStore(
    useShallow((state) => ({
  ...
  useEffect(() => {
    const commands = [
      ...
      {
        id: 'new-burner',
        name: t('chat:sidebar.new_burner', 'New Burner Chat'),
        action: handleCreateBurner,
        icon: <FiZap />,
        section: 'General',
        keywords: 'create burner temporary secure incognito'
      }
    ]
  ...
  const itemContent = useCallback(
    (index: number, c: Conversation) => {
      const peerUser = !c.isGroup
  ...
  return (
    ...
    >
        ...
        {error && !isLoading && (
          <div className="p-6 mx-4 text-center">
            <div className="text-red-500 font-bold mb-2 text-sm">
              {t('chat:sidebar.connection_error')}
            </div>
            <button
              onClick={handleRetry}
      ...
      {showBurnerModal && (
        <CreateBurnerModal onClose={() => setShowBurnerModal(false)} />
      )}
      {showScanModal && (
        <ScanQRModal
