...
const MessageStatusIcon = ({
  ...
  const otherParticipants =
    participants.filter((p: Participant) => p.id !== meId) || []
  if (otherParticipants.length === 0)
    return <FaCheck size={14} className="text-text-secondary" />

  const statuses = message.statuses || []
  const isReadAll = otherParticipants.every((p: Participant) =>
    statuses.some(
      (s: MessageStatus) => s.userId === p.id && s.status === 'READ'
    )
  )
  if (isReadAll) return <FaCheckDouble size={14} className="text-blue-500" />

  const isDeliveredAll = otherParticipants.every((p: Participant) =>
    statuses.some(
      (s: MessageStatus) => s.userId === p.id && s.status === 'DELIVERED'
    )
  )
  if (isDeliveredAll)
    return <FaCheckDouble size={14} className="text-text-secondary" />
  ...
  return (
    <div className="flex gap-1 mt-1.5">
      {Object.entries(grouped).map(([emoji, count]) => (
        <span
          key={emoji}
          className="px-2 py-0.5 rounded-full bg-bg-surface/80 text-text-primary text-xs cursor-default"
        >
          {emoji} {count > 1 ? count : ''}
        </span>
      ))}
...
const MessageItem = ({
  ...
  const setEditingMessage = useMessageInputStore(
  ...
  const profile = useUserProfile(
    message.sender as { id: string; encryptedProfile?: string | null }
  )
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) {
          ...
          const alreadyRead = message.statuses?.some(
            (s: MessageStatus) => s.userId === meId && s.status === 'READ'
          )
          if (!alreadyRead) {
  ...
  if (
    message.type === 'SYSTEM' ||
    message.content?.startsWith('You sent') ||
    message.content?.startsWith('Secure session') ||
    ...
    const getSystemIcon = (text: string) => {
      ...
      if (
        ...
        return <FiLock size={12} className="text-emerald-500" />
      if (lowerText.includes('file') || lowerText.includes('attachment'))
        return <FiPaperclip size={12} className="text-blue-400" />
      if (lowerText.includes('restart') || lowerText.includes('sync'))
        return <FiRefreshCw size={12} className="text-blue-500" />
      if (lowerText.includes('error') || lowerText.includes('failed'))
        return <FiAlertTriangle size={12} className="text-red-500" />
      return <FiInfo size={12} className="text-text-secondary" />
    }

    const isError =
      message.content?.includes('Error') ||
      message.content?.includes('Unreadable') ||
      message.content?.includes('Key out of sync')
          ...
          {isDesyncError && (
            <button
              onClick={() =>
                useMessageStore
                  .getState()
                  .repairSecureSession(message.conversationId, isGroup)
              }
  ...
  if (
    ...
    return (
      <div
        ref={ref}
        className={`flex items-center p-2 ${mine ? 'justify-end' : 'justify-start'}`}
      >
  ...
  const handleContextMenu = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    ;(
      window as { currentReactionHandler?: (emoji: string) => void }
    ).currentReactionHandler = reactToMessage
    ...
    openMenu(
      ...
      [
        ...
        ...(isEditable
          ...
          : []),
        {
          label: t('chat:actions.copy_text'),
          icon: <FiCopy />,
          onClick: () => navigator.clipboard.writeText(message.content || '')
        },
        {
          label: t('chat:actions.security_info'),
          icon: <FiShield />,
          onClick: () => toast(t('chat:messages.security_info'))
        },
        {
          label: t('chat:actions.copy_id'),
          icon: <FiInfo />,
  ...
  return (
    <motion.div
      ...
      className={clsx(
        ...
        isHighlighted && 'bg-accent/10 rounded-lg p-1 -mx-1'
      )}
    >
      <SwipeableItem
        leftAction={{
          ...
          onAction: () => setReplyingTo(message)
        }}
        rightAction={{
          icon: <FiInfo size={20} />,
          color: 'bg-bg-surface/80',
          onAction: () =>
            toast(t('chat:messages.message_id_toast', { id: message.id }))
        }}
      >
        <div
          onContextMenu={handleContextMenu}
          onClick={handleClick}
          className={`flex items-end gap-2 w-full select-none ${mine ? 'flex-row-reverse justify-start' : 'flex-row justify-start'}`}
        >
          {isSelectionMode && (
              ...
              <div
                className={clsx(
                  ...
                  isSelected
                    ...
                    : 'border-text-secondary/50 bg-black/20'
                )}
              >
              ...
              {isLastInSequence &&
                (profile.avatarUrl ? (
                  ...
                  <DefaultAvatar
                    ...
                    id={message.senderId}
                    className="w-8 h-8 bg-secondary shadow-sm cursor-pointer hover:scale-105 transition-transform pointer-events-auto"
                  />
                ))}
            </div>
          )}
          ...
          >
            {!mine && isGroup && profile.name && isFirstInSequence && (
              <p
                className="text-[10px] font-bold mb-1 ml-1 user-color-name cursor-pointer hover:underline uppercase tracking-wide pointer-events-auto"
                style={
                  {
                    '--user-color': getUserColor(message.senderId)
