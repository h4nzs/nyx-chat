...
export default function RecoveryPhraseModal({
  ...
  const renderStep2 = () => (
    <>
      <h2 className="text-xl font-black uppercase tracking-wide text-text-primary text-center mb-2">
        {t('modals:recovery.step2_title')}
      </h2>
      <p className="text-xs text-text-secondary text-center mb-6 font-mono">
        {t('modals:recovery.step2_desc')}
      </p>

      <div className="relative bg-bg-main p-6 rounded-2xl shadow-neumorphic-concave mb-6 border border-white/5 min-h-[200px]">
        {/* Content Layer */}
        <div
          className={`grid grid-cols-3 gap-3 transition-all duration-300 ${!showPhrase ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}
        >
          ...
          {words.map((word, index) => (
            <div
              key={index}
              className="flex items-center gap-2 p-2 rounded bg-black/5 dark:bg-white/5 border border-white/10"
            >
              <span className="text-[10px] text-text-secondary font-mono w-4 select-none">
                {index + 1}.
              </span>
              <span className="text-sm font-bold text-gray-900 dark:text-gray-100 tracking-wide select-all">
                {word}
              </span>
            </div>
          ))}
        ...
        {showPhrase && (
          <div className="absolute top-2 right-2 flex gap-2 z-20">
            <button
              ...
              aria-label={t(
                'modals:recovery.aria_toggle_visibility',
            ...
            <button
              ...
              aria-label={t(
                'modals:recovery.aria_copy_phrase',
  ...
  const renderStep3 = () => (
    <>
      <h2 className="text-xl font-black uppercase tracking-wide text-text-primary text-center mb-2">
        {t('modals:recovery.verify_title')}
      </h2>
      <p className="text-xs text-text-secondary text-center mb-6 font-mono">
        {t('modals:recovery.verify_desc')}
      </p>
      ...
      >
        {selectedIndices.length === 0 && (
          <span className="text-text-secondary/40">
            {t('modals:recovery.select_words')}
          </span>
        )}
        {selectedIndices.map((origIdx) => (
          <span
            key={origIdx}
            className="px-2 py-1 rounded bg-accent/20 text-accent border border-accent/30"
          >
            {words[origIdx]}
          </span>
        ))}
