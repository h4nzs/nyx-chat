  ...
  useEffect(() => {
    ...
    const mediaMessages = messages
      .filter((m) => {
        ...
        const hasFile =
          m.fileUrl && !m.content?.startsWith('waiting_for') && !m.error
    ...
    <div className="grid grid-cols-3 gap-2 p-1 max-h-[400px] overflow-y-auto">
      {media.map((item) => (
        ...
        >
          {item.type === 'IMAGE' ? (
            <img
              src={toAbsoluteUrl(item.content)}
              alt={item.fileName || t('media.shared_media')}
            ...
            <div className="flex flex-col items-center gap-2 text-center p-2">
              ...
              {item.fileName && (
                <span className="text-[10px] text-text-secondary truncate w-full px-1">
                  {item.fileName}
                </span>
              )}
            </div>
