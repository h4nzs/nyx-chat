...
export default function SystemInitModal() {
  ...
  const renderIcon = (status: PermStatus) => {
    if (status === 'loading')
      return <FiLoader className="animate-spin text-accent" />
    if (status === 'granted')
      return <FiCheckCircle className="text-green-500" />
    if (status === 'denied') return <FiXCircle className="text-red-500" />
    return (
      <div className="w-4 h-4 rounded-full border-2 border-text-secondary/50" />
    )
  }
      ...
      >
        ...
        >
          ...
          <div className="flex flex-col gap-3">
            ...
            >
              {notifStatus === 'loading' || mediaStatus === 'loading'
                ? t('system_init.calibrate')
                : t('system_init.authorize')}
            </button>
            <button
              onClick={close}
