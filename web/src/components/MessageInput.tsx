...
const EditPreview = () => {
  ...
  const { editingMessage, setEditingMessage } = useMessageInputStore(
    useShallow((s) => ({
      ...
      <div className="relative flex items-center justify-between bg-bg-main rounded-t-xl p-3 border-b border-accent/20 shadow-neumorphic-concave">
        <div className="flex flex-col border-l-2 border-accent pl-3">
          <span className="text-[10px] font-mono uppercase tracking-widest text-accent flex items-center gap-1">
            <FiEdit2 size={10} /> {t('input.editing')}
          </span>
...
const ReplyPreview = () => {
  const { t } = useTranslation('chat')
  const { replyingTo, setReplyingTo } = useMessageInputStore(
    useShallow((state) => ({
  ...
  const profile = useUserProfile(
    replyingTo?.sender as { id: string; encryptedProfile?: string | null }
  )
  ...
  if (replyingTo.duration) contentPreview = `[${t('input.voice_transmission')}]`
    ...
    contentPreview = `[${t('input.file_attachment', { fileName: replyingTo.fileName })}]`
      ...
      <div className="relative flex items-center justify-between bg-bg-main rounded-t-xl p-3 border-b border-accent/20 shadow-neumorphic-concave">
        <div className="flex flex-col border-l-2 border-accent pl-3">
          <span className="text-[10px] font-mono uppercase tracking-widest text-accent">
            {t('input.replying_to', { name: authorName })}
          </span>
          <span className="text-xs text-text-secondary truncate max-w-[200px]">
            {contentPreview}
          </span>
...
export default function MessageInput({
  ...
  const messages = useMessageStore(
    (state) => state.messages[conversation.id] || []
  )
  ...
  const otherParticipant =
    isOneToOne &&
    (conversation.participants as { id: string }[] | undefined)?.find(
  ...
  const isWaitingForGuest =
    isBurner && !burnerSession?.drState && !!useAuthStore.getState().user

  const isInputDisabled =
    !isConnected || isOtherParticipantBlocked || isWaitingForGuest

  const getPlaceholderText = () => {
    if (isWaitingForGuest)
      return 'Waiting for guest to establish secure connection...'
    return t('chat:input.placeholder')
  }

  const absoluteLastMessage =
    ...
    if (e.target.files && e.target.files.length > 0) {
      ...
      for (const file of selectedFiles) {
        if (file.size > MAX_FILE_SIZE) {
          toast.error(
            t('chat:messages.file_too_large', {
              name: file.name,
        ...
        if (restrictedExtensions.includes(ext)) {
          toast.error(
            t('chat:messages.file_restricted', {
              name: file.name,
  ...
  const handleSubmit = async (e?: React.FormEvent, forceSilent = false) => {
    ...
    if ((!currentText.trim() && stagedFiles.length === 0) || !isConnected)
      return
      ...
      {showTimerMenu && (
        ...
        >
          <div className="p-2 text-[10px] uppercase font-bold text-text-secondary border-b border-white/5">
            {t('chat:input.auto_delete', 'Auto-Delete')}
          </div>
          {DURATIONS.map((opt) => (
            <button
              key={opt.label}
      ...
      {showPlusMenu && (
        ...
        >
          ...
          >
            ...
            <span>
              {isHD
                ? t('chat:input.hd_on', 'HD Quality: ON')
                : t('chat:input.hd_off', 'Standard Quality')}
            </span>
          </button>
          <button
            onClick={() => {
          ...
          >
            <FiCpu
              size={18}
              className={
                isVoiceAnonymized ? 'text-red-500' : 'text-text-secondary'
              }
            />
            <span className={isVoiceAnonymized ? 'text-red-500' : ''}>
              {isVoiceAnonymized
                ? t('chat:input.anon_on', 'Anon Voice: ON')
                : t('chat:input.anon_off', 'Anon Voice: OFF')}
            </span>
          </button>
          <button
            onClick={() => {
      ...
      {isOtherParticipantBlocked ? (
        ...
        <div className="flex items-center gap-2 md:gap-4 animate-fade-in p-2 md:p-4 w-full">
          <button
            ...
            className="p-3 rounded-full text-text-secondary hover:text-red-500 hover:bg-red-500/10 transition-all flex-shrink-0"
          ...
          <div className="flex-1 bg-bg-main shadow-neu-pressed dark:shadow-neu-pressed-dark rounded-full h-12 flex items-center px-4 md:px-6 gap-3 min-w-0">
            ...
            <span className="hidden md:inline text-xs text-text-secondary uppercase tracking-wider ml-auto truncate">
              {t('input.recording_state')}
            </span>
          </div>
          <button
            onClick={handleStopRecording}
            className="p-3 rounded-full bg-accent text-white shadow-[0_0_15px_rgba(var(--accent),0.5)] hover:scale-110 active:scale-95 transition-all flex-shrink-0"
        ...
        <form
          ...
          className="relative flex items-center gap-2 p-2 rounded-2xl bg-bg-main w-full m-4 shadow-neu-pressed dark:shadow-neu-pressed-dark max-w-[calc(100%-2rem)]"
        >
          <div className="hidden md:flex items-center gap-1">
            <button
              ...
              className="p-3 rounded-xl text-text-secondary transition-all hover:text-accent active:scale-95 shadow-neu-icon dark:shadow-neu-icon-dark"
            >
              <FiPaperclip size={18} />
            </button>
            <button
            ...
            <button
              ...
              disabled={isInputDisabled}
              className="p-3 rounded-xl text-text-secondary transition-all hover:text-yellow-500 active:scale-95 shadow-neu-icon dark:shadow-neu-icon-dark"
            >
              <FiSmile size={18} />
            </button>
          </div>
          ...
          <div className="relative">
            <AnimatePresence>
              {showSilentMenu && (
                ...
                >
                  ...
                  >
                    <FiVolumeX className="text-accent" size={16} />{' '}
                    {t('input.send_silent')}
                  </button>
                </motion.div>
              )}
            ...
            {hasContentToSend ? (
              <button
                ...
                disabled={isInputDisabled}
                className="p-3 rounded-xl bg-accent text-white shadow-neu-flat dark:shadow-neu-flat-dark hover:-translate-y-0.5 active:translate-y-0 transition-all"
              >
                <FiSend
                  size={18}
              ...
              <button
                ...
                disabled={isInputDisabled}
                className="p-3 rounded-xl text-text-secondary shadow-neu-icon dark:shadow-neu-icon-dark hover:text-red-500 active:scale-95 transition-all"
              >
                <FiMic size={18} />
              </button>
            )}
