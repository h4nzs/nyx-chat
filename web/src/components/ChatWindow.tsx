      ...
      <div className="flex items-start gap-3">
        ...
        <div className="font-mono text-xs">
          <p className="font-bold uppercase tracking-wider mb-1">
            {t('banners.encryption_recommendation')}
          </p>
...
const ChatHeader = ({
  ...
  const { openProfileModal, openChatInfoModal } = useModalStore(
    useShallow((s) => ({
      openProfileModal: s.openProfileModal,
      openChatInfoModal: s.openChatInfoModal
    }))
  )
  ...
  const peerProfile = useUserProfile(
    peerUser as unknown as { id: string; encryptedProfile?: string | null }
  )
  ...
  const handleVoiceCall = async () => {
    if (peerUser) {
      ...
      startCall(
        peerUser.id,
        false,
        (user as unknown as MinimalProfile) || { id: user?.id || 'unknown' }
      )
    }
  }
  ...
  const handleDestroyBurner = async () => {
    ...
    import('react-hot-toast').then((m) =>
      m.default.success('Burner Session Destroyed')
    )
      ...
      <div className="flex items-center gap-4">
        ...
        >
          <div className="relative">
            ...
            {isOnline && (
              <div className="absolute bottom-0 right-0 w-2.5 h-2.5 bg-green-500 rounded-full border-2 border-bg-surface shadow-sm"></div>
            )}
          </div>
          ...
          <div className="text-left">
            <div className="flex items-center gap-2">
              <p
                className={clsx(
                  'font-bold text-text-primary text-sm group-hover:text-accent transition-colors',
                  cloakClass
                )}
              >
                {title}
...
export default function ChatWindow({
  ...
  const { conversation, messages, isLoading, error, actions, isFetchingMore } =
    useConversation(id)
  const {
  ...
  const handleStopRecording = useMessageInputStore(
    (state) => state.handleStopRecording
  )
  ...
  useEffect(() => {
    ...
    const markUnreadAsRead = async () => {
      ...
      const unreadVisible = messages.filter(
        (m) =>
          m.senderId !== meId &&
          visibleMessageIds.has(m.id) &&
          (!m.statuses ||
            !m.statuses.some((s) => s.userId === meId && s.status === 'READ'))
      )
  ...
  const handleImageClick = useCallback(
    (message: Message) => setLightboxMessage(message),
    []
  )
  ...
  const handleBulkDelete = () => {
    ...
    showConfirm(t('actions.bulk_delete_title'), confirmMessage, async () => {
      await removeMessages(conversation.id, selectedMessageIds)
      toast.success(
        t('messages.processed', { count: selectedMessageIds.length })
      )
    })
  }
  ...
  const typingUsersInThisConvo = typingIndicators.filter(
    (i) => i.conversationId === id && i.id !== meId && i.isTyping
  )
                ...
                <div className="h-16 flex items-center justify-between px-4 bg-accent/10 border-b border-white/5 backdrop-blur-md z-30">
                  <div className="flex items-center gap-4">
                    <button
                      ...
                      aria-label={t('common:actions.cancel_bracket')}
                      className="p-2 hover:bg-white/10 rounded-full transition-colors text-text-secondary hover:text-white"
                    >
                      <FiX size={20} />
                    </button>
                    <span className="font-bold text-lg text-accent tracking-wide">
                ...
                <div className="h-full px-4 md:px-6 pt-6 pb-2">
                  <Virtuoso
                    ...
                    computeItemKey={(index, item) =>
                      item.tempId
                        ? `virtuoso-t-${item.tempId}`
                        : `virtuoso-r-${item.id}`
                    } // 🔥 Ini menghilangkan kedip/glitch dari framework list
                  />
                </div>
