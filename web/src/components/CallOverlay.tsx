          ...
          <div className="relative">
            {avatarUrl ? (
              <img
                ...
                alt={name}
                className="w-20 h-20 rounded-full object-cover mb-2 border-2 border-accent/30 shadow-lg"
              />
            ) : (
              <DefaultAvatar
                name={name}
                id={userId}
                className="w-20 h-20 mb-2 border-2 border-accent/30 shadow-lg text-2xl"
              />
            )}
            {stream && (
...
export default function CallOverlay() {
  ...
  const mainRemoteUser = remoteUsers[0] || {
    id: 'unknown',
    name: t('common:defaults.someone', 'Someone')
  }
  const profileName =
    remoteUsers.length > 1
      ? `Group (${remoteUsers.length})`
      : mainRemoteUser.name || t('common:defaults.someone', 'Someone')
        ...
        >
          ...
          >
            ...
            <p className="text-text-secondary mb-8 text-sm uppercase font-black tracking-widest opacity-60">
              {isVideoCall
                ? t('chat:calls.incoming_video')
                : t('chat:calls.incoming_voice')}
            </p>
            ...
            <div className="flex gap-6">
              ...
              <button
                ...
                aria-label={
                  isVideoCall
                    ? t('chat:calls.actions.accept_video')
                    : t('chat:calls.actions.accept_voice')
                }
      ...
      {(callState === 'calling' || callState === 'connected') && (
        ...
        >
          ...
          {callState === 'connected' && !isMinimized && (
            <div
              className={`absolute top-6 right-20 px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-tighter flex items-center gap-2 shadow-lg backdrop-blur-md transition-colors ${networkQuality === 'Poor' ? 'bg-red-500/80' : networkQuality === 'Fair' ? 'bg-yellow-500/80' : 'bg-green-500/80'} text-white z-50`}
            >
          ...
          <div className="flex-1 relative flex items-center justify-center w-full h-full p-4 overflow-auto">
            {callState === 'calling' && !isMinimized && (
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center z-10">
                ...
                <p className="text-accent font-mono text-xs mt-2 animate-bounce">
                  {t('chat:calls.dialing')}
                </p>
              </div>
            )}
            ...
            {!isMinimized && isVideoCall && (
              <div className="absolute bottom-24 right-6 w-32 h-48 md:w-48 md:h-72 bg-gray-900 rounded-2xl overflow-hidden shadow-2xl border border-white/20 z-30">
                <video
                  ...
                  className="w-full h-full object-cover"
                />
                <div className="absolute bottom-2 left-2 px-1.5 py-0.5 bg-black/40 backdrop-blur-md rounded text-[8px] text-white/80 font-bold uppercase">
                  {t('common:defaults.you', 'YOU')}
                </div>
              </div>
            )}
            ...
            <div className="absolute bottom-0 left-0 right-0 p-8 flex justify-center gap-4 sm:gap-6 z-40">
              ...
              {isVideoCall && !isMobile && (
                <button
                  ...
                  aria-label={
                    isScreenSharing
                      ? t('chat:calls.actions.stop_sharing')
                      : t('chat:calls.actions.share_screen')
                  }
              ...
              <button
                ...
                aria-label={
                  isMuted
                    ? t('chat:calls.actions.unmute')
                    : t('chat:calls.actions.mute')
                }
              ...
              {isVideoCall && (
                <button
                  ...
                  aria-label={
                    isVideoOff
                      ? t('chat:calls.actions.video_on')
                      : t('chat:calls.actions.video_off')
                  }
              ...
              <button
                ...
                aria-label={
                  isSpeakerphone
                    ? t('chat:calls.actions.earpiece')
                    : t('chat:calls.actions.speaker')
                }
