...
export default function MessageBubble({
  ...
  const isPlaceholder =
    content === 'waiting_for_key' ||
    content.startsWith('[') ||
    content === 'Decryption failed'

  const cloakClass = privacyCloak
    ? 'blur-[6px] opacity-75 hover:blur-none hover:opacity-100 active:blur-none active:opacity-100 transition-all duration-300 select-none'
    : ''
  ...
  const getStatusIcon = () => {
    ...
    const readCount = statuses.filter(
      (s: MessageStatus) => s.status === 'READ' && s.userId !== myId
    ).length
    const deliveredCount = statuses.filter(
      (s: MessageStatus) => s.status === 'DELIVERED'
    ).length

    if (readCount > 0)
      return <FaCheckDouble size={14} className="text-green-400" />
    if (deliveredCount > 0)
      return <FaCheckDouble size={14} className="text-white/70" />
  ...
  const hasBubbleStyle =
    !isPlaceholder &&
    (!message.fileUrl || (message.fileUrl && !isImage && !isVoiceMessage))
  ...
  const bubbleClasses = clsx(
    'relative max-w-md md:max-w-lg shadow-neumorphic-bubble rounded-2xl',
    {
      ...
      'bg-bg-main text-text-secondary rounded-xl shadow-neumorphic-concave italic text-xs py-2 px-3':
        isDeleted,
      'rounded-bl-2xl': isOwn,
      'rounded-br-2xl': !isOwn,
      'rounded-br-sm': isOwn && isLastInSequence,
      'rounded-bl-sm': !isOwn && isLastInSequence,
  ...
  return (
    <div className={bubbleClasses}>
            ...
            {message.isViewOnce ? (
                ...
                {message.isViewed ? (
                  ...
                  >
                    ...
                    <span className="text-sm font-bold tracking-wider uppercase">
                      {t('messages.view_once', 'View Once')}
                    </span>
                  </button>
                )}
              </div>
              ...
              <>
                ...
                {message.fileUrl && isImage && (
                  <button
                    onClick={() => onImageClick?.(message)}
                    className="block w-full min-w-[200px] sm:min-w-[250px] relative"
                  >
                    <LazyImage
                      message={message}
                ...
                {!!content && (
                  <div className={message.fileUrl ? 'mt-2' : ''}>
                    {isPlaceholder ? (
                      ...
                      <div
                        className={clsx('text-base break-words w-full', {
                          'text-white/95': isOwn,
                          'text-text-primary': !isOwn
                        })}
                      >
                        <div
                          className={clsx(
                        ...
                        {isLongMessage && (
                          <button
                            ...
                            className={clsx(
                              'mt-2 text-xs font-bold uppercase tracking-wider block active:scale-95 transition-all',
                              {
      ...
      <div
        className={clsx(
          'text-xs mt-1.5 flex items-center gap-1.5 select-none',
          {
            'absolute bottom-2 right-2 bg-black/40 backdrop-blur-sm px-1.5 py-0.5 rounded text-white shadow-sm':
              isImage && !message.content && !message.isViewOnce,
            'justify-end': !isImage || message.content || message.isViewOnce,
            'text-white/80':
              isOwn && (!isImage || message.content || message.isViewOnce),
      ...
      >
        ...
        {message.isSilent && (
          <FiVolumeX
            size={12}
            className="opacity-60 text-text-secondary"
            title="Sent Silently"
          />
        )}
        {timeLeft && (
