          ...
          {showAllEmojis ? (
            ...
            >
              <div className="flex items-center p-2 border-b border-white/10 bg-secondary/30">
                ...
                <span className="text-xs font-bold text-text-secondary uppercase tracking-wider ml-2">
                  {t('context_menu.all_reactions')}
                </span>
              </div>
              <div className="p-1">
                <Suspense
                  fallback={
                    <div className="w-[300px] h-[400px] flex items-center justify-center text-text-secondary">
                      {t('actions.loading')}
                    </div>
                  }
                >
                  <EmojiPicker
                    onEmojiClick={(emojiData: EmojiClickData) => {
                      if (
                        ...
                        ).currentReactionHandler === 'function'
                      ) {
                        ;(
                          window as unknown as {
                            currentReactionHandler: (emoji: string) => void
                          }
                        ).currentReactionHandler(emojiData.emoji)
                      }
                      closeMenu()
                    }}
