...
export default function Lightbox({ message, onClose }: LightboxProps) {
  ...
  const handleClose = useCallback(() => {
    if (message.isViewOnce && !message.isViewed) {
      useMessageStore
        .getState()
        .updateMessage(message.conversationId, message.id, { isViewed: true })
      import('@lib/socket').then(({ getSocket }) => {
        const socket = getSocket()
        const emitViewedAck = () => {
          if (isMountedRef.current) {
            socket?.emit('message:view_once_opened', {
  ...
  useEffect(() => {
    ...
    const handleDecryption = async () => {
      ...
      try {
        ...
        let originalType =
          message.fileType?.split(';')[0] || 'application/octet-stream'
        ...
        {!isLoading &&
          ...
          (isVideo ? (
            <video
              ...
              controls
              playsInline
              className="max-w-full max-h-[90vh] shadow-2xl rounded-lg outline-none bg-black"
              onEnded={handleClose}
            />
          ) : isAudio ? (
