...
export default function Lightbox({ message, onClose }: LightboxProps) {
  ...
  const content = (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/95 backdrop-blur-xl animate-fade-in p-4 md:p-8" onClick={handleClose}>
      <button className="absolute top-4 right-4 text-white text-3xl hover:opacity-80 transition-opacity z-10" onClick={handleClose}>&times;</button>
      <div className="relative max-w-[90vw] max-h-[90vh] flex items-center justify-center" onClick={(e) => e.stopPropagation()}>
        ...
        {!isLoading && !error && decryptedUrl && (
          <img src={decryptedUrl} alt={message.fileName || "Lightbox view"} className="object-contain max-w-full max-h-[90vh] select-none shadow-2xl rounded-lg" onError={() => { setError("Failed to load image."); setIsLoading(false); }} />
        )}
      </div>
    </div>
  );
