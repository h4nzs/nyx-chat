...
export default function FileAttachment({
  ...
  const [status, setStatus] = useState<
    'idle' | 'decrypting' | 'waiting' | 'error' | 'success'
  >('idle')
  ...
  const isGroup = useConversationStore(
    (s) =>
      s.conversations.find((c) => c.id === message.conversationId)?.isGroup ||
      false
  )
  ...
  const getFileType = () => {
    if (
      message.fileType &&
      !message.fileType.includes('application/octet-stream')
    ) {
  ...
  useEffect(() => {
    ...
    const handleDecryption = async () => {
      ...
      const isEncrypted =
        !!message.fileKey ||
        message.fileType?.includes('encrypted=true') ||
        message.isBlindAttachment ||
        !message.fileUrl
      ...
      try {
        ...
        let decryptedBlob = await decryptFile(
          encryptedBlob,
          rawFileKey,
          originalType
        )
        ...
        if (isMounted) {
          const errorMessage =
            e instanceof Error
              ? e.message
              : t('chat:media.default_error', 'Failed to decrypt file')
  ...
  if (status === 'decrypting') {
    return (
      <div className={containerClass}>
        <Spinner size="sm" />
        <span className="text-sm">{t('chat:media.decrypting')}</span>
      </div>
    )
  }
  ...
  if (status === 'waiting') {
    return (
      <div
        className={`${containerClass} border border-yellow-500/30 text-yellow-600`}
      >
    ...
    return (
      ...
      >
        ...
        <span className="text-sm">
          {t('chat:media.decrypt_failed', { error: errorMsg })}.{' '}
          {t('chat:media.retry')}
        </span>
      </div>
    )
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex-1 min-w-0">
            <p className="font-medium text-sm truncate">
              {message.fileName ||
                t('chat:media.document_placeholder', 'Document.pdf')}
            </p>
            <p className="text-xs opacity-60">
              {message.fileSize
                ? formatBytes(message.fileSize)
                : t('common:defaults.unknown', 'Unknown')}
            </p>
          </div>
        </div>
    ...
    return (
      <div className={containerClass}>
        <div className="flex items-center gap-2">
          ...
          <div className="flex-1 min-w-0">
            <p className="font-medium text-sm truncate">
              {message.fileName || t('chat:media.image_placeholder', 'Image')}
            </p>
            <p className="text-xs opacity-60">
              {message.fileSize
                ? formatBytes(message.fileSize)
        ...
        <div className="mt-2 border rounded overflow-hidden max-h-64">
          <img
            src={decryptedUrl}
            alt={
              message.fileName || t('chat:media.image_alt', 'Image attachment')
            }
    ...
    return (
      <div className={containerClass}>
        <div className="flex items-center gap-2">
          ...
          <div className="flex-1 min-w-0">
            <p className="font-medium text-sm truncate">
              {message.fileName || t('chat:media.video_placeholder', 'Video')}
            </p>
            <p className="text-xs opacity-60">
              {message.fileSize
                ? formatBytes(message.fileSize)
    ...
    return (
      <div className={containerClass}>
        <div className="flex items-center gap-2">
          ...
          <div className="flex-1 min-w-0">
            <p className="font-medium text-sm truncate">
              {message.fileName || t('chat:media.audio_placeholder', 'Audio')}
            </p>
            <p className="text-xs opacity-60">
              {message.fileSize
                ? formatBytes(message.fileSize)
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="font-medium text-sm truncate">
          {message.fileName || t('chat:media.file_placeholder', 'File')}
        </p>
        <p className="text-xs opacity-60">
          {message.fileSize
            ? formatBytes(message.fileSize)
