...
export async function sendPushNotification(
  ...
  try {
    ...
    const notifications = subscriptions.map(async (sub) => {
      let devicePayload =
        payload.data?.pushPayloadMap?.[sub.deviceId] ||
        payload.data?.encryptedPushPayload
      ...
      try {
        ...
        if (statusCode === 410 || statusCode === 404) {
          ...
          console.error(
            `Error sending push notification for sub ${sub.id}: statusCode=${statusCode || 'unknown'}`
          )
        }
      }
    })
