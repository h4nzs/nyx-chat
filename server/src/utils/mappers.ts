...
export const toMinimalProfile = (
  user: PrismaUserProfileInput
): MinimalProfile => ({
  id: asUserId(user.id),
  username: '',
...
export const toRawServerMessage = (
  msg: PrismaMessageInput,
  maxDepth = 1
): RawServerMessage => ({
  ...
  repliedTo:
    msg.repliedTo && maxDepth > 0
      ? toRawServerMessage(msg.repliedTo, maxDepth - 1)
      : undefined
})
