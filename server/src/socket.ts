  ...
  io = new Server<ClientToServerEvents, ServerToClientEvents>(httpServer, {
    cors: {
      origin: (origin, callback) => {
        ...
        const baseOrigins = Array.isArray(env.corsOrigin)
          ? env.corsOrigin
          : [env.corsOrigin]
  ...
  io.use(async (socket: AuthenticatedSocket, next) => {
    try {
      ...
      const payload = verifyJwt(token) as
        | { id?: string; sub?: string; role?: string; deviceId?: string }
        | null
        | string
      ...
      const user = await prisma.user.findUnique({
        ...
        select: {
          ...
          devices: {
            where: { id: deviceId },
            select: { publicKey: true, pqPublicKey: true }
          }
        }
      })
      ...
      socket.user = {
        ...
        pqPublicKey: user.devices[0].pqPublicKey
          ? Buffer.from(user.devices[0].pqPublicKey).toString('base64url')
          : null,
        deviceId: deviceId
      }
      next()
  ...
  io.on('connection', async (socket: AuthenticatedSocket) => {
    ...
    socket.on('burner:join', async (payload: { roomId: string }) => {
      if (payload && payload.roomId && typeof payload.roomId === 'string') {
        const isTerminated = await redisClient.exists(
          `burner:terminated:${payload.roomId}`
        )
        if (isTerminated) {
    ...
    socket.on(
      ...
      async (
        ...
        if (!payload.targetDeviceId || !payload.hostUserId) {
          return callback?.({
            ok: false,
            error: 'Invalid burner routing metadata'
          })
        }
        const isTerminated = await redisClient.exists(
          `burner:terminated:${payload.roomId}`
        ...
        try {
          ...
          if (delivered) {
            ...
            callback?.({
              ok: false,
              error: 'Host device is offline or unavailable.'
            })
          }
        } catch (e) {
    ...
    socket.on(
      ...
      async (payload: { roomId: string; ciphertext: string }) => {
        if (payload?.roomId && payload?.ciphertext) {
          ...
          io.to(payload.roomId).emit('burner:receive', {
            roomId: payload.roomId,
            ciphertext: payload.ciphertext
          })
        }
    ...
    socket.on(
      'burner:destroy',
      async (payload: { roomId: string; hostUserId?: string }) => {
        if (payload?.roomId) {
          if (payload.hostUserId && socket.user?.id !== payload.hostUserId)
            return
          if (!socket.user && !payload.hostUserId) return

          await redisClient.set(`burner:terminated:${payload.roomId}`, '1', {
            EX: 86400
          })
          io.to(payload.roomId).emit('burner:terminated', {
            roomId: payload.roomId
          })
    ...
    if (!userId) {
      socket.on(
        ...
        async (
          ...
          callback?: (res: { ok: boolean; qrData?: string }) => void
        ) => {
    ...
    socket.on(
      ...
      async (
        { conversationId, keys }: DistributeKeysPayload,
        ...
        try {
          ...
          const validKeys = keys.filter(
            (k) => k.userId && k.key && validParticipantIds.has(k.userId)
          )
    ...
    socket.on(
      ...
      async (
          ...
          try {
            ...
            if (count > 5)
              return callback?.({
                ok: false,
                error:
                  'SANDBOX_LIMIT_REACHED: Max 5 messages per minute. Verify account to unlock.'
              })
          } catch (redisError) {
            return callback?.({
              ok: false,
        ...
        try {
          ...
          if (tempId !== undefined) {
            const parsed =
              typeof tempId === 'string' ? parseInt(tempId, 10) : tempId
            ...
            if (participant.userId !== userId) {
              sendPushNotification(participant.userId, {
                ...
                data: {
                  ...
                  pushPayloadMap: pushPayloads || undefined
                }
              }).catch(console.error)
            }
          })
      ...
      async ({
        ...
        try {
          ...
          await prisma.message.deleteMany({
            where: { id: messageId, senderId: uid }
          })
          socket
            .to(conversationId)
            .emit('message:deleted_remotely', {
              messageId,
              conversationId,
              deletedBy: uid
            })
    ...
    socket.on(
      ...
      async ({
        ...
        try {
          ...
          socket
            .to(conversationId)
            .emit('message:viewed', { messageId, conversationId })
        } catch (error) {
          processLogger.error(error)
        }
      }
    ...
    socket.on(
      ...
      async ({ messageId, conversationId }: MarkAsReadPayload) => {
        if (!messageId || !socket.user) return
        const uid = socket.user.id
      ...
      async ({
        ...
        try {
          ...
          if (targetSenderId) {
            ...
            if (targetDeviceKey) {
              const matchingSocket = targetSockets.find(
                (s) =>
                  (s as unknown as AuthenticatedSocket).user?.publicKey ===
                  targetDeviceKey
              )
              if (matchingSocket) {
          ...
          if (!fulfillerSocket) {
            ...
            const onlineParticipants = participants.filter((p) =>
              allOnlineUsers.includes(p.userId)
            )
          ...
          if (fulfillerSocket && fulfillerId) {
            ...
            if (requesterPublicKey && requesterPqPublicKey) {
              ...
              socket.emit('group:key_request_failed', {
                conversationId,
                reason: 'Missing classical or PQ public key'
              })
            }
          }
        } catch (error) {
      ...
      if (targetId) {
        try {
          ...
          if (
            ...
            !participantIds.includes(targetId)
          )
            return
        ...
        if (onlineParticipants.length > 0) {
          ...
          if (requesterPublicKey && requesterPqPublicKey) {
            io.to(fulfillerId).emit('session:fulfill_request', {
              ...
              requesterPqPublicKey: requesterPqPublicKey
            })
          } else {
            socket.emit('session:request_key_failed', {
              sessionId,
              targetId: fulfillerId,
              reason: 'Missing PQ or classical public key'
            })
          }
        }
      } catch (error) {
    ...
    socket.on(
      ...
      async (payload: KeyFulfillmentPayload) => {
        const {
          requesterId,
          conversationId,
          sessionId,
    ...
    socket.on(
      'webrtc:secure_signal',
      async (data: { to: string; type: string; payload: string }) => {
        if (!data || !data.to) return
        if (!(await checkRateLimit(userId, 'webrtc_signal', 20, 60))) {
          socket.emit('error', {
            message: 'Rate limit exceeded for WebRTC signaling'
          })
          return
        }
        socket
          .to(data.to)
          .emit('webrtc:secure_signal', {
            from: userId,
            type: data.type,
    ...
    socket.on(
      ...
      async (data: {
        ...
        if (
          ...
          !data.roomId.startsWith('mig_') ||
          data.roomId.length <= 20
        )
          return
        ...
        await redisClient.set(`migration_owner:${data.roomId}`, userId, {
          EX: 3600
        })
