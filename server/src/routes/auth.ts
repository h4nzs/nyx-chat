...
async function issueTokens(
  ...
  const jti = newJti()
  const refresh = signAccessToken(
    { sub: user.id, jti, deviceId },
    { expiresIn: '30d' }
  )

  const rawIp = req.ip || ''
  const sodium = await getSodium()
  const ipAddress = sodium
...
async function verifyTurnstileToken(token: string): Promise<boolean> {
  if (env.nodeEnv !== 'production' && !process.env.TURNSTILE_SECRET_KEY)
    return true
...
const validateKey = (expectedBytes?: number) =>
  z
    .string()
    .refine((val) => isValidBase64Url(val, expectedBytes), {
      message: `Invalid key format or length (expected ${expectedBytes || 'valid base64url'})`
    })
  ...
  async (req, res, next) => {
    try {
      ...
      if (!isHuman)
        throw new ApiError(
          400,
          'Bot detected. Please try again or reload page.'
        )
      ...
      if (existingUser)
        throw new ApiError(409, 'Username already taken (Hash Collision).')
    ...
    try {
      ...
      if (publicKey && signingKey && encryptedPrivateKey) {
        ...
        if (device) {
          device = await prisma.device.update({
            ...
            data: {
              ...
              name:
                deviceName || getGenericDeviceName(req.headers['user-agent'])
            }
          })
        } else {
        ...
        activeEncryptedPrivateKey = device.encryptedPrivateKey
          ? Buffer.from(device.encryptedPrivateKey).toString('utf8')
          : ''
      } else {
        ...
        if (explicitDeviceId) {
          ...
          if (user.devices.length === 0)
            throw new ApiError(
              404,
              'No device found. Please recover your account.'
            )
          activeDeviceId = user.devices[0].id
          activeEncryptedPrivateKey = user.devices[0].encryptedPrivateKey
            ? Buffer.from(user.devices[0].encryptedPrivateKey).toString('utf8')
            : ''
          await prisma.device.update({
...
router.post('/refresh', async (req, res, next) => {
  try {
    ...
    const payload = verifyJwt(token) as
      | { jti?: string; sub?: string; deviceId?: string }
      | string
      | null
    if (
    ...
    if (user.bannedAt)
      throw new ApiError(
        403,
        `ACCESS DENIED: ${user.banReason || 'Account suspended'}`
      )
  ...
  async (req, res, next) => {
    try {
      ...
      if (!cachedNonce || cachedNonce !== nonce)
        throw new ApiError(401, 'Invalid or expired recovery challenge.')
      ...
      if (!user || user.devices.length === 0)
        throw new ApiError(404, 'User not found or invalid keys.')
      ...
      if (!publicKey || !signingKey || !newEncryptedKeys) {
        throw new ApiError(
          400,
          'Missing required cryptographic keys or encrypted private keys.'
        )
      }
        ...
        prisma.device.create({
          data: {
            ...
            encryptedPrivateKey: newEncryptedKeys
              ? Buffer.from(newEncryptedKeys, 'utf8')
              : null,
...
router.get('/pow/challenge', requireAuth, async (req, res, next) => {
  try {
    ...
    if (!ip && !userId) {
      throw new ApiError(
        400,
        'Cannot determine client identifier for PoW challenge.'
      )
    }

    const identifier = ip || userId
    const stableHash = sodium
      .to_hex(
        sodium.crypto_generichash(32, Buffer.from(String(identifier)), null)
      )
      .slice(0, 16)
    const rateKey = ip
      ? `pow:ip_count:${stableHash}`
      : `pow:user_count:${stableHash}`
    ...
    await redisClient.setEx(
      `pow:challenge:${req.user!.id}`,
      300,
      JSON.stringify({ salt, difficulty })
    )
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      if (!identifier)
        throw new ApiError(400, 'Cannot determine client identifier.')
      ...
      if (hash.startsWith('0'.repeat(difficulty))) {
        await redisClient.del(`pow:challenge:${identifier}`)
        if (userId) {
        ...
        res.json({
          success: true,
          message: 'Account verified via Proof of Work'
        })
      } else {
        throw new ApiError(
          400,
          'Invalid Proof of Work. Hash does not meet difficulty target.'
        )
      }
    } catch (e) {
      next(e)
  ...
  async (req, res, next) => {
    try {
      ...
      type ExcludeCredential = NonNullable<
        GenOpts['excludeCredentials']
      >[number]

      const excludeCredentials = forceNew
        ? []
        : userAuthenticators.reduce((acc: ExcludeCredential[], auth) => {
            try {
              if (!auth.credentialID) return acc
              const base64 = String(auth.credentialID)
                .replace(/-/g, '+')
                .replace(/_/g, '/')
              ...
              acc.push({
                ...
                transports: auth.transports
                  ? (auth.transports.split(
                      ','
                    ) as ExcludeCredential['transports'])
                  : undefined
              })
            } catch (e) {
              console.warn(
                `Skipping invalid credential ID: ${auth.credentialID}`,
                e
              )
            }
            return acc
  ...
  async (req, res, next) => {
    try {
      ...
      if (!user || !user.currentChallenge)
        throw new ApiError(400, 'No challenge found.')
      ...
      if (verification.verified && verification.registrationInfo) {
        ...
        await prisma.user.update({
          where: { id: user.id },
          data: { currentChallenge: null, isVerified: true }
        })
    ...
    const verification = await verifyAuthenticationResponse({
      ...
      credential: {
        ...
        transports: userAuthenticator.transports
          ? (userAuthenticator.transports.split(',') as NonNullable<
              VerifyOpts['credential']
            >['transports'])
          : undefined
      },
    ...
    if (verification.verified) {
      ...
      if (safeUser.bannedAt)
        return res
          ...
          .json({
            error: 'ACCESS DENIED: Your account has been suspended.',
            reason: safeUser.banReason
          })

      const activeDeviceId = safeUser.devices[0]?.id
      if (!activeDeviceId)
        throw new ApiError(404, 'No device found for this account.')
      ...
      res.json({
        ...
        user: {
          id: safeUser.id,
          usernameHash: safeUser.usernameHash,
          encryptedProfile: safeUser.encryptedProfile,
