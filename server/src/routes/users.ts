...
router.get('/search', async (req, res, next) => {
  try {
    ...
    if (!user?.isVerified)
      throw new ApiError(
        403,
        'SANDBOX_SEARCH_RESTRICTION: Unverified users cannot search for other users.'
      )
    ...
    const mappedUsers = users.map((u) => {
      ...
      return {
        ...
        publicKey: device?.publicKey
          ...
          : undefined,
        pqPublicKey: device?.pqPublicKey
...
router.delete('/me/devices/:deviceId', requireAuth, async (req, res, next) => {
  try {
    ...
    if (targetDeviceId === authUser.deviceId) {
      throw new ApiError(
        400,
        'Cannot revoke the current device from here. Use the Logout button instead.'
      )
    }
...
router.put(
  ...
  zodValidate({
    body: z
      ...
      .refine(
        (data) =>
          data.encryptedProfile !== undefined ||
          data.autoDestructDays !== undefined,
  ...
  async (req, res, next) => {
    try {
      ...
      if (encryptedProfile !== undefined)
        dataToUpdate.encryptedProfile = encryptedProfile
      if (autoDestructDays !== undefined)
        dataToUpdate.autoDestructDays = autoDestructDays
      ...
      if (
        ...
        (!existingUser || encryptedProfile !== existingUser.encryptedProfile)
      ) {
        getIo()
          .to(userId)
          .emit('user:updated', {
  ...
  zodValidate({
    body: z.object({
      publicKey: z
        ...
        .min(43)
        .max(256)
        .regex(base64UrlRegex, { message: 'Invalid public key format.' }),
      pqPublicKey: validateKey(1216),
      signingKey: z
        .string()
        .min(43)
        .max(256)
        .regex(base64UrlRegex, { message: 'Invalid signing key format.' })
    })
  }),
  async (req, res, next) => {
    try {
...
router.delete(
  ...
  async (req, res, next) => {
    try {
      ...
      const isPasswordValid = await import('../utils/password.js').then((m) =>
