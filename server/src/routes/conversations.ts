...
const userSelectWithKeys = {
  ...
  devices: {
    select: { id: true, publicKey: true, pqPublicKey: true, signingKey: true },
...
type UserWithDevices = {
  ...
  devices?: {
    ...
    pqPublicKey?: string | Uint8Array | null
...
const hoistKeys = (u: UserWithDevices | null) => {
  ...
  if (u.devices && u.devices.length > 0) {
    result.devices = u.devices.map((d) => ({
      id: d.id,
      publicKey:
        Buffer.isBuffer(d.publicKey) || d.publicKey instanceof Uint8Array
          ? Buffer.from(d.publicKey).toString('base64url')
          : d.publicKey,
      pqPublicKey: d.pqPublicKey
        ? Buffer.isBuffer(d.pqPublicKey) || d.pqPublicKey instanceof Uint8Array
          ? Buffer.from(d.pqPublicKey).toString('base64url')
          : d.pqPublicKey
        : null,
      signingKey:
        Buffer.isBuffer(d.signingKey) || d.signingKey instanceof Uint8Array
          ? Buffer.from(d.signingKey).toString('base64url')
          : d.signingKey
    }))
    ...
    result.publicKey =
      Buffer.isBuffer(pk) || pk instanceof Uint8Array
        ? Buffer.from(pk).toString('base64url')
        : pk
    result.pqPublicKey = pqk
      ? Buffer.isBuffer(pqk) || pqk instanceof Uint8Array
        ? Buffer.from(pqk).toString('base64url')
        : pqk
      : null
    result.signingKey =
      Buffer.isBuffer(sk) || sk instanceof Uint8Array
...
const hoistConvoKeys = (c: RawConversationData) => {
  ...
  if (c.participants) {
    result.participants = c.participants.map((p) => ({
      ...p,
      user: hoistKeys(p.user)
    }))
  }
  if (c.messages) {
    result.messages = c.messages.map((m) => ({
      ...m,
      sender: hoistKeys(m.sender)
    }))
  }
...
router.get('/', async (req, res, next) => {
  try {
    ...
    const safeConversations = conversationsData.map((convo) => {
      const safeConv = toConversation(
        hoistConvoKeys(convo as unknown as RawConversationData)
      )
...
const initialSessionSchema = z
  .object({
    sessionId: z.string(),
    initialKeysPerDevice: z.record(
      z.string(),
      z.string().regex(/^[A-Za-z0-9_-]+$/, 'Must be base64url')
    ),
    initiatorCiphertextsPerDevice: z.record(
      z.string(),
      z.string().regex(/^[A-Za-z0-9_-]+$/, 'Must be base64url')
    )
  })
  ...
  async (req, res, next) => {
    try {
      ...
      if (!isVerified && isGroup)
        throw new ApiError(
          403,
          'SANDBOX_GROUP_RESTRICTION: Unverified users cannot create groups.'
        )
      ...
      if (!isGroup) {
        ...
        const otherUserId = isSelfChat
          ? creatorId
          : userIds.find((id: string) => id !== creatorId)
        if (!otherUserId)
          return res
            .status(400)
            .json({ error: 'Another user ID is required for a private chat.' })
        ...
        if (isSelfChat) {
          const selfChats = await prisma.conversation.findMany({
            ...
            include: {
              participants: {
                include: { user: { select: userSelectWithKeys } }
              },
              creator: { select: userSelectWithKeys }
            }
          })
          existingConversation = selfChats.find(
        ...
        if (existingConversation)
          return res
            ...
            .json(
              toConversation(
                hoistConvoKeys(
                  existingConversation as unknown as RawConversationData
                )
              )
            )
        ...
        if (!isVerified) {
          ...
          const count = (await redisClient.eval(
            ...
            { keys: [key], arguments: ['86400'] }
          )) as number
          if (count > 3)
            throw new ApiError(
              429,
              'SANDBOX_NEW_CHAT_LIMIT: Max 3 new conversations per day.'
            )
        }
      }
      ...
      try {
        newConversation = await prisma.$transaction(async (tx) => {
          ...
          if (initialSession) {
            const {
              sessionId,
              initialKeysPerDevice,
              initiatorCiphertextsPerDevice
            } = initialSession
            if (
              !sessionId ||
              !initialKeysPerDevice ||
            ...
            for (const d of devices) {
              ...
              if (!deviceCiphertext || !deviceKey) {
                console.error(
                  'Missing initiator ciphertext or key for participant device during conversation creation'
                )
                throw new Error(
                  'Missing initiator ciphertext or key for a participant device'
                )
              }
              keyRecords.push({
                sessionId,
                ...
                initiatorCiphertexts: Buffer.from(
                  deviceCiphertext,
                  'base64url'
                ),
        ...
        if (!isVerified && !isGroup) {
          const today = new Date().toISOString().split('T')[0]
          try {
            await redisClient.decr(`sandbox:newchat:${creatorId}:${today}`)
          } catch (_e) {}
        }
        throw dbError
      }

      const safeConversation = toConversation(
        hoistConvoKeys(newConversation as unknown as RawConversationData)
      )

      getIo()
        .to(allUserIds.filter((uid) => uid !== creatorId))
        .emit('conversation:new', safeConversation)
...
router.get('/:id', async (req, res, next) => {
  try {
    ...
    res.json(
      toConversation(
        hoistConvoKeys(conversation as unknown as RawConversationData)
      )
    )
  } catch (error) {
    next(error)
...
router.put('/:id/details', async (req, res, next) => {
  try {
    ...
    if (!participant || participant.role !== 'ADMIN')
      return res
        .status(403)
        .json({ error: 'Forbidden: You are not an admin of this group.' })
    ...
    getIo()
      ...
      .emit('conversation:updated', {
        ...
        encryptedMetadata: updatedConversation.encryptedMetadata ?? undefined
      })
  ...
  try {
    ...
    if (currentCount + userIds.length > MAX_GROUP_MEMBERS)
      return res
        .status(400)
        .json({
          error: `Group limit reached (${MAX_GROUP_MEMBERS} members max).`
        })
    ...
    const conversationRaw = await prisma.conversation.findUnique({
      ...
      include: {
        participants: { include: { user: { select: userSelectWithKeys } } },
        creator: { select: userSelectWithKeys }
      }
    })
    ...
    getIo()
      .to(conversationId)
      .emit('conversation:participants_added', {
        conversationId: safeConversationId,
        newParticipants: safeParticipants as unknown as {
          id: string
          role: 'ADMIN' | 'MEMBER'
          user: User
          isPinned: boolean
    ...
    getIo()
      .to(conversationId)
      .emit('group:participants_changed', {
        conversationId: safeConversationId
      })

    if (conversationRaw) {
      const safeConv = toConversation(
        hoistConvoKeys(conversationRaw as unknown as RawConversationData)
      )
...
router.put('/:id/participants/:userId/role', async (req, res, next) => {
  try {
    ...
    getIo()
      .to(conversationId)
      .emit('conversation:participant_updated', {
        conversationId: asConversationId(conversationId),
        userId: asUserId(userToModifyId),
        role
      })
  ...
  try {
    ...
    getIo()
      ...
      .emit('conversation:participant_removed', {
        conversationId: asConversationId(conversationId),
        userId: asUserId(userToRemoveId)
      })
    getIo()
      .to(conversationId)
      .emit('group:participants_changed', {
    ...
    getIo()
      .to(userToRemoveId)
      .emit('conversation:deleted', { id: asConversationId(conversationId) })
...
router.delete('/:id/leave', async (req, res, next) => {
  try {
    ...
    if (conversation?.creatorId === userId)
      return res.status(400).json({ error: 'Creator cannot leave.' })

    await prisma.$transaction(async (tx) => {
      await tx.participant.delete({
        where: { userId_conversationId: { userId, conversationId } }
      })
      const remainingAdmin = await tx.participant.findFirst({
...
router.delete('/:id', async (req, res, next) => {
  try {
    ...
    if (
      ...
      return res.status(404).json({ error: 'Not found or not participant.' })
    ...
    if (conversation.isGroup) {
      ...
      getIo()
        .to(conversation.participants.map((p) => p.userId))
        .emit('conversation:deleted', { id: asConversationId(id) })
    } else {
...
router.post('/:id/key-rotation', async (req, res, next) => {
  try {
    ...
    const safeConv = toConversation(
      hoistConvoKeys(updatedConversation as unknown as RawConversationData)
    )
    res.json({
      success: true,
