...
const ensureLegacyMessageFields = <T extends Record<string, unknown>>(
  msg: T
) => ({
  ...msg,
...
router.get('/:conversationId', async (req, res, next) => {
  try {
    ...
    const safeMessages = messages.map((msg) =>
      toRawServerMessage(ensureLegacyMessageFields(msg))
    )
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const safeMessage = toRawServerMessage(
        ensureLegacyMessageFields(newMessageRaw)
      )
      ...
      if (tempId !== undefined) {
        if (typeof tempId === 'number') {
          ...
          safeMessage.tempId = /^\d+$/.test(tempId)
            ? parseInt(tempId, 10)
            : tempId
        }
      }
    ...
    if (r2Key) {
      ...
      if (!filename.startsWith(`${userId}-`)) {
        console.warn(
          ...
          'attempted to delete unauthorized file:',
          sanitizeForLog(safeR2Key)
        )
        return res.status(403).json({ error: 'Unauthorized file deletion' })
      } else {
        console.log(
          '[R2] Deleting blind attachment:',
          sanitizeForLog(safeR2Key)
        )
        try {
          ...
          console.error(
            '[R2] Failed to delete blind file:',
            sanitizeForLog(safeR2Key),
            ':',
