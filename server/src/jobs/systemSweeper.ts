  ...
  cron.schedule('0 3 * * *', async () => {
    ...
    try {
      ...
      if (deletedTokens.count > 0) {
        console.log(
          `[Cron] Berhasil menghapus ${deletedTokens.count} token kadaluarsa.`
        )
      }
      ...
      if (deletedSessionKeys.count > 0) {
        console.log(
          `[Cron] Berhasil menghapus ${deletedSessionKeys.count} kunci sesi kadaluarsa/usang.`
        )
      }
      ...
      for (const u of usersWithSwitch) {
        ...
        if (now > deadline) {
          console.log(
            `[Cron] 💀 DEAD MAN'S SWITCH TRIGGERED for User ${u.id}. Erasing all traces...`
          )
          try {
            ...
            console.error(
              '[Cron] Failed to execute Dead Man Switch for user:',
              u.id,
              err
            )
          }
